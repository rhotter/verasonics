...
%   for its use
%
% File name: SetUpL11_4vWideBeamDoppler_32LE.m - Example of WideBeam doppler
...
%
% Description:
%   Sequence programming file for L11-4v Linear array, using WideBeam
%   transmits for 2D (B-mode) and Doppler imaging.
%   - this version uses TPC profiles to provide a seperate high voltage
%     level for Doppler.
%   - Of the 128 transducer elements, only numTx are used, with numTx transmitters on
%     each side of the center element (where possible).
%   - Acquisition of Doppler ensembles is interleaved - e.g. for 4 Doppler beams, each
%     beam is acquired at 4x the Doppler PRF, repeated for the ensemble length.
...
%   08/31/18 - modified for Vantage 32LE system.
...
if mod(P.numTx,2) == 0 , P.numTx = P.numTx+1;end
...
% Set 2D parameters in wavelengths
...
P.dopDispEle = 88; % 88 elements width will be used for Doppler display
P.dopPRIs = 16;
...
P.persfreq = 70;
P.perspwr = 90;
P.cpl = 20;  % level at which 2D overwrites Doppler
...
TxOrgX = (-63.5*Trans.spacing):(Trans.numelements-1)*Trans.spacing/(P.numRays-1):(63.5*Trans.spacing);
...
TxDopOrgX = ([16 38 60 82]-63.5)*Trans.spacing;
for n = 1:m
...
Resource.DisplayWindow(1).Title = 'L11-4vWideBeamDoppler';
...
for n = 1:m
    ...
    [Dummy,Ced(n)] = min(abs(scaleToWvl*Trans.ElementPos(:,1)-TX(n+k).Origin(1))); % ce is closest ele to cntr of aper.
    % Compute TX.Apod.
    lft = round(Ced(n) - P.dopNumTx/2);
    if lft < 1, lft = 1; end
    rt = round(Ced(n) + P.dopNumTx/2);
    if rt > 128, rt = 128; end
    TX(n+k).aperture = min(lft,65);
    TX(n+k).Apod((lft:rt)-TX(n+k).aperture+1) = 1;
...
maxAcqLngthDop =  sqrt(P.dopEndDepth^2 + (Trans.numelements*Trans.spacing)^2)-P.dopStartDepth;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.dopPRIs
        ind = m*(j-1) + P.numRays + k;
        for n = 1:m  % for each Doppler beam
            Receive(ind+n).aperture = TX(n+P.numRays).aperture;
            Receive(ind+n).startDepth = P.dopStartDepth;
            Receive(ind+n).endDepth = maxAcqLngthDop;
            Receive(ind+n).sampleMode = 'BS100BW';
            Receive(ind+n).demodFrequency = demodFreq;
            Receive(ind+n).InputFilter = BPF;
            Receive(ind+n).TGC = 2;
            Receive(ind+n).framenum = i;
            Receive(ind+n).acqNum = P.numRays+m*(j-1)+n;  % Doppler acqNums continue after 2D
        end
    end
end
...
Recon(2).RINums = (P.numRays+1):(P.numRays + m*P.dopPRIs); % m*P.dopPRIs ReconInfos needed for Doppler ensembles.
...
for n = 1:m
    for j = 1:P.dopPRIs
%         ReconInfo(P.numRays+n+(j-1)*m).mode = 'replaceIQ_normalize';
...
filename = 'MatFiles/L11-4vWideBeamDoppler_32LE';
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_4vWideBeamDoppler_32LE_QSApp.mat');
