...
%   for its use
%
% File name: SetUpC5_2gHWideBeamPolarSC.m - Example of curved array imaging with wide
%                beam transmits and spatial compounding using polar coordinate PData.
% Description:
%   Sequence programming file for C5-2gH curved array using wide beam transmits
...
%
% Last update:
%   10/25/2020 Adapted from SetUpC5_2vWideBeamPolarSC.m script.

%clear[ 	]+all
P.startDepthMm = 2; % startDepth in mm
P.endDepthMm = 150; % endDepth in mm
P.numTx = 40;     % no. of elements in transmit aperture - 1
P.numRays = 64;   % no. of transmit beams to program
P.txFocusMm = 600; % focus in mm - set to larger than endDepth
P.dtheta = 8*(pi/180); % steering angle for transmit beams
...
% Modify a few parameters for the actual scan
radius = Trans.radius;
scanangle = (Trans.numelements-8)*Trans.spacing/radius;    % set scan angle smaller than array scanangle
firstAngle = -(scanangle/2); % first element angle
rayDelta = (scanangle)/(P.numRays-1); % angle between rays
Angle = firstAngle:rayDelta:-firstAngle;
...
Resource.DisplayWindow(1).Title = 'C5-2gHWideBeamPolarSC';
...
for n = 1:P.numRays   % numRays transmit events
    ...
    ce = 8 + round(112*(Angle(n) - firstAngle)/scanangle);
...
TGC.CntrlPts = [200,400,540,690,800,890,940,1000];
...
k = P.numRays;
% Steered left reconstruction
ReconInfo(k+1).Pre = 'clearInterBuf';
for j = 1:P.numRays
    ReconInfo(j+k).txnum = j+k;
    ReconInfo(j+k).rcvnum = j+k;
    if j<=10
        ReconInfo(j+k).regionnum = round(j-6*(j-1)/10);
    elseif j>=(P.numRays-10)      
        ReconInfo(j+k).regionnum = round(j-6*(P.numRays-j)/10);
    else
        ReconInfo(j+k).regionnum = j-6;
    end
end
ReconInfo(k+P.numRays).Post = 'IQ2IntensityImageBuf';
k = k + P.numRays;
% Steered right reconstruction
ReconInfo(k+1).Pre = 'clearInterBuf';
for j = 1:P.numRays
    ReconInfo(j+k).txnum = j+k;
    ReconInfo(j+k).rcvnum = j+k;
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpC5_2gHWideBeamPolarSC_QSApp.mat');
...
function RangeChangeCallback(hObject,~,UIValue)
    ...
    deltaTheta = scanangle/(P.numRays-1);
