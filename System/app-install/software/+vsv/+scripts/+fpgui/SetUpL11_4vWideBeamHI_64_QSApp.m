...
%   for its use
%
% File name: SetUpL11_4vWideBeamHI_64.m - Pulse inversion imaging with widebeam transmit
%
% Description:
...
Resource.DisplayWindow(1).Title = 'L11-4vWideBeamHI_64';
...
for n = 1:P.numRays
  ...
  % Apodization:Now compute TX(n).Apod based on P.numTX
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays
        ...
        rt = Ce(j) + 32;
        if rt > Trans.numelements, rt = Trans.numelements; end
        Receive(2*P.numRays*(i-1)+j).Apod(lft:rt) = 1;
    ...
    for j = 1:P.numRays
        ...
        Receive(2*P.numRays*(i-1)+j+k).Apod = Receive(2*P.numRays*(i-1)+j).Apod; % same receive apod as in the first pulse
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_4vWideBeamHI_64_QSApp.mat');
