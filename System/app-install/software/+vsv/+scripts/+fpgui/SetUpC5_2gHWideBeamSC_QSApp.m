...
%   for its use.
%
% File name: SetUpC5_2gHWideBeamSC.m - Example of curved array imaging with
...
%
% Last update:
%   10/21/2020 - Adapted from GE4CD script
...
firstAngle = -(scanangle/2); % first element angle
rayDelta = scanangle/(P.numRays-1); % angle between rays
Angle = firstAngle:rayDelta:(-firstAngle); % angle of the P.numRays beams
...
maxBufLength = ceil(sqrt((P.maxDepth+Trans.radius)^2 + Trans.radius^2 - ...
                     2*(P.maxDepth+Trans.radius)*Trans.radius*cos(scanangle)));
maxBufSizePerAcq = 128*ceil(maxBufLength*8/128);

% Specify PData structure array.
PData.PDelta = [1.0, 0, 0.5];  % x, y and z pdeltas
sizeRows = 10 + ceil((P.endDepth + Trans.radius - (Trans.radius * cos(scanangle/2)))/PData.PDelta(3));
sizeCols = 10 + ceil(2*(P.endDepth + Trans.radius)*sin(scanangle/2)/PData.PDelta(1));
PData.Size = [sizeRows,sizeCols,1];     % size, origin and pdelta set region of interest.
PData.Origin(1,1) = (P.endDepth+Trans.radius)*sin(-scanangle/2) - 5;
PData.Origin(1,2) = 0;
PData.Origin(1,3) = ceil(Trans.radius * cos(firstAngle)) - Trans.radius - 5;
% Define the first PData region as the entire scan region to use as a mask.
PData.Region(1) = struct(...
    'Shape',struct('Name','Sector',...
                   'Position',[0,0,-Trans.radius],...
                   'r1',Trans.radius+P.startDepth,...
                   'r2',Trans.radius+P.endDepth,...
                   'angle',scanangle,...
                   'steer',0));
m = 1;
...
Resource.DisplayWindow(1).Title = 'C5-2gHWideBeamSC';
...
for n = 1:P.numRays   % P.numRays transmit events
    ...
    ce = round(1+(Trans.numelements-1)*(Angle(n) - firstAngle)/(-2*firstAngle));
...
TGC.CntrlPts = [0,250,590,710,770,830,890,900];
...
SeqControl(2).argument = 227; % 227usec
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpC5_2gHWideBeamSC_QSApp.mat');
