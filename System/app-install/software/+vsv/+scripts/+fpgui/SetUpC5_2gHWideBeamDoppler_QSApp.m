...
%   for its use.
%
% File name: SetUpC5_2gHWideBeamDoppler.m - Example of curved array imaging with
%            wide beam transmits and color Doppler imaging.
%
% Description:
%   Sequence programming file for C5-2gH curved array using P.numRays wide
%   beam transmits and receive acquisitions for grayscale imaging. For the
%   wide beam transmits, P.numTx elements are used, with floor(P.numTx/2)
%   transmitters on each side of the center element (where possible).
%   Transmit origins are spaced equally across the array, starting with the
%   element closest to P.numTx/8 and ending with the element closest to
%   128-P.numTx/8. (By not centering transmits on the end elements, the
%   image quality at the edges of the scan is improved.) The transmit focus
%   is set to below the bottom of the scan to generate a wide beam that is
%   as wide at the bottom as a single PData region.
%
...
%
% Last update:
%   11/04/2020 - Adapted from SetUpC5_2vWideBeamDoppler script
%   09/28/2021 -  modified for C5-2gH
...
P.txFocusMm = 800; % focus in mm
P.numTx = 64;    % no. of elements in TX aperture.
...
scanangle = 127*Trans.spacingMm/Trans.radiusMm;
firstAngle = -(scanangle/2);
dtheta = scanangle/P.numRays; % angle between rays
Angle = firstAngle:dtheta:(-firstAngle);
radius  = Trans.radius;
demodFreq = 2.9762;
...
P.dopNumTx = 80;  % no. of elements in TX aperture (might be adjusted later)
...
PData(1).PDelta = [1.0, 0, Trans.spacing];  % x, y and z pdeltas
...
% Define PData Regions for full scan and numRays scanlines
...
% Overlap with Region 1 to have sharp edge in Bmode
for n = 1:5
...
% RandR = P.endDepth*mm2wl*Media.MP(:,1)+radius;
...
Resource.DisplayWindow(1).Title = 'C5-2gHWideBeamDoppler';
...
for n = 1:P.numRays   % numRays transmit events
    ...
    ce = round(1+(Trans.numelements-1)*(Angle(n) - firstAngle)/scanangle);
...
[~,ind] = min(abs(radius*sin(dopBeamSteer(1)-abs(P.regionSteerAngle)) - mm2wl*Trans.ElementPos(:,1)));
...
for n = 1:P.dopNumRays
    ...
    [~,ind] = min(abs(TX(TxInd).Origin(1) - mm2wl*Trans.ElementPos(:,1)));
...
% - 2D TGC
TGC(1).CntrlPts = [0,340,570,690,835,905,990,1020];
...
% - We need P.numRays Receives for a 2D frame and m*P.dopPRIs Receives for a Doppler frame.
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = (P.numRays+1):(P.numRays+P.dopPRIs*m)
        ...
        Receive(j+k).endDepth = wl2sPer128*ceil(maxAcqLengthDop/wl2sPer128)+P.dopStartDepth;
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpC5_2gHWideBeamDoppler_QSApp.mat');
...
% filename = 'C5-2gHWideBeamDoppler';  VSX;
