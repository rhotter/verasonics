...
%    its use.
%
% File Name: SetUpL12_3vWideBeamSC_64LE.m - Wide beam, spatial compounding example
%
% Description: The L12-3v 38mm Linear array is used to acquire frames using wide beam transmits.
...
%    Wide beam transmits and receive acquisitions are used with a 64 element aperture that is
...
demodFreq = 7.8125; % demodulation frequency
...
Resource.DisplayWindow.Title = 'L12-3vWideBeamSC_64LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'peakCutOff', 0.2, ...
                   'peakBLMax', 10.0, ...
                   'Delay', zeros(1,Resource.Parameters.numTransmit)), 1, 3*P.numRays);
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays
        ...
        rt = ce(j) + 33;
        if rt > 192, rt = 192; end
        Receive(k+j).Apod((lft-(TX(j).aperture-1)):(rt-(TX(j).aperture-1))) = 1;
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL12_3vWideBeamSC_64LE_QSApp.mat');
