...
%   for its use.
%
% File name SetUpL22_14vXWideBeamSC_64LE.m: example of linear array wide beam
...
% On receive aperture, a 64 element aperture is used.
%
% Last update:
...
Resource.RcvBuffer(1).numFrames = numRcvFrames;    % 20 frames stored in RcvBuffer.
...
Resource.RcvBuffer(2).numFrames = numRcvFrames;    % 20 frames stored in RcvBuffer.
...
Resource.RcvBuffer(3).numFrames = numRcvFrames;    % 20 frames stored in RcvBuffer.
Resource.InterBuffer(1).numFrames = 1;   % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = 'L22-14vXWideBeamSC_64LE';
...
% - Set event specific TX attributes.Ce = fix(1:(127/(P.numRays-1)):128); % calculate center element numbers.
Ce = fix(1:(127/(P.numRays-1)):128); % calculate center element numbers.
for n = 1:P.numRays
    TX(n).Origin(1) = TxOrgX(n);
    % Compute TX(n).Apod based on P.numTX for straight ahead beams
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL22_14vXWideBeamSC_64LE_QSApp.mat');
