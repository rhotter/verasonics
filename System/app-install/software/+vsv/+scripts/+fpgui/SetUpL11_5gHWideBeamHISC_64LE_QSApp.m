...
%   for its use
%
% File Name: SetUpL11_5gHWideBeamHISC_64LE.m - Example of wide beam scan sequence with
%   pulse inversion harmonic imaging and spatial compounding.
%
% Description:
%   Sequence program for L11-5gH linear array using wide beam transmit scanning with
%   pulse inversion harmonic imaging and spatial compounding.  Three RcvBuffers are
%   used to store wide beam scan frames acquired with three different steering
...
demodFreq = 8.9286; % use 2X transmit frquency for demodulation frequency
...
Resource.DisplayWindow(1).Title = 'L11-5gHWideBeamHISC_64LE';
...
%   Each widebeam Ray has 4 Receives - 2 Receives for synthetic aperture, and 2 for
%   pulse inversion harmonic imaging. For 3 steering directions, this gives
%   12*P.numRays Receives per frame.
...
UI(2).Control = VsSliderControl('LocationCode','UserA1',...
    'Label',['Range (',AxesUnit,')'],'SliderMinMaxVal',[20,P.maxDepthMm,P.endDepthMm]*scaleToWvl,...
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_5gHWideBeamHISC_64LE_QSApp.mat');

save(filename)


% **** functions functions used by UI controls. ****
...
function ApertureCallback(hObject,~,UIValue)
    ...
    for n = 1:numRays  % specify P.numRays transmit events
        ...
        TX(n).Delay = computeTXDelays(TX(n));
        % Inverted transmits
        TX(n+numRays).waveform = 2;
        TX(n+numRays).Origin = TX(n).Origin;
        TX(n+numRays).Apod = TX(n).Apod;
        TX(n+numRays).Delay = TX(n).Delay;
    end
    m = 2*numRays;
    for n = 1:numRays
        TX(n+m).Origin(1) = TX(n).Origin(1);
        ...
        TX(n+m).Delay = computeTXDelays(TX(n+m));
        % Inverted transmits
        TX(n+m+numRays).waveform = 2;
        TX(n+m+numRays).Origin = TX(n+m).Origin;
        TX(n+m+numRays).Apod = TX(n+m).Apod;
        TX(n+m+numRays).Delay = TX(n+m).Delay;
    end
    m = m + 2*numRays;
    for n = 1:numRays
        TX(n+m).Origin(1) = TX(n).Origin(1);
    ...
    for i = 1:3
        for j = 1:numRays
            ind = numRays*2*(i-1)+j;
            TX(ind).TXPD = computeTXPD(TX(ind),PData);
            TX(ind+numRays).TXPD = TX(ind).TXPD;
            waitbar((numRays*(i-1)+j)/steps)
        end
    end
    close(h)
    assignin('base','TX', TX);
