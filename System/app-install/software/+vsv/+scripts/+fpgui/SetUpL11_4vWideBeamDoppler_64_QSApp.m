...
%   for its use
%
% File name: SetUpL11_4vWideBeamDoppler_64.m - Example of WideBeam 2D amd Doppler
%   imaging.
%
% Description:
%   Sequence programming file for L11-4v Linear array, using WideBeam transmits
...
%   Aperture 17 for almost middle wide beam 24 (P.numTx=32, P.numRays=48):
%   ------------------------tttttttttttttttt------------------------
%   ----------------rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr----------------
%   Aperture 65 for last wide beam (P.numTx=32, P.numRays=48):
%   ------------------------------------------------tttttttttttttttt
%   --------------------------------rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
%
%   Doppler Imaging: Rcv Aperture = Tx Aperture
%
%
% Last update:
%   08/09/19 - modified for software release 4.1.
%   08/20/18 - modified for software release 4.0.
...
Resource.DisplayWindow(1).Title = 'L11-4vWideBeamDoppler_64';
...
for n = 1:P.numRays
    ...
    % Set 64 channel transmit aperture centered around center element, Ce(n), if possible
    lft = Ce(n) - 31;
    if lft < 1, lft = 1; end
    if lft > 65, lft = 65; end
    TX(n).aperture = lft;
    % Now compute TX(n).Apod based on P.numTX
    for j = lft:(lft+Resource.Parameters.numTransmit-1)
        if (j>(Ce(n)-k))&&(j<=(Ce(n)+k))
            TX(n).Apod(j-lft+1) = ApodFunc(j-(Ce(n)-k));
        end
    end
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays  % acquisitions for 2D
        Receive(j+k).aperture = TX(j).aperture; % Receive aperture same as transmit
    ...
    for j = 1:numRaysDop
        ...
        for n = 1:P.PRIsDop  % acquire Doppler PRIs for each Doppler beam
            Receive(ind+n).aperture = TX(P.numRays+j).aperture; % Receive aperture same as transmit
...
filename = 'MatFiles/L11-4vWideBeamDoppler_64LE';
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_4vWideBeamDoppler_64_QSApp.mat');
