...
%   for its use
%
% File name: SetUpP4_2vWideBeamDoppler.m - Example of WideBeam doppler
% imaging used in Clinical Application
%
% Description:
%   Sequence programming file for P4-2v Phase array, using WideBeam
...
P.dopRadius = (P.aperture/3)/tan(-P.dopTheta); % dist. to virt. apex
P.dopRayDelta = 2*(-P.dopTheta)/(P.dopNumRays);
dopZ = zVal*scaleToWvl;
dopR = rVal*scaleToWvl;
...
Resource.DisplayWindow(1).Title = 'P4-2vWideBeamDoppler';
...
maxAcqLngth2D = ceil(sqrt(P.aperture^2 + P.endDepth^2 - 2*P.aperture*P.endDepth*cos(P.theta-pi/2)) - P.startDepth);
...
if strcmp(AxesUnit,'wls')
    ...
    zVal = zVal*scaleToWvl;
    rVal = rVal*scaleToWvl;
end
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpP4_2vWideBeamDoppler_QSApp.mat');
...
% filename = 'P4-2vWideBeamDoppler';  VSX;
...
%RangeChangeCallback - range change
    ...
    for n = 1:P.numRays
        PData(1).Region(n).Shape.r = P.radius + P.endDepth;
    end
