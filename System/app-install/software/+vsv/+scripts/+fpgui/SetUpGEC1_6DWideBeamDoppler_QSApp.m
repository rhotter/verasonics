...
%   for its use
%
% File name: SetUpGEC1_6DWideBeamDoppler.m - Example of WideBeam doppler
% imaging used in Clinical Application
%
% Description:
%   Sequence programming file for GEC1-6D curved array, using WideBeam
...
%   - The image depth, doppler region size, and acquisition PRF can be adjused
...
P.dopNumTx = 150;  % no. of elements in TX aperture (might be adjusted later)
P.dopNumRays = 3; % no. of rays in frame
P.dopStartDepth = P.startDepth;
P.dopEndDepth = P.endDepth;
P.dopPRIs = 16;
P.dopPRF = 2.0e+03; % Doppler PRF in Hz.
P.regionSteerAngle = 0;
P.pwrThres = 0.3;
P.persfreq = 60;
P.perspwr = 70;
P.cpl = 30;  % level at which 2D overwrites Doppler
...
PData(1).PDelta = [1, 0, Trans.spacing];  % x, y and z pdeltas
...
PData(1).Region(1).Shape.angle = (pi/180)*65;
...
Resource.DisplayWindow(1).Title = 'GEC1-6DWideBeamDoppler';
...
% - Doppler TGC
TGC(2).CntrlPts = [0,494,769,829,829,829,769,769];
...
% - time between frames (2000 usec = 2msec)
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpGEC1_6DWideBeamDoppler_QSApp.mat');
...
% filename = 'GEC1-6DWideBeamDoppler';  VSX;
