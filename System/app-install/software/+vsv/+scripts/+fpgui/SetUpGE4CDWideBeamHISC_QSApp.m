...
%   for its use.
%
% File name: SetUpGE4CDWideBeamHISC.m - Example of curved array imaging with
%   wide beam transmits, pulse inversion harmonic imaging and spatial compounding.
%
% Description:
%   Sequence programming file for GE4CD curved array using P.numRays wide beam pulse
%   inversion transmits and receive acquisitions. For the wide beam transmits,
%   P.numTx elements are used, with floor(P.numTx/2) transmitters on each side of the
%   center element (where possible). Transmit origins are spaced equally across the
%   array, starting with the element closest to P.numTx/8 and ending with the element
%   closest to 128-P.numTx/8. (By not centering transmits on the end elements, the
%   image quality at the edges of the scan is improved.) The transmit focus is set to
%   below the bottom of the scan to generate a wide beam that is as wide at the
%   bottom as a single PData region.
%
...
P.numTx = 85;   % no. of elements in TX aperture (preferably odd).
P.numRays = 80; % no. of rays in frame
P.txFocusMm = 600; % focus in mm
P.startDepthMm = 2;  % startDepth in mm
P.endDepthMm = 120;
P.maxDepthMm = 180;  % maxDepth for RangeChange and RcvBuffer
...
Resource.DisplayWindow(1).Title = 'GE4CDWideBeamHISC';
Resource.DisplayWindow(1).pdelta = 0.50;
...
for n = 1:P.numRays   % P.numRays transmit events
    ...
    w1 = 1;
    w2 = L;
    lft = round(ce - floor(P.numTx/2));
    if lft < 1, w1 = -lft+2; lft = 1; end
    rt = round(ce + floor(P.numTx/2));
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpGE4CDWideBeamHISC_QSApp.mat');
...
% filename = 'GE4CDWideBeamHISC';  VSX;
