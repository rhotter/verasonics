...
%   for its use
%
% File name SetUpL11_5gHWideBeamHISC.m - Example of wide beam scan sequence with harmonic imaging
%   and spatial compounding.
%
% Description:`
%   Generate sequence program for L11-5gH linear array using wide beam transmit scanning with
%   harmonic imaging and spatial compounding.  Three wide beam scans are performed with P.numRays
...
Resource.DisplayWindow(1).Title = 'L11-5gHWideBeamHISC';
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_5gHWideBeamHISC_QSApp.mat');
...
% filename = 'L11-5gHWideBeamHISC';  VSX;
...
function ApertureCallback(hObject,~,UIValue)
    ...
    TxOrgX = (-63.5*Trans.spacing):(127*Trans.spacing/(P.numRays-1)):(63.5*Trans.spacing);
    % - Redefine event specific TX attributes for the new aperture.for n = 1:P.numRays
    h = waitbar(0,'Program TX parameters, please wait!');
    steps = 3*P.numRays;
    for n = 1:P.numRays  % specify P.numRays transmit events
        TX(n).Origin(1) = TxOrgX(n);
