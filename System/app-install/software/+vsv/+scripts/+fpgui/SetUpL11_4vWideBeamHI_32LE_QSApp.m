...
%   for its use
%
% File name: SetUpL11_4vWideBeamHI_32LE.m - Pulse inversion imaging with widebeam transmit
%
% Description:
...
%   The receive aperture covers the 32 element aperture.
%   Processing is asynchronous with respect to acquisition.
%
% Last update:
% 08/31/2018 - modified for 32LE vantage
...
for n = 1:P.numRays
  ...
  % aperture
  lft = Ce(n) - round(P.numTx/2);
  if lft < 1, lft = 1; end
  rt = Ce(n) + round(P.numTx/2);
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_4vWideBeamHI_32LE_QSApp.mat');
