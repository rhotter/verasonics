...
%   for its use
%
% File name SetUpL11_4vWideBeamSC_64.m - Example of wide beam scan sequence with spatial compounding.
%
% Description:
...
Resource.DisplayWindow(1).Title = 'L11-4vWideBeamSC_64';
...
for n = 1:P.numRays  % specify P.numRays transmit events
    ...
    % Compute 64 channel transmit aperture around center element, Ce(n)
    lft = Ce(n) - 31;
    if lft < 1
        lft = 1;
        rt = 64;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays
        % Compute 64 chnl Receive.Apod around center element, Ce(j), if possible
        lft = Ce(j) - 31;
        if lft < 1 % if at lft edge of transducer, use 64 element aperture from element 1
            lft = 1;
            rt = 64;
        else
            rt = Ce(j) + 32;
        end
        if rt > Trans.numelements % if at rt edge of xducer, use 64 ele aper ending in ele 128
            rt = Trans.numelements;
            lft = Trans.numelements - 63;
        end
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_4vWideBeamSC_64_QSApp.mat');
