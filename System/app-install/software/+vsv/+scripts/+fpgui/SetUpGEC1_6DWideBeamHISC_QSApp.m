...
%   for its use.
%
% File name: SetUpGEC1_6DWideBeamHISC.m - Example of curved array imaging with
%            wide beam transmits, harmonic imaging and spatial compounding
%
% Description:
%   Sequence programming file for GEC1-6D curved array using m wide beam
%   pulse inversion transmits and receive acquisitions. Of the 192 transmit channels, only
%   P.numTx are used, with P.numTx transmitters on each side of the center element
%   (where possible). All 192 receive channels are used, although the
...
%   6 regions at every pixel.  The transmit beams are steered in 3 directions within each
%   wide beam region. The transmit focus is set to below the bottom of the PData region.
...
P.numTx = 140;   % no. of elements in TX aperture.
P.numRays = 120; % no. of rays in frame
...
% Specify Resources. Three RcvBuffers are defined, one for each TX steering angle.
...
Resource.DisplayWindow(1).Title = 'GEC1-6DWideBeamHISC';
...
UI(2).Control = {'UserA1','Style','VsSlider','Label',['Range (',AxesUnit,')'],'SliderMinMaxVal',MinMaxVal,...
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpGEC1_6DWideBeamHISC_QSApp.mat');
...
% filename = 'GEC1-6DWideBeamHISC';  VSX;
