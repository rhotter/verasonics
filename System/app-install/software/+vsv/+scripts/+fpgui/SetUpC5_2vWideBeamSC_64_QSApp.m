...
%   for its use.
%
% File name: SetUpC5_2vWideBeamSC_64.m - Example of curved array imaging with
%            wide beam transmits and 3 angle spatial compounding
%
% Description:
%   Sequence programming file for C5-2v curved array using wide beam transmit
%   scanning with spatial compounding. Of the transmit channels, only P.numTx are
%   used, with P.numTx transmitters on each side of the center element (where
%   possible). The P.numRays transmit beams are steered over one of three angles on
%   each successive frame.
%   The receive aperture always covers a full 64 element aperture centered
%   about the transmit beam origin (where possible).
...
% Aperture for almost middle wide beam 48 (P.numTx=32, P.numRays=96):
%   ------------------------tttttttttttttttt------------------------
%   ----------------rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr----------------
% Aperture for last wide beam (P.numTx=32, P.numRays=96):
%   ------------------------------------------------tttttttttttttttt
%   --------------------------------rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
%
...
%
%   Three wide beam scans are performed using P.numRays scan lines in each
...
P.numTx = 62;   % no. of elements in TX aperture. No larger than 63
...
PData.Region(1) = struct(...
    'Shape',struct('Name','Sector',...
                   ...
                   'steer',0));
m = 1;
% Define PData Regions for P.numRays scanlines (no steering)
for n = 1:P.numRays
...
Resource.DisplayWindow(1).Title = 'C5-2vWideBeamSC_64';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays
        ...
        % Compute 64 channel Receive.Apod centered around center element
        lft = ce - 31;
        if lft < 1, lft = 1; end
        rt = ce + 32;
        if rt > Trans.numelements, rt = Trans.numelements; end
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpC5_2vWideBeamSC_64_QSApp.mat');
...
function RangeChangeCallback(hObject, ~, UIValue)
    ...
    for n = 1:P.numRays + 1 % include first region
