...
%   for its use.
%
% File name: SetUpP4_2vWideBeamHIPolar.m - Example of phased array pulse inversion
...
Resource.RcvBuffer(1).rowsPerFrame = P.numRays*36*128; % for maximum depth defined in the GUI (360 wavelength)
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = numRcvFrames;     % 24 frames used for RF cineloop.
Resource.InterBuffer(1).numFrames = 1;
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = 'P4-2vWideBeamHIPolar';
Resource.DisplayWindow(1).pdelta = 0.38;
...
maxAcqLength = 10+ceil(sqrt((aperture/4)^2 + P.endDepth^2 - 2*(aperture/4)*P.endDepth*cos(P.theta/2+pi/2)));
...
UI(3).Control = VsSliderControl('LocationCode','UserC2',...
    'Label','Reg. Width',...
    'SliderMinMaxVal',[1,9,PData.Region(2).Shape.angle/P.rayDelta],...
    'SliderStep',[0.0625,0.125],'ValueFormat','%1.3f',...
    'Callback',@RegionWidthCallback);

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 3;

% Save all the structures to a .mat file.
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpP4_2vWideBeamHIPolar_QSApp.mat');
...
function RegionWidthCallback(~, ~, UIValue)
    P = evalin('base','P');
