...
%   for its use
%
% File name: SetUpP4_2gH_128RyLnsPolar.m - Example of phased array imaging with
%                                    focus transmits using polar coord. PData
% Description:
%   Sequence programming file for P4-2gH phased array in virtual apex format,
%   using 128 ray lines(focus transmits) and receive acquisitions. All 96
...
%   is a 96 element probe that is wired to the scanhead connector with
%   element 1-96 connected to inputs 1-96. We therefore need a
%   Trans.Connector array to specify the connector channels used, which
%   will be defined by the Trans function.
%
% Last update:
%   10/02/2016 - modified to use polar coordinates.
%   09/24/2021 - modifed for P4-2gH transducer

%clear[ 	]+all

P.numRays = 128;      % no. of Rays
P.startDepth = 0;
P.endDepth = 300;   % Acquisition depth in wavelengths
...
% Specify Resources.
maxBufLength = sqrt(P.aperture^2 + P.maxDepth^2 - 2*P.aperture*P.maxDepth*cos(P.theta/2+pi/2));
maxBufSizePerAcq = 128*ceil(maxBufLength*4*(Trans.frequency/Trans.frequency)/128); % 4 to match the receive samplemode 
Resource.RcvBuffer(1).datatype = 'int16';
Resource.RcvBuffer(1).rowsPerFrame = maxBufSizePerAcq*P.numRays; % This is for the max range on range slider.
...
Resource.DisplayWindow(1).Title = 'P4_2gH128RyLnsPolar';
...
% - Range Change
wls2mm = 1;
MinMaxVal = [100,P.maxDepth,P.endDepth]; % default unit is wavelength
AxesUnit = 'wls';
...
UI(3).Control = VsSliderControl('LocationCode','UserB4',...
    ...
    'SliderMinMaxVal',[20,320,P.txFocus]*wls2mm,'SliderStep',[0.1,0.2],...
    'ValueFormat','%3.0f',...
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpP4_2gH_128RyLnsPolar_QSApp.mat');
