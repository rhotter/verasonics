...
%   for its use
%
% File name SetUpL11_5gHWideBeamSC_64LE.m - Example of wide beam scan sequence with
%                                          spatial compounding.
%
% Description:
%   Generate sequence program for L11-5gH linear array using wide beam transmit
%   scanning with spatial compounding.  Three RcvBuffers are used to store wide beam
%   scan frames acquired with three different steering angles.  The different scan
%   angles (steered left, no steering, steered right) are acquired in series using
%   P.numRays scan lines in each frame.  The most recent frame in each buffer is
%   processed and displayed sequentially. Frames are averaged after image processing
%   using a running average that updates every frame. The transmit aperture size can
%   be set by the variable - numTx. The receive aperture always covers the full 128
%   element aperture.
%
% Last update:
% 01/30/2019 - working with software release 4.0.0

%clear[ 	]+all

P.numTx = 32;   % no. of elements in TX aperture (set odd for .
...
% Convert mm parameters to wavelengths
demodFreq = Trans.frequency; % use nearest available demodulation frequency
scaleToWvl = Trans.frequency/(Resource.Parameters.speedOfSound/1000);
txFocus = P.txFocusMm*scaleToWvl;
startDepth = P.startDepthMm*scaleToWvl;  % startDepth in wavelength
endDepth = P.endDepthMm*scaleToWvl;
maxDepth = P.maxDepthMm*scaleToWvl;
maxBufWvls = ceil(sqrt(maxDepth^2 + ((Trans.numelements-1)*Trans.spacing)^2));
maxBufSmplsPerAcq = 128*ceil(maxBufWvls*8*(demodFreq/Trans.frequency)/128); % multiple of 128

% Specify PData structure arrays.
PData.PDelta = [Trans.spacing/2, 0, 0.5];
PData.Size(1,1) = ceil((endDepth-startDepth)/PData.PDelta(3)); % range, origin and pdelta set PData(1).Size.
PData.Size(1,2) = ceil((Trans.numelements*Trans.spacing)/PData.PDelta(1));
PData.Size(1,3) = 1;      % single image page
PData.Origin = [-Trans.spacing*(Trans.numelements-1)/2,0,startDepth]; % x,y,z of uppr lft crnr.
% Predefine all the Region structures, then modify them below.
PData.Region = repmat(struct('Shape',struct('Name','Parallelogram',...
                                               'Position',[0,0,startDepth],...
                                               'width',5*Trans.spacing*Trans.numelements/(P.numRays),...
                                               'height',endDepth-startDepth,...
...
for n = 1:P.numRays
    ...
    PData.Region(n+m).Shape.height = (endDepth-startDepth)/cos(angle);
...
Resource.RcvBuffer(1).rowsPerFrame = 2*P.numRays*maxBufSmplsPerAcq; % this size allows for all rays, with range of up to 400 wvlngths
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = numRcvFrames;
Resource.RcvBuffer(2).datatype = 'int16';
Resource.RcvBuffer(2).rowsPerFrame = 2*P.numRays*maxBufSmplsPerAcq; % this size allows for all rays, with range of up to 400 wvlngths
...
Resource.RcvBuffer(3).rowsPerFrame = 2*P.numRays*maxBufSmplsPerAcq; % this size allows for all rays, with range of up to 400 wvlngths
...
Resource.DisplayWindow(1).Title = 'L11-5gHWideBeamSC_64LE';
...
for n = 1:P.numRays  % specify P.numRays transmit events
    ...
    % Find number of closest element to TxOrgX(n).
...
maxAcqLength = ceil(sqrt(endDepth^2 + ((Trans.numelements-1)*Trans.spacing)^2));
...
for j = 1:2:2*P.numRays
    ReconInfo(j+k).txnum = (j+1)/2 + P.numRays;
    ReconInfo(j+k).rcvnum = j+k;
    ReconInfo(j+k).regionnum = (j+1)/2 + P.numRays;
    ReconInfo(j+k+1).txnum = (j+1)/2 + P.numRays;
    ReconInfo(j+k+1).rcvnum = j+k+1;
    ReconInfo(j+k+1).regionnum = (j+1)/2 + P.numRays;
end
...
for j = 1:2:2*P.numRays
    ...
    ReconInfo(j+k).rcvnum = j+k;
    ReconInfo(j+k).regionnum = (j+1)/2 + 2*P.numRays;
    ReconInfo(j+k+1).txnum = (j+1)/2 + 2*P.numRays;
    ReconInfo(j+k+1).rcvnum = j+k+1;
    ReconInfo(j+k+1).regionnum = (j+1)/2 + 2*P.numRays;
end
ReconInfo(k+2*P.numRays).Post = 'IQ2IntensityImageBuf';
...
filename = fullfile( vsv.file.getVSXDir, 'MatFiles/SetUpL11_5gHWideBeamSC_64LE_QSApp.mat');

save(filename)


% **** Funcitons functions used by UI Controls. ****
...
function RangeChangeCallback (~,~,UIValue)
    ...
    if isfield(Resource.DisplayWindow(1),'AxesUnits')&&~isempty(Resource.DisplayWindow(1).AxesUnits)
        if strcmp(Resource.DisplayWindow(1).AxesUnits,'mm')
            ...
            startDepth = P.startDepthMm*scaleToWvl;
            endDepth = UIValue*scaleToWvl;
        else
            startDepth = P.startDepthMm*scaleToWvl;
            endDepth = UIValue;
            P.endDepthMm = endDepth/scaleToWvl;
        end
    end
    assignin('base','P',P);
    ...
    PData.Size(1) = ceil((endDepth-startDepth)/PData.PDelta(3)); % startDepth, endDepth and pdelta set PData.Size(1).
