...
%   for its use.
%
% SetUpL11_5vCDIExternalProc.m
% Color Doppler Imaging by external MatLab function processing.
% performs:  WF, correlation/est, detection, and AGC.
%
...
%     steering angle of dopAngle radians, receiving on the central 64 elements.
% Modified from trunk SetUpL7_4FlashDoppler.m
% John Flynn April 2012 / Yao July 2014 / Michaela July 2017

clear all

DopplerReconRows = 128;
DopplerReconCols = 64; % even number
...
P(1).rayDelta = 128*Trans.spacing;  % spacing in radians(sector) or dist. between rays (wvlnghts)
P(2).rayDelta = 64*Trans.spacing;  % spacing in radians(sector) or dist. between rays (wvlnghts)
...
PData(2).PDelta = [1, 0, 1];
PData(2).Size(1,1) = ceil(DopplerReconRows/PData(2).PDelta(3)); % flow window rows
PData(2).Size(1,2) = ceil((DopplerReconCols*Trans.spacing)/PData(2).PDelta(1));  % flow window columns
PData(2).Size(1,3) = 1;             % single image page
PData(2).Origin = [-Trans.spacing*(DopplerReconCols/2-0.5),0,P(2).startDepth]; % x,y,z of upper lft crnr.
...
Resource.DisplayWindow(1).Title = 'L11-5vCDIExternalProc';
...
TW(2).Parameters = [5.208,0.67,6,1];
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = (na+1):(na+ne)
        % Doppler acquisition
        Receive(j+k).Apod(64-DopplerReconCols/2+1:64+DopplerReconCols/2) = 1.0;
...
Recon(1).RINums = (1:na);  % na ReconInfos needed for na angles
k = na + 1;
...
cpt = 28;  %color priority.  define here so we can use in UIControl below
persf = 80; %persistence for Velocity mode
persp = 90; %persistence for Power mode
...
Process(2).method = 'cdiprocessing';
...
UI(3).Control = VsSliderControl('LocationCode','UserB4',...
    ...
    'SliderStep',[0.025 0.1],'ValueFormat','%1.3f',...
    'Callback',@(~,~,UIValue) assignin('base','pwrThres', UIValue));
...
scriptInfo.name = mfilename;
scriptInfo.compileDate = datestr(now);
scriptInfo.PRF = dopPRF;
scriptInfo.TW = TW;
scriptInfo.Trans = Trans;

% Save all the structures to a .mat file.
save('MatFiles/L11-5vCDIExternalProc');
...
% filename = 'L11-5vCDIExternalProc';  VSX;
...
function velocityPowerCallback(~, ~, UIValue)
    ...
    switch UIValue
        case 1 % Velocity mode
            ...
            Control(1).Parameters = {'Process',3,'srcData',...
            ...
            for k = 1:2:length(Process(3).Parameters)
                if strcmp(Process(3).Parameters{k},'srcData')
                    Process(3).Parameters{k+1} = 'signedColor';
                elseif strcmp(Process(3).Parameters{k},'persistMethod')
                    Process(3).Parameters{k+1} = 'dynamic';
                elseif strcmp(Process(3).Parameters{k},'persistLevel')
                    Process(3).Parameters{k+1} = persf;
                end
            end
        case 2 % Power mode
            ...
            Resource.DisplayWindow(1).Colormap = newMap;
            for k = 1:2:length(Process(3).Parameters)
                if strcmp(Process(3).Parameters{k},'persistLevel')
                    persf = Process(3).Parameters{k+1};
                end
            end
            ...
            for k = 1:2:length(Process(3).Parameters)
                if strcmp(Process(3).Parameters{k},'srcData')
                    Process(3).Parameters{k+1} = 'unsignedColor';
                elseif strcmp(Process(3).Parameters{k},'persistMethod')
                    Process(3).Parameters{k+1} = 'simple';
                elseif strcmp(Process(3).Parameters{k},'persistLevel')
                    Process(3).Parameters{k+1} = persValue;
                end
            end
    end
