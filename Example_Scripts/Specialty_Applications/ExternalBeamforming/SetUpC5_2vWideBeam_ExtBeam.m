...
%   for its use.
%
% File name: SetUpC5_2vWideBeam_ExtBeam.m - Example of using an external
%                                           beamforming process with a  
%                                           C5-2v transducer
% Description:
%   Two new reconstruction modes were introduced to the software, 
%   ‘replaceIQ_pages’ and ‘accumIQ_pages’. These new methods allow the user 
%   to have access to the delayed but unsummed IQ data, at each pixel 
%   location for all of the individual elements of the transducer, which 
%   are stored in separate pages of a multi-page InterBuffer. The pages of 
%   the InterBuffer correspond to the channel numbers of the system, which 
%   in turn correspond to the individual elements of the transducer. 
%   Delayed channel data can be accessed in two ways, after quitting VSX or
%   through the use of an external function. After closing the VSX Control 
%   GUI window (quitting VSX), the workspace will contain the individual 
%   pixel delayed IQ data from each of the transducer elements for the 
%   image frames acquired and processed. The second way is through the use 
%   of an external function that is evaluated while VSX is running. 
%   External functions can use the individual IQ channel data as inputs and 
%   then output the result of processing to either an InterBuffer or 
%   ImageBuffer for further processing or display. 
%
%   This example script is a C5-2v Widebeam script modified such that an
%   external beamformer can be used with it. The external beamforming 
%   function is defined as an external function at the end of the script. 
%   The external function uses an external beamforming class that can be 
%   specified and initialized in the SetUp script allowing for the external
%   beamformer to be easily customized in the SetUp script. One additional 
%   requirement is that the external beamformer class must have a function 
%   named compute image, that computes the beamformed image. If this is the 
%   case, then only the code pertaining to the external beamformer definition 
%   and initialization in the SetUp script needs to be changed for a new 
%   beamforming method to be used. A different external beamforming method 
%   can also be hard coded into the external function, but this is not 
%   recommended.
%   
%   Short-Lag Spatial Coherence (SLSC) beamforming was used in this example
%   as the external beamformer. SLSC beamforming is a beamforming method
%   that relies solely on the spatial coherence between 
%   signals from two elements a set distance apart to recontruct an image. 
%   More information about SLSC beamforming can be found at the link below.  
%   * https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3172134/
%   
%   While the script does run with live acquistions, it isn't real-time as 
%   SLSC beamforming can be computationally expensive. This script is set up 
%   to run in simulation mode 2, reading in previously saved C5-2v Widebeam 
%   RcvData back into the system for processing and display. An in-vivo 
%   liver data set taken with a C5-2v transducer has been uploaded to 
%   to the Verasonics Community Portal for ease in trying out the example.
%
% How to run example:
%   1. Download the example data set from the Verasonics Community Portal.
%      It can be found in the 'Vantage Resource Documents' under 'SLSC
%      Beamforming Example Data'.
%   2. Run SetUpC5_2vWideBeam_ExtBeam script
%   3. Clear the workspace (clear all)
%   4. Load in example data downloaded from the Verasonics Community
%      Portal.
%   5. Run VSX specifying SetUpC5_2vWideBeam_ExtBeam as the file to process
%
% Last update:
%   SW 4.5.0 
%   04/12/2021 
...
%  Resource.Parameters.simulateMode = 0 forces the system to run hardware
...
% Define External Beamformer
MaxShortLag = 13; % Maximum short-lag value used in SLSC beamforming
beamformer = vsv.recon.SLSCBeamFormer('NumElementsRcv',Trans.numelements,...
                                      'MaxShortLag',MaxShortLag);
...
Resource.RcvBuffer.rowsPerFrame = P.numRays*4096*2;    % numrays segments of 4096 samples
Resource.RcvBuffer.colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer.numFrames = 20;
Resource.InterBuffer.numFrames = 1;
Resource.InterBuffer.pagesPerFrame = Trans.numelements;
Resource.ImageBuffer.numFrames = 10;
Resource.DisplayWindow(1).Title = 'C5-2vWideBeam with SLSC Beamforming';
...
ReconInfo = repmat(struct('mode', 'accumIQ_pages', ...  % accumulate IQ
                   'Pre',[],...
                   'Post',[],...
                   'txnum', 1, ...
                   'rcvnum', 1, ...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'External Reconstruction of Image';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 1;
    Event(n).process = 1;
...
% - SLSC Control: Controls number of short-lags (MaxShortLag) taken into
% consideration during reconstruction
UI(7).Control = VsSliderControl('LocationCode','UserC3',...
    'Label','Max Short Lag',...
    'SliderMinMaxVal',[1,beamformer.NumElementsRcv-1,MaxShortLag],...
    'SliderStep',[0.005,0.020],'ValueFormat','%3.0f',...
    'Callback',@MaxShortLagCallback);
...
save('MatFiles/C5-2vWideBeam_ExtBeam');
...
function MaxShortLagCallback(~, ~, UIValue)
% Controls the short-lag value in the spatial coherence calculations in the
% external beamformer
%
    beamformer = vsv.seq.getBaseComp('beamformer');
    beamformer.MaxShortLag = UIValue;
    assignin('base','beamformer',beamformer);
end

function ImageData = externalBeamforming(IData,QData)
% externalBeamforming This function is an example of how to use a different  
% beamforming method
%   @param IData, QData: @type numeric The channel IData and QData 
%                        reconstructed by the system
%   @result ImageData: @type numeric Image reconstructed using the
%                      external beamformer specified in the setup script.
%
    % Get external beamformer from the base workspace
    beamformer = vsv.seq.getBaseComp('beamformer');
    
    % Calculate the beamformed image
    ImageData = beamformer.computeImage(IData,QData);
    % NOTE: QData is input into the function computeImage to make
    % modifying the beamforming method the script implements easier for
    % the user, however the SLSC beamforming example's processing does not 
    % use the QData in reconstruction, only the IData.
end