...
%   for its use
%
% File name: SetUpL11_4vFlashExtMex.m - Example of imaging with single plane wave
%                                 transmit and external processing function.
% Description: 
...
ReconInfo = struct('mode', 'replaceIQ_normalize', ...  
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 1);
...
Process(2).method = 'extmexMultiThreads';
Process(2).Parameters = {'srcbuffer','inter',...  % buffer to process, in this case InterBuffer
                         'srcbufnum',1,...
                         'srcframenum',-1,...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'Process extmex ImageBuffer';
    Event(n).tx = 0;        % no TX
    Event(n).rcv = 0;       % no Rcv
    Event(n).recon = 0;     % no Recon
    Event(n).process = 1; 
...
save('MatFiles/L11-4vFlashExtMex');
return
