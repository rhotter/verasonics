...
%   for its use
%
% File name: SetUpRC15gV_Flash.m - Example of plane wave imaging.
% Description:
%   Sequence programming file for 80x80 element Row/Column array, using
...
Resource.DisplayWindow(1).Title = '3D RC15gV Flash - XZ plane';
...
Resource.DisplayWindow(2).Title = '3D RC15gV Flash - YZ plane';
...
Resource.DisplayWindow(3).Title = '3D RC15gV Flash - XY plane';
...
%     Receive((i-1)+1).callMediaFunc = 1;
    m = 2*(i-1); % m counts frames
        ...
        Receive(m+1).Apod(81:160) = 1; 
        Receive(m+1).framenum = i;
        Receive(m+1).acqNum = 1;
        Receive(m+2).Apod(1:80) = 1;
