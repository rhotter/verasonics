...
%   for its use
%
% File name: SetUpRC6gV_FMC.m - Example of full matrix capture imaging with row/col
%                                   array.
% Description:
%   Sequence programming file for 128x128 element Row/Column array, 128 transmits
%   on single elements in both the x and y directions.  256 elements are used on
...
Resource.DisplayWindow(1).Title = '3D RC6gV FMC - XZ plane';
...
Resource.DisplayWindow(2).Title = '3D RC6gV FMC - YZ plane';
...
Resource.DisplayWindow(3).Title = '3D RC6gV FMC - XY plane';
...
for n = 1:256   % for 256 transmit events
    TX(n).Apod(n) = 1;
...
Receive = repmat(struct('Apod', zeros(1,256), ...
                        ...
                        'sampleMode', 'NS200BW', ...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, 256*Resource.RcvBuffer(1).numFrames);
...
%     Receive(na*(i-1)+1).callMediaFunc = 1;
    ...
    for j = 1:256
        if j<129
            Receive(k+j).Apod(129:256) = 1;
        else
            Receive(k+j).Apod(1:128) = 1;
        end 
