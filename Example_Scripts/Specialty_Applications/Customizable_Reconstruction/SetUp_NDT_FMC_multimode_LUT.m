...
%   for its use.
%
% File name: SetUp_NDT_FMC_multimode_LUT.m - Example of full matrix capture
% multimode imaging.  Direct + half-skip TFM implemented through look up
% tables.
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el Phase array, using total
%   focusing reconstruction implemnted through user definable look up tables.
%   For each acquisition, one channel is active for transmit and 64 channels are active for receive.
%   Reconstruction and processing are synchronoused with respect to acquisition.  The same
%   FMC data set is used to generate direct and half-skip TFM image
%   components.
%
% Last update:
% 01/20/2020
...
Trans.elementWidth = 0.3;                          % element width in mm; assumes 50 micron kerf
...
Resource.DisplayWindow(1).Title = 'Direct + half-skip TFM';
Resource.DisplayWindow(1).pdelta = PData(1).PDelta(1)/3;
...
Resource.DisplayWindow.Colormap = jet(256);
...
Receive = repmat(struct('Apod', [zeros(1,ElOffset), ones(1,P.Elnum), zeros(1,Trans.numelements-P.Elnum-ElOffset)], ...
    'startDepth', 0, ... % Acquisation start depth needs to 0 at all time
...
% - Direct and half-skip tfm have the same receive paths so we only need one Recon structure.  Each frame will use
%   2*P.Elnum ReconInfo structures, first half for direct path, second for
%   half-skip.
Recon = struct('senscutoff', 0, ...
    ...
    'rcvLUT', zeros(P.Elnum*PData.Size(1)*PData.Size(2),1,'uint16'),...  %rcvLUT defined for every pixel address in PData
    'RINums', 1:2*P.Elnum);
...
for j = 1:PData.Region.numPixels % Calculating path lengths only for pixels to be subsequently reconstructed

    PLA=double(PData.Region.PixelsLA(j));%pixel linear address

    x = PData.Origin(1) + floor(PLA/PData.Size(1))*PData.PDelta(1);
    z = PData.Origin(3) + rem(PLA,PData.Size(1))*PData.PDelta(3);
    zsq = z*z;
    k = Trans.numelements*(PLA);

    for m = 1:Trans.numelements

        pl = sqrt((x - mm2wls*Trans.ElementPos(m,1))*(x - mm2wls*Trans.ElementPos(m,1)) + zsq); %path length from pixel to receiving element
...
ReconInfo = repmat(struct('mode', 'accumIQ', ...
    ...
    'rcvnum', 1, ...
    'regionnum', 1, ...
    'LUT', zeros(3*PData.Region.numPixels,1,'int32')),1,2*P.Elnum);
...
if P.Elnum>1
    ...
    for j = 1:P.Elnum  % For each row in the column
        ReconInfo(j).txnum = j;
        ReconInfo(j).rcvnum = 1;

        ReconInfo(j+P.Elnum).txnum = j;
        ReconInfo(j+P.Elnum).rcvnum = 1;
    end

    ReconInfo(2*P.Elnum).mode = 'accumIQ_replaceIntensity'; % accum & detect
else
...
for kk=1:2*P.Elnum
    ...
    n = 1;
    for j = 1:PData.Region.numPixels% reconstruct all PData pixels in region 1

        ReconInfo(kk).LUT(n) = PData.Region.PixelsLA(j); % linear adr.
        PLA=double(PData.Region.PixelsLA(j));%pixel linear address

        n = n+1;
        ReconInfo(kk).LUT(n) = int32(256); % amplitude weight of 1.0 in 24.8 format.
        n = n+1;

        x = PData.Origin(1) + floor(PLA/PData.Size(1))*PData.PDelta(1); %pixel coordinates
        z = PData.Origin(3) + rem(PLA,PData.Size(1))*PData.PDelta(3);
        zsq = z*z;

        if kk<=P.Elnum

            pl = spcf*sqrt((x-mm2wls*Trans.ElementPos(kk,1)).^2 + zsq) + TX(kk).Delay(kk); %Direct path length (wavelengths)

        else

            pl = spcf*sqrt((x-mm2wls*Trans.ElementPos(kk-P.Elnum,1)).^2 + (2*D*mm2wls-z).^2) + TX(kk-P.Elnum).Delay(kk-P.Elnum); %Half-skip path length (wavelengths)

        end

        ReconInfo(kk).LUT(n) = int32(round((pl + 1.0 + Trans.lensCorrection)*16));
        n = n+1;
...
save('MatFiles/NDT_FMC_multimode_LUT');
...
filename = 'NDT_FMC_multimode_LUT';  VSX;
