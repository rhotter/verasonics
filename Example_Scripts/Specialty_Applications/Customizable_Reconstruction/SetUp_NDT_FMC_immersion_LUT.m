...
%   for its use.
%
% File name: SetUp_NDT_FMC_immersion_LUT.m - Example of imaging using full matrix capature
%                         and total focusing reconstruction for an array wedge or planar surface immersion
%                         implemented through look up tables.
%
% Description:
...
%   for transmit and 64 channels are active for receive. Reconstruction
%   and processing are synchronoused with respect to acquisition.  Focal
%   laws calculated for a planar interface between two media and implemented
%   as user defined look up tables,
%
% Last update:
% 01/20/2020
...
c1=1500; %velocity in first medium
c2=6300; %velocity in second medium

Beta=0; %array angle (degrees)
D=5.0; %stand-off distance (mm; Could SetUpFMCDataViewer.m to measure stand-off 
...
Resource.Parameters.speedOfSound = c2;
...
%transformed array coordinates
x_array=((Trans.ElementPos(:,1)-min(Trans.ElementPos(:,1)))*cos(Beta*pi/180))*scaleToWvl;
x_array=x_array-(max(x_array)-min(x_array))/2;
z_array=(D+(Trans.ElementPos(:,1)-min(Trans.ElementPos(:,1)))*sin(Beta*pi/180))*scaleToWvl;
...
Resource.DisplayWindow(1).Title = 'Immersion/wedge imaging TFM';
...
Recon = struct('senscutoff', 0, ...
    ...
    'rcvLUT', zeros(P.Elnum*PData.Size(1)*PData.Size(2),1,'uint16'),...
    'RINums', 1:P.Elnum);
...
ReconInfo = repmat(struct('mode', 'accumIQ', ...
    ...
    'rcvnum', 1, ...
    'regionnum', 1, ...
    'LUT', zeros(3*PData.Region.numPixels,1,'int32')),1,P.Elnum);
...
for j = 1:PData.Region.numPixels
    ...
    for m = 1:Trans.numelements
        ...
        % Crude Fermat's principle numerical solution
        ReconInfo(m).LUT(3*(j-1)+2) = int32(256); % weight of 1.0 in 24.8 format.
        xi_arr=-PData.Size/2:PData.Size/100:PData.Size/2;
        pl_arr=(sqrt((x_array(m) - xi_arr).^2 +z_array(m).^2)/c1 + sqrt((x - xi_arr).^2 +z.^2)/c2)*c2;
        pl=min(pl_arr);

        Recon.rcvLUT(k+m) = uint16(16*pl);
        ReconInfo(m).LUT(3*(j-1)+1) = PData.Region.PixelsLA(j); % linear adr. from 0
        ReconInfo(m).LUT(3*(j-1)+3) = int32(round((pl + 1.0 + Trans.lensCorrection)*16));
...
save('MatFiles/NDT_FMC_immersion_LUT');
...
filename = 'NDT_FMC_immersion_LUT';  VSX;
