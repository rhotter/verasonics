...
%   for its use.
%
% File name: SetUp_NDT_FMC_LUT.m - Example of imaging using full matrix capature
%                         and total focusing reconstruction implemented through user defined
%                         look up tables.
%
% Description:
...
for kk=1:P.Elnum
    ...
    for j = 1:PData.Region.numPixels% reconstruct all PData pixels in region 1
        ...
        ReconInfo(kk).LUT(n) = PData.Region.PixelsLA(j); % linear adr. from 0
        PLA=double(PData.Region.PixelsLA(j));%pixel linear address

        n = n+1;
        ReconInfo(kk).LUT(n) = int32(256); % weight of 1.0 in 24.8 format.
        n = n+1;
        x = PData.Origin(1) + floor(PLA/PData.Size(1))*PData.PDelta(1);
        z = PData.Origin(3) + rem(PLA,PData.Size(1))*PData.PDelta(3);
        zsq = z*z;

        pl = spcf*sqrt((x-mm2wls*Trans.ElementPos(kk,1)).^2 + zsq) + TX(kk).Delay(kk); %Transmit path length calculation
