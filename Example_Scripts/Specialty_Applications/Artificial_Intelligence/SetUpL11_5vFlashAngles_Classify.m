% File name: SetUpL11_5vFlashAngles_Classify.m
% Example of plane wave imaging with steering angle transmits and
% classifying image data in realtime
%
% Description:
%   This example script uses an external function to perform live
%   classification of image data. The script will load a predefined
%   classification model that was trained on previously acquired image
%   data. The model is loaded at the beginning of the script and will be
%   saved in the mat file for script execution.
%   See vsv.ai.vsvAIDemo and vsv.ai.vsvAITutorial for more information
%
% Usage:
%   start the script. While imaging find the place where you expect the
%   targets (e.g., bright circular inclusions), then hit the start button.
%   A second image will appear displaying the classification results in
%   form of some boundaries
%
%   Transmit Sequence:
...
%
% Last update:
% 25/10/2019 - test with SW 4.2.0

clear all; %#ok
P.startDepth = 5;   % Acquisition depth in wavelengths
P.endDepth   = 257; % This should preferrably be a multiple of 128 samples.
NumBuffer    = 1;

modelPath = fullfile( vsv.file.getVSXDir, ['UserData' filesep 'AI' filesep 'brightCircleModel.mat'] );

% load the classifier and save it in the mat file. this will then be used
% by the external function to classify the images
...
% Define a map threshold to exclude small classification scores
% Small classification scores are an indication of miss classification
mapThresh = 12; %10 Bright Circle (Not broken phantom) % 9.5 Small Dark Circle
...
PData(1).PDelta = [Trans.spacing./3, 0,  0.3072];
...
Resource.DisplayWindow(1).Title = 'AI-Bmode Image';
...
TGC.CntrlPts = [0 259.1693 505.4720 742.5843 988.5709 1023 1023 1023];
TGC.rangeMax = P.endDepth;
TGC.Waveform = computeTGCWaveform(TGC);

% Specify TPC high voltage
TPC.inUse            = 1;
TPC.hv               = 50;
TPC.maxHighVoltage   = 50;
TPC.highVoltageLimit = 50;
...
% Define a storage class for each Buffer
    % Save Data Process
Process(2).classname  = 'External'; % define the storage class name
Process(2).method     = 'externalAIFunction';    % to export files specify 'save'
Process(2).Parameters = { ...
    'srcbuffer', 'image', ...'receive',... % name of buffer to export.
...
UI(3).Control = {'UserB2','Style','VsPushButton','Label', 'Start'};
UI(3).Callback = text2cell('%StartCallback');

EF(1).Function = text2cell('SetUpL11_5vFlashAngles_Classify.m','%EF#1');

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 5;

% Save all the structures to a .mat file.
filename = 'L11-5vFlashAngles_Export_frames';
...
%EF#1
externalAIFunction(Image)

classifier   = evalin('base', 'classifier');
mapThreshold = evalin('base', 'mapThresh');

if evalin('base', 'exist(''isStartClassify'');')
   isStartClassify = evalin('base', 'isStartClassify');
else
   isStartClassify = false;
end
...
if ~isStartClassify
    fh = findobj( 'Style', 'pushbutton', 'String', 'Start' );
else
    fh = findobj( 'Style', 'pushbutton', 'String', 'Stop' );
end
...
assignin('base','isStartClassify', ~isStartClassify);
