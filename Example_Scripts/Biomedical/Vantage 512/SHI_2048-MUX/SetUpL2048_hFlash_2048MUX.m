% File name: SetUpL2048_hFlash_2048MUX.m - Hypothetical 2048-element linear
% array Flash imaging script for development and testing of Vantage 512
% system software using four acquisitions per frame with all 2048 elements
% transmitting, and receiving a different 512 element subset of the 2048 on
% each of the four acquisitions. This script is intended to be fully
% compatible with the Vantage 512 system with SHI 2048-MUX. Note however
% that HVMUX programming tables and SHI ES to CH mapping have been
% arbitrarily created; they will have to be updated to match actual
% hardware design when it becomes available
% 
% Last update
%    10/19/2020 Script created, based on the simOnly version of same basic script

clear all

numEl = 2048; % number of elements to use throughout the script
...
Trans.HVMux = computeSHIMux2048; % add the HV Mux programming for the SHI 2048-MUX
% End of Trans definition

% Create Papod arrays to be pasted into the TX and Receive structures; the
% fifth one enables all elements in parallel for 2048 Flash transmit
Papod = [ ones(1,512) zeros(1,512) zeros(1,512) zeros(1,512); ...
         ...
         zeros(1,512) zeros(1,512) zeros(1,512)  ones(1,512);
         ones(1,2048)];
...
pt1;
Media.attenuation = -1.5;
Media.function = 'movePoints';

% Specify Resources.
Resource.RcvBuffer(1).datatype = 'int16';
Resource.RcvBuffer(1).rowsPerFrame = 4096 * 4; % this size allows for maximum range
...
Resource.DisplayWindow(1).Title = 'L2048-hFlash_2048MUX';
...
% Specify TX structure array, using Mux aperture selecting all 2048 elements
...
Receive = repmat(struct('Apod', zeros(1, 2048), ...
                        'aperture', 1, ...
...
for i = 1:Resource.RcvBuffer(1).numFrames  % 3 acquisitions per frame
    fi = 4*(i-1); % index of last receive for previous frame
    Receive(fi+1).callMediaFunc = 1;
    for n = 1:4
        % four acquisitions for frame i
        Receive(fi+n).framenum = i;
        Receive(fi+n).acqNum = n;
        Receive(fi+n).Apod = Papod(n, :);
        Receive(fi+n).aperture = Paper(n);
    end
end
...
save('MatFiles/L2048-hFlash_2048MUX');
return
