% File name: SetUpL512_hFlash3ap_512.m - Hypothetical 512-element linear array
% imaging script for development and testing of Vantage 512 system
% using Flash acquisition with three receive apertures
...
Resource.DisplayWindow(1).Title = 'L512-hFlash3ap_512';
...
Receive = repmat(struct('Apod', zeros(1,numEl), ...
...
for i = 1:Resource.RcvBuffer(1).numFrames  % 3 acquisitions per frame
    ...
    Receive(3*i-2).Apod(1:round(numEl/3)) = 1.0;
    ...
    Receive(3*i-1).Apod(round(numEl/3)+1:round(2*numEl/3)) = 1.0;
    Receive(3*i-1).framenum = i;
    Receive(3*i-1).acqNum = 2;
    % -- 3rd synthetic aperture acquisition for aperture 129.
    Receive(3*i).Apod(round(2*numEl/3)+1:numEl) = 1.0;
...
save('MatFiles/L512-hFlash3ap_512');
return
