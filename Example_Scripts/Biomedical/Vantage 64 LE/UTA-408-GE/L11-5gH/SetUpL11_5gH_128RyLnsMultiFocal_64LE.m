...
%   for its use
%
% File name: SetUpL11_5gH_128RyLnsMultiFocal_64LE.m - Imaging with dynamic focus transmits
%
% Description:
%   Sequence programming file for L11-5gH Linear array, using 128 ray lines
...
%   limited based on user-entered transmit focus and f-number. All 64
...
Resource.RcvBuffer.rowsPerFrame = nr*4096; % this should be larger than 128*Receive.endDepth*2(round trip)*2(smpls/wave) for max depth
Resource.RcvBuffer.colsPerFrame = Resource.Parameters.numTransmit;
Resource.RcvBuffer.numFrames = 10;
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow.Title = 'L11-5gH_128RyLnsMultiFocal_64LE';
...
% - We need 2*nr*fz Receives for every frame.
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'sampleMode', 'BS100BW',...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, 2*fz*nr*Resource.RcvBuffer(1).numFrames);
    ...
    for j = 1:2:2*P.numRays
        ...
        for z = 1:2:fz*2
            ...
            if z == 1
                ...
                Receive(k+w+z).endDepth = RcvZone((z+1)/2);
            end
            Receive(k+w+z).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
            Receive(k+w+z).framenum = i;
            Receive(k+w+z).acqNum = j;      % two acquisitions per frame
            %Second half of aperture - Set max acq length for first TX in each set of focal zones
            if z == 1
                Receive(k+w+z+1).endDepth = maxAcqLength;
            else
                Receive(k+w+z+1).endDepth = RcvZone((z+1)/2);
            end
            Receive(k+w+z+1).Apod((Resource.Parameters.numRcvChannels+1):Trans.numelements) = 1.0;
            Receive(k+w+z+1).framenum = i;
            Receive(k+w+z+1).acqNum = j+1;  % two acquisitions per frame
        end
    end
end

% Specify TGC Waveform structure.
TGC.CntrlPts = [0,239,455,593,742,848,878,920];
...
for j = 1:2:2*nr
    ReconInfo(j).txnum = 1+(fz*(j-1))/2;
    ReconInfo(j).rcvnum = 1+fz*(j-1);
    ReconInfo(j).regionnum = (j+1)/2;
    ReconInfo(j+1).mode = 'accumIQ_replaceIntensity'; % accumulate IQ and detect
    ReconInfo(j+1).txnum = 1+(fz*(j-1))/2;
    ReconInfo(j+1).rcvnum = 2+fz*(j-1) ;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:nr                     % Acquire frame
        w = 2*fz*(j-1);
        %Acquire all focal zones in ascending order overwriting previous
        %data with a shorter acq
        for z = 1:2:2*fz             % Acquire Zones
...
save('MatFiles/L11-5gH_128RyLnsMultiFocal_64LE');


% **** Callback function to be used by UI control. ****
