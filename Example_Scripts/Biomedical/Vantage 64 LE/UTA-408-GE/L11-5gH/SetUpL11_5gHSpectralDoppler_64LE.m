...
%   for its use
%
% File name: SetUpL11_5gHSpectralDoppler_64LE.m - Spectral doppler with plane wave transmit.
%
% Description:
...
% 12/30/2020 - Update to support Verasonics Viewer and ROI graphic
...
SDDespeckle = 0.4;
dopWFCutoffNorm = 0.025; %all filter normalized cutoff frequency
...
Resource.DisplayWindow(1).Title = 'L11-5gHSpectralDoppler_64LE';
...
SeqControl(3).command = 'loopCnt'; % for 2D only
SeqControl(3).argument = 0;
SeqControl(4).command = 'loopCnt'; % for doppler
...
Event(n).info = 'Jump back to the start of aquisition';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0;
...
save('MatFiles/L11-5gHSpectralDoppler_64LE');
