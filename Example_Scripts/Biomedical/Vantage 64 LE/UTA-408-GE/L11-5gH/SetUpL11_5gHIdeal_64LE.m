...
%   for its use.
%
% File name: SetUpL11_5gHIdeal_64LE.m - Example of imaging using ideal synthetic
%                                 aperture reconstruction
%
% Description:
%   Sequence programming file for L11-5gH Linear array, using ideal synthetic
%   aperture reconstruction. For each acquisition, one channel is active
%   for transmit and 64 channels are active for each of the 2-1 synthetic
%   aperture receive. Reconstruction and processing are synchronoused with
%   respect to acquisition.
%
% Last update:
% 09/21/21 modified for L11-5gH
...
Resource.RcvBuffer(1).rowsPerFrame = 1536*m*2; % 1280 samples ok for up to 240 wl range at 2 samples per wavelength.
...
Resource.DisplayWindow(1).Title = 'L11-5gHIdeal_64LE';
...
% - We need 2*m Receive structures for each frame.
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'sampleMode','BS100BW', ...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, 2*m*Resource.RcvBuffer(1).numFrames);
% - Set event specific Receive attributes.
for i = 1:Resource.RcvBuffer(1).numFrames
    Receive(2*m*(i-1)+1).callMediaFunc = 1;
    for j = 1:2:2*m
        Receive(2*m*(i-1)+j).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
        Receive(2*m*(i-1)+j).framenum = i;
        Receive(2*m*(i-1)+j).acqNum = j;      % two acquisitions per frame
        Receive(2*m*(i-1)+j+1).Apod((Resource.Parameters.numRcvChannels+1):Trans.numelements) = 1.0;
        Receive(2*m*(i-1)+j+1).framenum = i;
        Receive(2*m*(i-1)+j+1).acqNum = j+1;  % two acquisitions per frame
    end
end

% Specify Recon structure arrays.
% - We need one Recon structure.  Each frame will use
%   2*m ReconInfo structures, since we are using m 2-1
...
for j = 1:2:2*m  % For each row in the column
    ReconInfo(j).txnum = (j+1)/2;
    ReconInfo(j).rcvnum = j;
    ReconInfo(j+1).txnum = (j+1)/2;
    ReconInfo(j+1).rcvnum = j+1;
end
    ReconInfo(2*m).mode = 'accumIQ_replaceIntensity'; % accum & detect
else
...
save('MatFiles/L11-5gHIdeal_64LE');
