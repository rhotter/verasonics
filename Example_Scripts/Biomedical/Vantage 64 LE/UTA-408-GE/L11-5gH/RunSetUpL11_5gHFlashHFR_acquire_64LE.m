...
%   for its use
%
% File name: RunSetUpL11_5gHFlashHFR_acquire_64LE.m - "High Frame Rate" acquisition for ultrafast imaging (script 1 of 2).
%
% Description:
...
P.numFrames = 10;      % no. of Receive frames (real-time images are produced 1 per frame)
simulateMode = 0;   % set to acquire data using Vantage 128 hardware

RFdataFilename = 'RFdataHFR_64LE';
...
filename = 'L11-5gHFlashHFR_acq_64LE'; % used to launch VSX automatically
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'sampleMode', 'NS200BW', ...
                        'mode', 0, ...
                        'callMediaFunc', 1), 1,2*P.numFrames*P.numAcqs);  % movepoints EVERY acquisition to illustrate superframe concept
...
if simulateMode ~= 2
    ...
    save (['MatFiles/',RFdataFilename], 'RcvData', 'P')
end

% then run full playback automatically after quitting the first script
RunSetUpL11_5gHFlashHFR_reconAll_64LE    % be sure that simulateMode=2 in that script!!!
