...
%   for its use
%
% File name: SetUpC5_2gH_128RyLns_64LE.m - Example of curved array imaging with
%                                    focus transmits
% Description:
%   Sequence programming file for C5-2gH curved array using 128 ray lines
%   (focus transmits) and 2-1 synthetic aperture receive acquisitions. Of
%   the 128 transmit channels, only 65 are used, with 32 transmitters on
%   each side of the center element (where possible). All 64 receive
%   channels are active for each synthetic aperture acquisition. The receive
...
Resource.RcvBuffer.rowsPerFrame = P.numRays*2048;    % 128 segments of 4096 samples
...
Resource.DisplayWindow(1).Title = 'C5-2gH_128RyLns_64LE';
...
maxAcqLength = ceil(sqrt((P.endDepth+radius)^2 + radius^2 - ...
                     2*(P.endDepth+radius)*radius*cos(-2*theta)));
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:m
        ce = round(1+127*(Angle(j) - theta)/(-2*theta));
        lft = ce - 31;
        if lft < 1, lft = 1; end
        rt = ce + 32;
        if rt > Trans.numelements, rt = Trans.numelements; end

        Receive(k+j).Apod(lft:rt) = 1.0;
...
TGC.CntrlPts = [0,261,520,695,797,877,916,956];
...
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:P.numRays                     % Acquire frame
        Event(n).info = 'Acquisition ray line';
        Event(n).tx = j;
...
save('MatFiles/C5-2gH_128RyLns_64LE');
