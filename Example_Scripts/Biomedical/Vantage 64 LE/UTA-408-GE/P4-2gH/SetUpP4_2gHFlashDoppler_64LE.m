...
%   for its use.
%
% File name: SetUpP4_2gHFlashDoppler_64LE.m - Example of phased array flash
%                                       Doppler imaging
% Description:
%   Sequence programming file for P4-2gH phased array in virtual apex format,
%   using flash tranmists for 2D (B-mode) and doppler imaging. 96 transmit
%   & 64 receive channels are active for each acquisition. The curvature of the
...
aperture = 96*Trans.spacing; % aperture based on 64 elements
P(1).theta = -pi/4;
...
Resource.DisplayWindow(1).Title = 'P4-2gHFlashDoppler_64LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'Delay', zeros(1,96)), 1, na+1); % na TXs for 2D + 1 for Doppler
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:na
        Receive(j+k).Apod(Trans.numelements/2-31:Trans.numelements/2+32) = 1;        
...
save('MatFiles/P4-2gHFlashDoppler_64LE');
