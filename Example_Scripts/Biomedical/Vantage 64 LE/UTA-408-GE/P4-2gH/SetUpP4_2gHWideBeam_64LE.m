...
%   for its use.
%
% File name: SetUpP4_2gHWideBeam_64LE.m - Example of phased array imaging with
%                                   wide beam transmits
% Description:
%   Sequence programming file for P4-2gH phased array in virtual apex format,
%   using m wide beam tranmits and receive acquisitions. 96 transmit and
%   64 receive channels are active for each acquisition. Processing is
...
P.aperture = 96*Trans.spacing; % aperture based on 96 elements
...
Resource.RcvBuffer(1).rowsPerFrame = P.numRays*4096; % Allow for max range setting
...
Resource.DisplayWindow(1).Title = 'P4-2gHWideBeam_64LE';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays
        Receive(P.numRays*(i-1)+j).Apod(Trans.numelements/2-31:Trans.numelements/2+32) = 1;
...
save('MatFiles/P4-2gHWideBeam_64LE');

% **** Callback routines that are used by UI Controls ****
