...
%   for its use
%
% File name: SetUpP4_2gH_128RyLns_64LE.m - Example of phased array imaging with
%                                    focus transmits
% Description:
%   Sequence programming file for P4-2gH phased array in virtual apex format,
%   using 128 ray lines (focus transmits) and receive acquisitions. 96
%   transmit and 64 receive channels are active for each acquisition. The
%   receive acquisitions use 100% bandwidth to improve DMA transfers.
%   Processing is asynchronous with respect to acquisition. Note: The P4-2gH
%   is a 96 element probe that is wired to the scanhead connector with
%   element 1-96 connected to inputs 1-96. We therefore need a Trans.Connector array to
...
Resource.DisplayWindow(1).Title = 'P4-2gH_128RyLns_64LE';
...
for n = 1:P.numRays   % P.numRays transmit events
    ...
    Ce(n) = round(96*(TXorgs(n) - Trans.ElementPos(1,1))/P.aperture); % will be used for Receive.Apod
end
%clear Angles TXorgs
...
UI(2).Control = VsSliderControl('LocationCode','UserA1',...
    ...
    'SliderMinMaxVal',[64,300,P.endDepth]*wls2mm,'SliderStep',[0.1,0.2],...
    'ValueFormat','%3.0f',...
...
save('MatFiles/P4-2gH_128RyLns_64LE');
