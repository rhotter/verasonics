...
%   for its use.
%
% File name: SetUpP4_2gHWideBeamHI_64LE.m - Example of phased array pulse inversion
...
%   96 transmit and 64 receive channels are active for each acquisition.
%   Processing is asynchronous with respect to acquisition. 
%
% Last update:
% 7/7/2020 - VTS-1758 Rcv Buffer too small for max range
% 09/27/2021 - modifed for P4-2gH 
...
P.aperture = 96*Trans.spacing; % aperture based on 64 elements
P.theta = -(pi/360)*80;  % 80 degree sector
...
Resource.RcvBuffer(1).rowsPerFrame = P.numRays*5120;  % Allow for max range setting
...
Resource.DisplayWindow(1).Title = 'P4-2gHWideBeamHI_64LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'focus', 4*P.endDepth, ...
                   'Steer', [0.0,0.0], ...
                   'Apod', ones(1,96), ...  % set TX.Apod for 64 elements
                   'Delay', zeros(1,96), ...
                   'TXPD', [], ...
                   'peakCutOff', 2.0, ...
...
BPF = ...
[+0.00043 -0.00012 -0.00156 +0.00024 -0.00040 +0.00693 -0.00439 ...
 +0.00226 -0.01907 +0.01599 -0.00488 +0.03864 -0.03867 +0.00339 ...
 -0.06189 +0.07739 +0.01883 +0.08124 -0.17725 -0.22604 +0.57788];
...
Receive = repmat(struct('Apod', zeros(1,96), ...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.numRays
        Receive(2*P.numRays*(i-1)+j).Apod(Trans.numelements/2-31:Trans.numelements/2+32) = 1;
        Receive(2*P.numRays*(i-1)+j).framenum = i;
        Receive(2*P.numRays*(i-1)+j).acqNum = j;
        Receive(2*P.numRays*(i-1)+j+P.numRays).Apod = Receive(2*P.numRays*(i-1)+j).Apod;
...
TGC.CntrlPts = [306,722,750,805,843,964,1023,1023];
...
SeqControl(2).argument = 265;
SeqControl(3).command = 'timeToNextAcq'; % time between frames
SeqControl(3).argument = 30000 - (2*P.numRays-1)*265;
...
save('MatFiles/P4-2gHWideBeamHI_64LE');
