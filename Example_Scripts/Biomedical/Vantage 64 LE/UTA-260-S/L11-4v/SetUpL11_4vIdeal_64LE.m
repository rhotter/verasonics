...
%   for its use.
%
% File name: SetUpL11_4vIdeal_64LE.m - Example of imaging using ideal synthetic
...
%   aperture receive. Reconstruction and processing are synchronized with
...
Resource.RcvBuffer(1).rowsPerFrame = 1280*m*2; % 1280 samples ok for up to 240 wl range at 2 spw.
...
Resource.DisplayWindow(1).Title = 'L11-4vIdeal_64LE';
...
% - We need 2*m Receive structures for each frame (with synthetic aperture).
...
%   2*m ReconInfo structures, since we are using 2*m
...
save('MatFiles/L11-4vIdeal_64LE');
return
