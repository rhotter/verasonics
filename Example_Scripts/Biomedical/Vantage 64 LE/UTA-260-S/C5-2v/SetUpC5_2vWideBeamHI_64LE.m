...
%   for its use.
%
% File name: SetUpC5_2vWideBeamHI_64LE.m - Example of curved array imaging with
%                                   wide beam transmits and harmonic imaging.
% Description:
%   Sequence programming file for C5-2v curved array using m wide beam pulse
%   inversion transmits and 2-1 synthetic aperture receive acquisitions.
%   Of the 128 transmit channels, only 81 are used, with 40 transmitters on
%   each side of the center element (where possible). All the 64 receive
%   channels are active for each synthetic aperture acquisation. The receive
...
Resource.RcvBuffer.numFrames = 6;
...
Resource.DisplayWindow(1).Title = 'C5-2vWideBeamHI_64LE';
...
m = P.numRays;
    frameOffset = m*Resource.RcvBuffer(1).numFrames;
for i = 1:Resource.RcvBuffer(1).numFrames

    Receive(frameOffset+1).callMediaFunc = 1;
    for j = 1:m
        Receive(m*(i-1)+j).framenum = i;
        ce = round(1+(Angle(j) + scanangle/2)/scanangle * 127);
        lft = ce - 31;
        if lft < 1, lft = 1; end;
        rt = ce + 32;
        if rt > Trans.numelements, rt = Trans.numelements; end;
        ...
        % Accumulation for pulse inversion
        Receive(m*(i-1)+j+frameOffset).framenum = i;
        Receive(m*(i-1)+j+frameOffset).acqNum = j;
        Receive(m*(i-1)+j+frameOffset).Apod = Receive(m*(i-1)+j).Apod;
		Receive(m*(i-1)+j+frameOffset).mode = 1;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:P.numRays            % Acquire rays
        ...
        Event(n).tx = m + j;
        Event(n).rcv = P.numRays*(i-1)+j+ frameOffset;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 2;
        n = n+1;
...
UI(3).Control = {'UserB4','Style','VsSlider','Label',['TX Focus (',AxesUnit,')'],...
                 'SliderMinMaxVal',[100,800,P.txFocus]*wls2mm,'SliderStep',[0.1,0.2],'ValueFormat','%3.0f'};
...
save('MatFiles/C5-2vWideBeamHI_64LE');
return
