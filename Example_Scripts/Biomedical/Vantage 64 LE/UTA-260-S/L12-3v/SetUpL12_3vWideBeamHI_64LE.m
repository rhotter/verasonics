...
%   for its use
%
% File name SetUpL12_3vWideBeamHI_64LE.m:
%
% Description:
%   Sequence programming file for L12-3v Linear array, using numrays
%   wide beam pulse-inversion transmits and receive acquisitions. The transmit
%   aperture size is set by the variable - numTx, and all 64 receive channels
%   are active for each synthetic aperture acquisition. The transmit and receive
%   apertures are moved across the transducer elements and positioned as
%   follows at different central element locations for numTx=48, P.numRays=48.
%   (each char represents 4 elements, and l and r represents the two halves
%    of 2-1 synthetic aperture on left and right half respectively)
%
%   Aperture for the 1st wide beam
%   tttttt-------------------------\----------------
%   llllllllllllllllrrrrrrrrrrrrrrrr----------------
%   Aperture for the 24th wide beam:
%   --------/----------tttttttttttt--------\--------
%   --------llllllllllllllllrrrrrrrrrrrrrrrr--------
%   Aperture for the 48th wide beam:
%   -----------------/-------------------------tttttt
%   -----------------llllllllllllllllrrrrrrrrrrrrrrrr
%
%   The receive data from each of these apertures for each wide beam are
%   stored under different acqNums in the Receive buffer. The reconstruction
%   sums the IQ data from the aquisitions for all beams that contribute to
%   a pixel and computes intensity values to produce the full frame.
...
Resource.RcvBuffer(1).rowsPerFrame = P.numRays*6144; % this size allows for max range
...
Resource.DisplayWindow(1).Title = 'L12-3vWideBeamHI_64LE';
...
% -- Determine x coords. of aperture centers.
TxOrgX = (-95.5*Trans.spacing):(191*Trans.spacing/(P.numRays-1)):(95.5*Trans.spacing);
for n = 1:P.numRays
  ...
  % Add a window on aperture apodization
  Vtemp=kaiser(P.numTx+1,1);
  if n>P.numRays/2
    V = Vtemp(1:size(V,2));           % aperture located at the right half of transducer
  else
    V = Vtemp((P.numTx+1-size(V,2)+1):end);% aperture located at the left half of transducer
  end
  ...
  % Define parameters for inverted pulse.
...
for i = 1:Resource.RcvBuffer(1).numFrames
    Receive(4*P.numRays*(i-1)+1).callMediaFunc = 1;
    for j = 1:2:2*P.numRays
        Receive(4*P.numRays*(i-1)+j).aperture = TX((j+1)/2).aperture; % aperture same as transmit
        Receive(4*P.numRays*(i-1)+j).Apod(1:64) = 1.0;
        Receive(4*P.numRays*(i-1)+j).framenum = i;
        Receive(4*P.numRays*(i-1)+j).acqNum = j;
        Receive(4*P.numRays*(i-1)+j+1).aperture = TX((j+1)/2).aperture; % aperture same as transmit
        Receive(4*P.numRays*(i-1)+j+1).Apod(65:128) = 1.0;
        Receive(4*P.numRays*(i-1)+j+1).framenum = i;
        Receive(4*P.numRays*(i-1)+j+1).acqNum = j+1;  % two acquisitions per frame
    end
    k = 2*P.numRays;
    for j = 1:2:2*P.numRays
        Receive(4*P.numRays*(i-1)+j+k).mode = 1;
        Receive(4*P.numRays*(i-1)+j+k).aperture = TX((j+1)/2).aperture; % aperture same as transmit
        Receive(4*P.numRays*(i-1)+j+k).Apod(1:64) = 1.0;
        Receive(4*P.numRays*(i-1)+j+k).framenum = i;
        Receive(4*P.numRays*(i-1)+j+k).acqNum = j;
        Receive(4*P.numRays*(i-1)+j+k+1).mode = 1;
        Receive(4*P.numRays*(i-1)+j+k+1).aperture = TX((j+1)/2).aperture; % aperture same as transmit
        Receive(4*P.numRays*(i-1)+j+k+1).Apod(65:128) = 1.0;
        Receive(4*P.numRays*(i-1)+j+k+1).framenum = i;
        Receive(4*P.numRays*(i-1)+j+k+1).acqNum = j+1;  % two acquisitions per frame
    end
end
...
for j = 1:2:2*P.numRays
        % synthetic aperture first half
    ReconInfo(j).txnum = (j+1)/2;
    ReconInfo(j).rcvnum = j;
    ReconInfo(j).regionnum = (j+1)/2;
    % synthetic aperture second half
...
SeqControl(3).argument = 50000 - (4*P.numRays-1)*160;  % 50000 usec = 50msec time between frames
...
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:4:4*P.numRays        % Acquire frame
        ...
        Event(n).tx = (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+(j+1)/2;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 2;
        n = n+1;

        Event(n).info = '1st half of aperture, inverted transmit';
        Event(n).tx = P.numRays + (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+(j+1)/2 + 2*P.numRays;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 2;
        n = n+1;
        ...
        Event(n).tx = (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+(j+1)/2+1;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 2;
        n = n+1;

        Event(n).info = '2nd half of aperture, inverted transmit';
        Event(n).tx = P.numRays + (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+(j+1)/2+1 + 2*P.numRays;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 2;
        n = n+1;
...
save('MatFiles/L12-3vWideBeamHI_64LE');
