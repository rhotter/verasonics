...
%   for its use
%
% File name SetUpL12_3vWideBeam_64LE.m: for 12-3v linear array wide beam
% transit
%
% Description:
%   Sequence programming file for L12-3v Linear array, using numrays
%   wide beam transmits and receive acquisitions. The transmit aperture
%   size is set by the variable - numTx, and all 64 receive channels are
%   active for each synthetic aperture acquisition. The transmit and receive
%   apertures are moved across the transducer elements and positioned as
%   follows at different central element locations for numTx=48, numRays=48.
...
P.numRays = 64;     % Number of ray lines in frame.
...
Resource.RcvBuffer(1).rowsPerFrame = P.numRays*4096*2; % this size allows for max range
...
Resource.DisplayWindow(1).Title = 'L12-3vWideBeam_64LE';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:2:2*P.numRays
        Receive(2*P.numRays*(i-1)+j).aperture = TX((j+1)/2).aperture; % aperture same as transmit
        Receive(2*P.numRays*(i-1)+j).Apod(1:64) = 1.0;
        Receive(2*P.numRays*(i-1)+j).framenum = i;
        Receive(2*P.numRays*(i-1)+j).acqNum = j;
        Receive(2*P.numRays*(i-1)+j+1).aperture = TX((j+1)/2).aperture; % aperture same as transmit
        Receive(2*P.numRays*(i-1)+j+1).Apod(65:128) = 1.0;
        Receive(2*P.numRays*(i-1)+j+1).framenum = i;
        Receive(2*P.numRays*(i-1)+j+1).acqNum = j+1;  % two acquisitions per frame
    end
end
...
SeqControl(3).argument = 5000;  % 5000 usec = 5 msec time between frames
...
save('MatFiles/L12-3vWideBeam_64LE');
