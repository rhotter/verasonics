...
%   for its use
%
% File name: SetUpL12_3vFlashDoppler_64LE.m - Example of 3-1 synthetic aperture plane
%                                       wave imaging with steering angle transmits
% Description:
%   Sequence programming file for L12-3v Linear array, using 3-1 synthetic
%   aperture plane wave transmits with multiple steering angles on 128
%   channels system. 128 transmit channels and 64 receive channels are
%   active and positioned as follows (each char represents 4 elements) for
%   each of the 3 synthetic apertures.
%
%   Element Nos.                                1              1
%                               6       9       2              9
%               1               5       7       9              2
...
Trans.maxHighVoltage = 23;  % limit to prevent overcurrent on HVMUX supply.
...
PData(2).Origin = [-Trans.spacing*31.5,0,P(2).startDepth]; % x,y,z of upper lft crnr.
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*(3*na+ne); % this size allows for 3 acqs per angle, maximum range
...
Resource.DisplayWindow(1).Title = 'L12-3vFlashDoppler_64LE';
...
TW(2).Parameters = [7.813,0.67,6,1];
...
for n = 1:3:3*na   % 3*na transmit events
    ...
    TX(n+1).aperture = 33; % Use the tx aperture that starts at element 33.
    TX(n+1).Delay = computeTXDelays(TX(n+1),'TOAE');
    TX(n+2).Steer = [angle,0.0];
    TX(n+2).aperture = 65; % Use the tx aperture that starts at element 65.
    TX(n+2).Delay = computeTXDelays(TX(n+2),'TOAE');
end
% -- only one TX struct needed for Doppler
TX(3*na+1).waveform = 2;
TX(3*na+1).Apod(97:end) = 0;
TX(3*na+1).aperture = 49;
TX(3*na+1).Steer = [dopAngle,0.0];
TX(3*na+1).Delay = computeTXDelays(TX(2*na+1));
...
for i = 1:Resource.RcvBuffer(1).numFrames  % 3*na acquisitions per frame
    k = (3*na+ne)*(i-1);
    Receive(k+1).callMediaFunc = 1;
    % -- Acquisitions for aperture 1.
    for j = 1:na; % for each aperture acquire all angles
        Receive(k+j).Apod(1:64) = 1.0;
        Receive(k+j).aperture = 1;
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;
    end
    % -- Acquisitions for aperture 33.
    for j = 1:na
        Receive(k+j+na).Apod(33:96) = 1.0;
        Receive(k+j+na).aperture = 33;
        Receive(k+j+na).framenum = i;
        Receive(k+j+na).acqNum =j+na;
    end
    % -- Acquisitions for aperture 65.
    for j = 1:na
        Receive(k+j+2*na).Apod(65:128) = 1.0;
        Receive(k+j+2*na).aperture = 65;
        Receive(k+j+2*na).framenum = i;
        Receive(k+j+2*na).acqNum = j+2*na;
    end
    for j = (3*na+1):(3*na+ne)
        Receive(j+k).Apod(1:64) = 1;
        Receive(j+k).aperture = 49;
...
if na>1
    ...
    for j = 1:na
        ReconInfo(j).txnum = 3*(j-1)+1;
        ReconInfo(j).rcvnum = j;
    end
    for j = 1:na
        ReconInfo(j+na).txnum = 3*(j-1)+2;
        ReconInfo(j+na).rcvnum = j+na;
    end
    for j = 1:na
        ReconInfo(j+2*na).txnum = 3*(j-1)+3;
        ReconInfo(j+2*na).rcvnum = j+2*na;
    end
...
for i = 1:Resource.RcvBuffer(1).numFrames
    k = (3*na+ne)*(i-1);
    for j = 1:na
        Event(n).info = '1st third of aperture.';
        Event(n).tx = 3*(j-1)+1;
        Event(n).rcv = k+j;
        Event(n).recon = 0;
        Event(n).process = 0;
    ...
    for j = 1:na
        Event(n).info = '2nd third of aperture.';
        Event(n).tx = 3*(j-1)+2;
        Event(n).rcv = k+j+na;
        Event(n).recon = 0;
        Event(n).process = 0;
    ...
    for j= 1:na
        Event(n).info = '3rd third of aperture.';
        Event(n).tx = 3*(j-1)+3;
        Event(n).rcv = k+j+2*na;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 1;
        n = n+1;
...
save('MatFiles/L12-3vFlashDoppler_64LE');
return
