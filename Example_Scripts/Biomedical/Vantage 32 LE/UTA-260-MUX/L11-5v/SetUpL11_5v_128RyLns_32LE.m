...
%   for its use
%
% File name: SetUpL11_5v_128RyLns_32LE.m - Example of linear array imaging with
%                         focused transmits on Vantage 32LE UTA Mux system
% Description:
%   Sequence programming file for L11-5v Linear array, using 128 ray lines
%   (focus transmits) with 4-1 synthetic aperture (Four acquisitions per line
...
Resource.RcvBuffer(1).rowsPerFrame = 6144*128;   % this size allows for maximum range
...
Resource.DisplayWindow(1).Title = 'L11-5v_128RyLns_32LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'Delay', zeros(1,Resource.System.transmitChannels)), 1, 128);
...
Receive = repmat(struct('Apod', zeros(1,Resource.System.transmitChannels), ...
                        ...
                        'sampleMode', 'BS100BW',...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, 4*nr*Resource.RcvBuffer(1).numFrames);
...
%  - Time between frames at 26 fps at max endDepth.
SeqControl(2).command = 'timeToNextAcq';
SeqControl(2).argument = 20000;
%  - Return to Matlab
...
n = 1;
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:4:4*nr                      % Acquire frame
        Event(n).info = '1st aperture.';
        Event(n).tx = (j+3)/4;
        Event(n).rcv = 4*nr*(i-1)+j;
        Event(n).recon = 0;
        Event(n).process = 0;
...
save('MatFiles/L11-5v_128RyLns_32LE');
