...
%   for its use.
%
% File name SetUpL11_4vWideBeam_32LE.m: example of linear array wide beam
% transmit
% Generate .mat Sequence Object file for L11_4v Linear array wide beam transmit. The
% transmit aperture size can be set by the variable - numTx. The transmit aperture is
% translated across the array with the number of rays set by the variable - numRays.
% On receive, a 64 element aperture is used with 2 synthetic aperture acquisitions.
%
...
Resource.RcvBuffer(1).rowsPerFrame = 2*P.numRays*4096;
...
Resource.DisplayWindow(1).Title = 'L11-4vWideBeam_32LE';
...
for n = 1:P.numRays
    ...
    % Compute 64 channel transmit aperture around Ce(n).
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:2:2*P.numRays
        n = (j+1)/2;
        Receive(k+j).aperture = TX(n).aperture; % mux aperture same as transmit
        % Compute 64 channel Receive.Apod centered around center element, Ce((j+1)/2)
        lft = Ce(n) - 31;
        if lft < 1
            lft = 1;
            rt = 64;
        ...
        if rt > Trans.numelements
            rt = Trans.numelements;
            lft = Trans.numelements - 63;
        end
        % Acquire first half of aperture
        Receive(k+j).Apod(1:32) = 1;
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;
        % Acquire second half of aperture
        Receive(k+j+1).aperture = Receive(k+j).aperture;
...
for j = 1:2:2*P.numRays
    ...
    ReconInfo(j).regionnum = k;
    ReconInfo(j+1).txnum = k;
    ReconInfo(j+1).rcvnum = j+1;
    ReconInfo(j+1).regionnum = k;
end
...
SeqControl(3).argument = 24000-(P.numRays-1)*TTNA;  % 20 ms between frames
...
save('MatFiles/L11-4vWideBeam_32LE');
