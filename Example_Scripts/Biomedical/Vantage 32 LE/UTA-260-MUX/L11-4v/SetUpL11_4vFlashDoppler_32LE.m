...
%   for its use
%
% File name: SetUpL11_4vFlashDoppler_32LE.m -  Example of 4-1 synthetic aperture doppler
%                                     imaging woth flash transmits
% Description:
%   Sequence programming file for L11-4v Linear array, using 4-1
%   synthetic aperture plane wave transmits and receive acquisitions on
%   64 channels system. 64 transmit channels and 32 receive channels
%   are active and positioned as follows (each char represents 2 elements)
%   for each of the 4 synthetic apertures over na steering angles.
%
%   Element Nos.                                                               1
%                               3   4           6         8    9               2
%               1               3   2           5         6    6               8
...
%   4 apertures and multiple angles and computes intensity values to produce
%   the full frame.
%
...
%
% Last update:
%   05/25/18 - Script improvements. Using dynmamic MUX programming
...
PData(2).Region = struct('Shape',struct('Name','Parallelogram', ...
                               'Position', [PData(2).Origin(1)+(P(2).firstElem+17)*Trans.spacing,0,P(2).startDepth], ...
                               'width', 32*Trans.spacing, ...
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*(na*4 + ne);
...
Resource.DisplayWindow(1).Title = 'L11-4vFlashDoppler_32LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'Delay', zeros(1,Resource.System.transmitChannels)), 1, (4*na)+1); % na TXs for 2D + 1 for Doppler
...
for j = 1:4:4*na
    ...
    TX(j+1).aperture = 17;% Use the tx aperture that starts at element 33.
    TX(j+1).Steer = [angle,0.0];
    TX(j+1).Delay = computeTXDelays(TX(j+1));
    TX(j+2).aperture = 49; % Use the tx aperture that starts at element 65.
...
% - We need 4*na Receives for a 2D frame and ne Receives for a Doppler frame.
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:4:4*na  % acquisitions for 2D (4 acqs for each angle).
    ...
    for j = (4*na+1):(4*na+ne)
        % Doppler acquisition
        Receive(j+k).aperture = TX(4*na+1).aperture;  % use aperture starting at P(2).firstElem
...
Recon(1).RINums = 1:4*na;  % 3*na ReconInfos needed for na angles and 3 apertures.
k = 4*na + 1;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = (4*na+1):(4*na+ne)   % Acquire Doppler ensemble.
    ...
    Event(n).process = 3;
    Event(n).seqControl = 0;
    if floor(i/4) == i/4     % Exit to Matlab every 4th frame
...
save('MatFiles/L11-4vFlashDoppler_32LE');
return
