...
%   for its use
%
% File name: SetUpC5_2v_128RyLns_32LE.m - Example of curved array imaging with
%                                    focus transmits
% Description:
...
%   to acquire data from all 128 receive elements). The receive acquisitions use 100% bandwidth to improve DMA
%   transfers. Processing is asynchronous with respect to acquisition.
%
%
% 09/04/2018 Modified for 32LE Vantage system
...
Resource.RcvBuffer.rowsPerFrame = P.numRays*4096*4;    % 2*128 segments of 4096 samples
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:4:4*P.numRays
        ...
        Receive(j+k).aperture = 1; % use precomputed apertures
...
ReconInfo = repmat(struct('mode', 'accumIQ', ...  % replace data.
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 1), 1, 4*P.numRays);
...
SeqControl(2).argument = 210; % 150 usec/ray * 128 *4 = 76.8msec
SeqControl(3).command = 'timeToNextAcq';
SeqControl(3).argument = 100000; % 100 msec
...
save('MatFiles/C5-2v_128RyLns_32LE');
return
...
if txNumEl > (Resource.System.transmitChannels/2 - 1)
    txNumEl = floor(Resource.System.transmitChannels/2 - 1);
