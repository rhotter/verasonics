...
%   using a flash tranmist and receive acquisitions. All 64 transmit and
...
%   the scanhead connector with element 0-63 connected to inputs 1-64. We
...
%   For the Vantage 32 LE configuration, two synthetic
%   aperture acquisitions are done for each transmit, to allow use of all
%   64 elements on receive with the Vantage 32 LE configuration.
%
% Last update:
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*2*na;
...
Resource.DisplayWindow(1).Title = 'P4-2vFlashAngles_32LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'focus', -P.radius, ...     % set focus to negative for concave TX.Delay profile.
                   'Steer', [0.0,0.0], ...
                   'Delay', zeros(1,Resource.Parameters.numTransmit)), 1, na);

% - Set event specific TX attributes.

    for j = 1:na    % na transmit events
             TX(j).Steer = [(P.startAngle+(j-1)*dtheta),0.0];
...
Receive = repmat(struct('Apod', zeros(1, 64), ...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:na
        ...
        Receive(k+2*j-1).Apod(1:32) = 1; % enable first half of the aperture
        Receive(k+2*j).framenum = i;
        Receive(k+2*j).acqNum = 2*j;
        Receive(k+2*j).Apod(33:64) = 1; % enable first half of the aperture
    end
end
...
for j = 1:na  % For each row in the column
        ReconInfo(2*j-1).txnum = j;
        ReconInfo(2*j-1).rcvnum = 2*j-1;
        ReconInfo(2*j).txnum = j;
        ReconInfo(2*j).rcvnum = 2*j;
end
...
SeqControl(2).argument = 5000; % 5msec (~100fps)
SeqControl(3).command = 'timeToNextAcq';  % set time between frames
SeqControl(3).argument = 10000; % 10msec (~100fps)
...
for i = 1:Resource.RcvBuffer(1).numFrames
    k = 2*na*(i-1);
    for j = 1:na
    Event(n).info = 'Acquire 1st half aperture';
    Event(n).tx = j;
    Event(n).rcv = k+2*j-1;
    Event(n).recon = 0;
    Event(n).process = 0;
...
save('MatFiles/P4-2vFlashAngles_32LE');
return
