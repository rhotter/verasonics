...
%   for its use
%
% File name: SetUpL22_14vLF_FlashDM_32LE.m - Example of 4-1 synthetic aperture plane
%                                       wave imaging with 260-MUX UTA.
%
% Description:
%   Sequence programming file for L22-14vLF Linear array,using a full aperture transmit
...
%               ------------------------------------------------rrrrrrrrrrrrrrrr
%
%   The receive data from the four of these apertures are stored under
...
%
% Last update:
% 06/06/2018 - modified for dynamic mux programing
...
Resource.RcvBuffer(1).colsPerFrame = Resource.System.transmitChannels;
Resource.RcvBuffer(1).numFrames = 100;       % 100 frames used for RF cineloop.
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = 'L22-14vLF FlashDM_32LE, 4X sampling at 62.5 MHz';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Receive(k+1).Apod(1:32) = 1.0;
    Receive(k+1).aperture = computeMuxAperture(Receive(k+1).Apod, Trans);
    Receive(k+1).framenum = i;
    Receive(k+1).acqNum = 1;
    Receive(k+1).callMediaFunc = 1;
    % -- 2nd synthetic aperture acquisition for aperture starting at element 33.
    Receive(k+2).Apod(33:64) = 1.0;
    Receive(k+2).aperture = computeMuxAperture(Receive(k+2).Apod, Trans);
    Receive(k+2).framenum = i;
    Receive(k+2).acqNum = 2;
    % -- 3rd synthetic aperture acquisition for aperture starting at element 65.
    Receive(k+3).Apod(65:96) = 1.0;
    Receive(k+3).aperture = computeMuxAperture(Receive(k+3).Apod, Trans);
    Receive(k+3).framenum = i;
    Receive(k+3).acqNum = 3;
    % -- 4th synthetic aperture acquisition for aperture starting at element 97.
    Receive(k+4).Apod(97:128) = 1.0;
    Receive(k+4).aperture = computeMuxAperture(Receive(k+4).Apod, Trans);
    Receive(k+4).framenum = i;
    Receive(k+4).acqNum = 4;
end
...
ReconInfo(1).mode = 'replaceIQ'; % accumulate and detect IQ data in output buffer.
...
save('MatFiles/L22-14vLF_FlashDM_32LE');
return
