...
%   for its use
%
% File name: SetUpL22_14vLF_128RyLns_32LE.m.m - Example of scanline imaging with focused
%                                          transmits at a single focal depth.
%
% Description:
%   Sequence programming file for L22-14vLF Linear array, using 128 ray lines
%   (focus transmits) and 128 receive acquisitions. Of the 64 transmit
%   channels, the active transmit aperture is limited based on user-entered
%   transmit focus and f-number. All 32 receive channels are active for
...
%   the L22-14vLF (18MHz center frequency, 67% bandwidth). Processing is
%   asynchronous with respect to acquisition.
%
% Last update:
% 09/01/2018 - Implemented V32LE.
...
Resource.RcvBuffer.rowsPerFrame = 4096*128*4;   % this size allows for maximum range
Resource.RcvBuffer.colsPerFrame = Resource.System.receiveChannels;
Resource.RcvBuffer.numFrames = 10;
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer.numFrames = 50;
Resource.DisplayWindow.Title = 'L22-14vLF_128RyLns_32LE';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'Delay', zeros(1,Resource.System.transmitChannels)), 1, P.numRays);
...
for n = 1:P.numRays   % P.numRays transmit events
    ...
    TX(n).aperture = min(lft, 65); % use precomputed apertures
    TX(n).Apod((lft:rt)-TX(n).aperture+1) = 1.0;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:4:4*P.numRays
        Receive(k+j).Apod(1:32) = 1;
        Receive(k+j).aperture = 1; % use precomputed apertures
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;      % four acquisitions per frame
        Receive(k+j+1).Apod(33:64) = 1;
        Receive(k+j+1).aperture = 1;
        Receive(k+j+1).framenum = i;
        Receive(k+j+1).acqNum = j+1;  % four acquisitions per frame
        Receive(k+j+2).Apod(1:32) = 1;
        Receive(k+j+2).aperture = 65;
        Receive(k+j+2).framenum = i;
        Receive(k+j+2).acqNum = j+2;      % four acquisitions per frame
        Receive(k+j+3).Apod(33:64) = 1;
        Receive(k+j+3).aperture = 65;
        Receive(k+j+3).framenum = i;
        Receive(k+j+3).acqNum = j+3;  % four acquisitions per frame
    end
end
...
Recon = struct('senscutoff', 0.5, ...
               ...
               'IntBufDest', [1,1], ...
               'ImgBufDest', [1,-1], ...
               'RINums', 1:(4*P.numRays));

% Define ReconInfo structures.
ReconInfo = repmat(struct('mode','accumIQ', ...  % replace IQ data
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 0), 1, 4*P.numRays);
% - Set specific ReconInfo attributes.
for j = 1:4:4*P.numRays
    ReconInfo(j).mode = 'replaceIQ';
    ReconInfo(j).txnum = (j+3)/4;
    ReconInfo(j).rcvnum = j;
    ReconInfo(j).regionnum = (j+3)/4;
    ReconInfo(j+1).txnum = (j+3)/4;
    ReconInfo(j+1).rcvnum = j+1;
    ReconInfo(j+1).regionnum = (j+3)/4;
    ReconInfo(j+2).txnum = (j+3)/4;
    ReconInfo(j+2).rcvnum = j+2;
    ReconInfo(j+2).regionnum = (j+3)/4;
    ReconInfo(j+3).mode = 'accumIQ_replaceIntensity'; % accum and detect
    ReconInfo(j+3).txnum = (j+3)/4;
    ReconInfo(j+3).rcvnum = j+3;
    ReconInfo(j+3).regionnum = (j+3)/4;
end
...
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:4:4*P.numRays                      % Acquire frame
        ...
        Event(n).tx = (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+j+1;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 1;
        n = n+1;
        ...
        Event(n).tx = (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+j+2;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 1;
        n = n+1;
        ...
        Event(n).tx = (j+3)/4;
        Event(n).rcv = 4*P.numRays*(i-1)+j+3;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = 1;
        n = n+1;
...
save('MatFiles/L22-14vLF_128RyLns_32LE');
% filename = ('L22-14vLF_128RyLns'); % VSX    % permits immediately running VSX without specifying the matfile name
return
