...
%   for its use
%
% File name: SetUpC5_2gH_128RyLnsPolar.m - Example of curved array imaging with
%            focused transmits and polar coordinates PData
% Description:
%   Sequence programming file for C5-2gH curved array using 128 ray lines
...
P.numTx = 14;     % no. of elements in transmit aperture
P.txFocus = 100;  % focal point in wavelengths
P.startDepth = 0; % startDepth and endDepth in wavelengths
P.endDepth = 220;
...
PData(1).Coord = 'polar';
PData(1).PDelta = [dtheta, 0.5, 0]; % [dtheta,dr,dz]
sizeRows = 10 + ceil((P.endDepth + radius)/PData(1).PDelta(2));
sizeCols = ceil(scanangle/dtheta + 1);
PData(1).Size = [sizeRows,sizeCols,1];     % size, origin and pdelta set region of interest.
PData(1).Origin = [0,0,-radius];   % Origin is in rectangular coords.
% Define PData Regions for numRays scanlines
for j = 1:P.numRays
    PData(1).Region(j) = struct('Shape',struct( ...
                       'Name','Sector',...
                       'Position',[0,0,-radius],...
...
Resource.RcvBuffer.rowsPerFrame = P.numRays*4096;    % 128 segments of 4096 samples
...
Resource.DisplayWindow(1).Title = 'C5-2gH_128RyLnsPolar';
...
RefPtY = ceil(radius * cos(scanangle/2)) - radius - 5;
DwHeight = ceil((P.endDepth-RefPtY)/Resource.DisplayWindow(1).pdelta);
Resource.DisplayWindow(1).Position = [250,(ScrnSize(4)-(DwHeight+150))/2, ...  % lower left corner position
                                      DwWidth, DwHeight];
Resource.DisplayWindow(1).ReferencePt = [-DwWidth/2*Resource.DisplayWindow(1).pdelta,0, ...
                                         RefPtY];   % 2D imaging is in the X,Z plane
...
save('MatFiles/C5-2gH_128RyLnsPolar');
...
% filename = 'C5-2gH_128RyLnsPolar';  VSX;
...
function RangeChangeCallback(hObject, ~, UIValue)
    ...
    evalin('base','DwHeight = ceil((P.endDepth-RefPtY)/Resource.DisplayWindow(1).pdelta);');
