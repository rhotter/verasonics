...
%   for its use
%
% File name: SetUpL39_21gD128RyLns.m - Example of scanline imaging with 
...
Resource.DisplayWindow.Title = 'L39-21gD 128RyLns interleaved sampling';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:2:2*P.numRays                 % Acquire all ray lines for frame
        Event(n).info = 'Acquire ray line, first interleave';
        Event(n).tx = j;
        Event(n).rcv = k+j;
        Event(n).recon = 0;
        Event(n).process = 0;
        ...
        Event(n).info = 'Acquire ray line, 2nd interleave samples';
...
save('MatFiles/L39-21gD128RyLns');
% filename = ('L39-21gD128RyLns'); % VSX    % permits immediately running VSX without specifying the matfile name
return
