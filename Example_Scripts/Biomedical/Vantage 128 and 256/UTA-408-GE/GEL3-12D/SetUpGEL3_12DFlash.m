...
%   for its use
%
% File name: SetUpGEL3_12DFlash  -  This script is for initial
% development and integration of the GE L3-12D HVMux linear array, with
% the following characteristics from the GE data sheet:
% Center frequency 6.5 MHz  Bandwidth 85%
% 256 elements multiplexed to 128 system channels
% pitch 0.200 mm (element spacing 0.8 wavelengths)
% Eleveation focus 5.0 mm
% HV Mux power supplies +/-100 V. HV; +3.8 V. HVMux logic rail (regulated
% down on the UTA adapter); 10 MHz clock rate
%
% Description:
%   Sequence programming file for GEL3-12D Linear array, using 3-1
...
Resource.DisplayWindow(1).Title = 'GEL3-12DFlash';
...
tstAp = [1 65 129]; % modify as desired for test; nominal state is [1 33 65]
TX(1).aperture = tstAp(1);
TX(2).aperture = tstAp(2);  % Use the tx aperture that starts at element 65.
TX(3).aperture = tstAp(3); % Use the tx aperture that starts at element 129.
...
for i = 1:Resource.RcvBuffer(1).numFrames  % 3 acquisitions per frame
    ...
    Receive(3*i-2).aperture = TX(1).aperture;
    ...
    Receive(3*i-1).aperture = TX(2).aperture;
    ...
    Receive(3*i).aperture = TX(3).aperture;
