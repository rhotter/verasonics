...
%   for its use.
%
% File name: SetUpGEIC5_9DWideBeamPolar.m - Example of curved array imaging with
...
P.endDepth = 100*mm2wl;  % endDepth (10 cm)
P.numTx = 96;    % no. of elements in TX aperture (-1).
P.numRays = 96; % no. of rays in frame
P.txFocus = 2*P.endDepth; % focus in wavelengths
P.maxDepth = 160*mm2wl;   % maxDepth for RangeChange and RcvBuffer
...
Trans = computeTrans(Trans); % GEC5-9D transducer is 'known' transducer so we can use computeTrans.
radius = Trans.radius;
...
Resource.RcvBuffer.rowsPerFrame = P.numRays*4096; % allows for max range
...
Resource.DisplayWindow(1).Title = 'GEIC5-9DWideBeamPolar';
...
UI(3).Control = {'UserB4','Style','VsSlider','Label',['TX Focus (',AxesUnit,')'],...
                 'SliderMinMaxVal',[100,600,P.txFocus],'SliderStep',[10/500,20/500],'ValueFormat','%3.0f'};
...
save('MatFiles/GEIC5-9DWideBeamPolar');
...
% filename = 'GEIC5-9DWideBeamPolar';  VSX;
