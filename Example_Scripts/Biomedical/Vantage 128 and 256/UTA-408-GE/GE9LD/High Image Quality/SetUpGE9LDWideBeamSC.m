...
%    its use.
%
% File Name: SetUpGE9LDvWideBeamSC.m - Wide beam, spatial compounding example
%
% Description: The 192 element GE9LD Linear array is used to acquire frames using wide beam transmits.
...
%    The receive data from each wide beam are stored under different acqNums in the Receive buffer.
%    The reconstruction sums the IQ data from the aquisitions for all beams that contribute to a
%    pixel and computes intensity values to produce the full frame.
%
...
%
% Last update:
%    10/04/2017 - tested with SW 3.3

clear all

% key parameters
P.numTx = 58; % Number of Transmit Elements in the aperture (should be even).
P.numRays = 50;  % Number of wide beams in a frame.
P.txFocusMm = 100;   % transmit focal pt in wavelengths
P.dtheta = 8*(pi/180);  % angle delta for beams steered left or right
P.startDepthMm = 0.5;  % startDepth in mm
P.endDepthMm = 50;  % endDepth in mm
P.maxDepthMm = 120;  % maxDepth for RangeChange and RcvBuffer
...
Trans = computeTrans(Trans);  % GE9LD transducer is 'known' transducer so we can use computeTrans.
...
Resource.RcvBuffer(1).numFrames = 6;
...
Resource.RcvBuffer(2).numFrames = 6;  % all RcvBuffers should have same no. of frames
...
Resource.RcvBuffer(3).numFrames = 6;
Resource.InterBuffer(1).datatype = 'complex';
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).datatype = 'double';
Resource.ImageBuffer(1).numFrames = 6;
Resource.DisplayWindow.Title = 'GE9LWideBeamSC';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'peakCutOff', 0.5, ...
                   'peakBLMax', 5.0, ...
                   'Delay', zeros(1,Trans.numelements)), 1, 3*P.numRays);

% - Set event specific TX attributes.
for n = 1:P.numRays
    TX(n).Origin(1) = TxOrgX(n);        %origin
...
Process(1).Parameters = {'imgbufnum',1,...   % number of buffer to process.
                         ...
                         'norm',1,...        % normalization method(1 means fixed) added this in
...
SeqControl(2).argument = 120;  % 120 usec
SeqControl(3).command = 'timeToNextAcq';
SeqControl(3).argument = 3000; %3000 usec = 1msec time between frames
...
function RangeChangeCallback(hObject, ~, UIValue)
    ...
    for n = 1:P.numRays
        PData.Region(n).Shape.height = P.endDepth-P.startDepth;
    end
    m = P.numRays;
    for n = 1:P.numRays
        if n<=8
    ...
    Control = evalin('base','Control');
    Control.Command = 'update&Run';
    Control.Parameters = {'PData','InterBuffer','ImageBuffer','DisplayWindow','TX','Receive','TGC','Recon','TGC'};
    assignin('base','Control', Control);
