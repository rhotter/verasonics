...
%   for its use.
%
% File name: SetUpGEM5ScD_WideBeamPolarHI.m - Example of phased array imaging with wide beam
%                transmits and harmonic imaging.
%
% Description:
%   Sequence programming file for GEM5ScD phased array using virtual apex format.  P.numRays scan lines
%   are programmed with two acquisition pairs per scan line. The first acquisition pair has all 180
%   transmit and receive channels active and the acquisitions acquire to P.endDepth with alternate
%   polarity transmits.  The second acquisition pair uses only the center row of elements and
%   overwrites the first acquisition's data down to P.midDepth using alternate transmit polarities.
...
P.txFocus = -700; % specify focus in wavelengths.
P.txMidFocus = -700; % specify focus in wavelengths.
...
% - specify P.numRays+1 Region structures.
PData(1).Region = repmat(struct('Shape',struct( ...
                    'Name','SectorFT',...
                    'Position',[0,0,-dapex],...
                    'z',P.startDepth,...
                    'r',dapex+P.endDepth+10,...
                    'angle',rayDelta*8,...
                    'steer',0, ...
...
for j = 1:P.numRays
    PData(1).Region(j+1).Shape.steer = Angle(j);
end
...
Resource.DisplayWindow(1).Title = 'GEM5ScD_WideBeamPolar';
...
for n = 1:4:4*P.numRays
    TX(n).Origin = [TXorgs((n+3)/4), 0.0, 0.0];
    ce = round(80*(TXorgs((n+3)/4) - Trans.ElementPos(1,1))/aperture);
    TApod(ce) = W(m/2);
    for j=1:80
        lft=ce-j;
    ...
    % 2nd Pair of transmits
    TX(n+2) = TX(n);
    TX(n+2).Apod(81:160) = 0; % turn off outer elements
    TX(n+2).focus = P.txMidFocus;
    TX(n+2).Delay = computeTXDelays(TX(n+2));
    TX(n+2).TXPD = computeTXPD(TX(n+2),PData);
    TX(n+3).waveform = 2;
    TX(n+3).Origin = TX(n+2).Origin;
    TX(n+3).Apod = TX(n+2).Apod;
    TX(n+3).Steer = TX(n+2).Steer;
    TX(n+3).Delay = TX(n+2).Delay;
    TX(n+3).TXPD = TX(n+2).TXPD;
end
...
Receive = repmat(struct('Apod', ones(1,160), ...
                        ...
                        'framenum', 1, ...
                        'acqNum', 1, ...
                        'InputFilter',[-0.00076 +0.00150 -0.00049 +0.00061 -0.00528 +0.00873 -0.00443 ...
                                       +0.00333 -0.01752 +0.02859 -0.01508 +0.00616 -0.03824 +0.06732 ...
                                       -0.03085 -0.00842 -0.05963 +0.15182 -0.04321 -0.25964 +0.43097], ...
                        'sampleMode', 'NS200BW', ...
                        'mode', 0, ...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:4:4*P.numRays
        % First acquisition pair
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = (j+1)/2;
        Receive(k+j+1).mode = 1;
        Receive(k+j+1).framenum = i;
        Receive(k+j+1).acqNum = (j+1)/2;
        % 2nd acquisition pair
        Receive(k+j+2).Apod(1:80) = 2.1;
        Receive(k+j+2).Apod(81:160) = 0;
        Receive(k+j+2).framenum = i;
        Receive(k+j+2).acqNum = (j+3)/2;
        Receive(k+j+2).endDepth = P.midDepth;
        Receive(k+j+3).mode = 1;
        Receive(k+j+3).Apod(1:80) = 2.1;
        Receive(k+j+3).Apod(81:160) = 0;
        Receive(k+j+3).framenum = i;
        Receive(k+j+3).acqNum = (j+3)/2;
        Receive(k+j+3).endDepth = P.midDepth;
    end
end
...
for j = 1:P.numRays
    ReconInfo(j).txnum = 4*(j-1)+1;
    ReconInfo(j).rcvnum = 4*(j-1)+1;
...
SeqControl(5).command = 'timeToNextAcq'; % time between rays
SeqControl(5).argument = 180;
nsc = 6;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    k = 4*P.numRays*(i-1);
    for j = 1:4:4*P.numRays
        Event(n).info = 'Acquire endDepth ray line pair';
        ...
        n = n+1;
        Event(n).tx = j+1;       % use 2nd TX structure.
        Event(n).rcv = k+j+1;
        Event(n).recon = 0;      % no reconstruction.
        Event(n).process = 0;    % no processing
        Event(n).seqControl = 2;
        n = n+1;
        Event(n).info = 'Acquire midDepth ray line pair';
        Event(n).tx = j+2;
        Event(n).rcv = k+j+2;
        Event(n).recon = 0;      % no reconstruction.
        Event(n).process = 0;    % no processing
...
UI(3).Control = {'UserB4','Style','VsSlider','Label',['TX Focus (',AxesUnit,')'],...
                 'SliderMinMaxVal',[-800,-400,P.txFocus]*wls2mm,'SliderStep',[0.1,0.2],'ValueFormat','%3.0f'};
...
save('MatFiles/GEM5ScD_WideBeamPolarHI');
return
...
% - Define a SectorFT region that will intersect with the x,z plane to form a trapezoid region.
PData.Region = repmat(struct(...
            'Shape',struct('Name','SectorFT', ...
            'Position',[0,0,-dapex], ...
            'z',P.startDepth, ...
            'r',P.endDepth+dapex+10, ...
            'angle',rayDelta*8, ...
            'steer',0, ...
            'andWithPrev',1)),1,P.numRays+1);
% First Region covers entire field of view
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:4:4*P.numRays
        % First acquisition pair
        Receive(k+j).endDepth = maxAcqLength;
        Receive(k+j+1).endDepth = maxAcqLength;
        % 2nd acquisition pair
        Receive(k+j+2).endDepth = P.midDepth;
        Receive(k+j+3).endDepth = P.midDepth;
    end
