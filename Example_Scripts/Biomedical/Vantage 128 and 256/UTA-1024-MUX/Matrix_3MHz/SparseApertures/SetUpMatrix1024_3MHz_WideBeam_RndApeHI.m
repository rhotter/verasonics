%  This script does WideBeam Harmonic Imaigng using Random apertures. Each
%  Random Apertures uses that use 256 elements for each transmit. The receive
%  is done with four subapertues (1-256, 257-512, 513-768, 769-1024).
%  Therefore every transmit needs to be repeated four times so that it can
%  be received with each subapertures.
%
...
%  The script does two consecutive transmits for each WideBeam using
%  inverse polarity in each of the transmits.
...
%  Taking into accound that the TTNA1 is 0.25 ms, which is used for 200 Tx/Rcv
%  (or 400 if the user does 2-time accumulation) of 100 Wide Beams (100 for
%  the normal transmit and 100 for the pulse inversion) and the 4 synthetic
%  receives. Therefore the Tx/Rcv for 1 frame takes 400 ms.
%  Then the TTNA2 (pause between frames) is set to 20 ms (this allow the time
%  to do the transfer to host). The total period for 1 acquisition is 420 ms.
...
%  case for this scrip the number of angles should be larger than 8
...
NumRndApe=P.numRaysX*P.numRaysY; %
...
Resource.VDAS.dmaTimeout=2000; %required if the user wants to accumulate more than just 2 acquisitions
...
Resource.InterBuffer(1).numFrames = P.numFrames;
...
TW(1).Parameters = [2,.67,2,1];  % A, B, C, D
TW(2).type = 'parametric';
TW(2).Parameters = [2,.67,2,-1];  % A, B, C, D
...
%         TX(i).TXPD = computeTXPD(TX(i),PData(1));
    ...
    TX(i+NumRndApe).waveform = 2;
    TX(i+NumRndApe).Steer = [AZ(i) EL(i) 0];
    TX(i+NumRndApe).Apod = Papod(i+P.numSyntheticRcvs, :);
    TX(i+NumRndApe).aperture = Paper(i+P.numSyntheticRcvs); %Since the synthetic receive is 4, then the rnd apertures start at Paper(5)
    TX(i+NumRndApe).Delay = computeTXDelays(TX(i));
%         TX(i+NumRndApe).TXPD = computeTXPD(TX(i),PData(1));
...
TGC(1).CntrlPts = [0 824.4475 866.8182 1023 1023 1023 1023 1023];
TGC(1).rangeMax = 128;
...
Receive = repmat(struct(...
    ...
    'sampleMode', 'NS200BW', ...
    'demodFrequency', 4.0, ...
    'LowPassCoef', [+0.01100 +0.00720 -0.00420 -0.02020 -0.03340 -0.03370...
                            -0.01280 +0.03110 +0.09140 +0.15380 +0.20080 +0.21830],...
    'InputFilter', [+0.00098 +0.00000 -0.00302 +0.00000 +0.00287 +0.00000 +0.00439 ...
                            +0.00000 -0.01904 +0.00000 +0.02893 +0.00000 -0.01169 +0.00000 ...
                             -0.04895 +0.00000 +0.14301 +0.00000 -0.23132 +0.00000 +0.26770],...
    'callMediaFunc', 0), 1, 2*NumRndApe*P.numFrames*P.numSyntheticRcvs);
...
for j = 1:P.numFrames
    ...
    k= 2*NumRndApe*P.numSyntheticRcvs*(j-1);
    for i = 1:P.numSyntheticRcvs*NumRndApe
        ...
        Receive(i+k+(P.numSyntheticRcvs*NumRndApe)).acqNum = i;
        Receive(i+k+(P.numSyntheticRcvs*NumRndApe)).framenum = j;
        Receive(i+k+(P.numSyntheticRcvs*NumRndApe)).aperture = tt;
        Receive(i+k+(P.numSyntheticRcvs*NumRndApe)).Apod = Papod(tt,:);
        Receive(i+k+(P.numSyntheticRcvs*NumRndApe)).mode = 1;
...
for w=1:NumRndApe
    ...
    for i = 1:P.numSyntheticRcvs
        ...
        ReconInfo(i+k).regionnum =  NumRndApe+4;% w to recon every region or  NumRndApe+4 to recon only the slices
    end
...
for j = 1:Resource.RcvBuffer(1).numFrames
    v=P.numSyntheticRcvs*2*NumRndApe*(j-1);
    for w=1:NumRndApe
        k= P.numSyntheticRcvs*(w-1);
        for i = 1:P.numSyntheticRcvs
            Event(n).info = 'Transmit/Receive Normal transmit';
            Event(n).tx = w;   % Normal transmit.
            Event(n).rcv = i+k+v;  % use 1st Rcv structure of frame.
            Event(n).recon = 0;      % no reconstruction.
            Event(n).process = 0;    % no processing
            Event(n).seqControl = 2; %
            n = n+1;

            Event(n).info = 'Transmit/Receive Pulse inverted';
            Event(n).tx = w+NumRndApe;   % Pulse inverted transmit.
            Event(n).rcv = i+k+v+P.numSyntheticRcvs*NumRndApe;  % use 1st Rcv structure of frame.
            Event(n).recon = 0;      % no reconstruction.
            Event(n).process = 0;    % no processing
            Event(n).seqControl = 2; %
            n = n+1;

            %Third and 4th TX and Rcv for accumulation. This two events are
            %to increase the harmonic signal
            Event(n).info = 'Transmit/Receive Pulse inverted';
            Event(n).tx = w;   % use 1st TX structure.
            Event(n).rcv = i+k+v+P.numSyntheticRcvs*NumRndApe;  % use 1st Rcv structure of frame.
            Event(n).recon = 0;      % no reconstruction.
            Event(n).process = 0;    % no processing
            Event(n).seqControl = 2; %
            ...
            Event(n).info = 'Transmit/Receive Pulse inverted';
            Event(n).tx = w+NumRndApe;   % use 1st TX structure.
            Event(n).rcv = i+k+v+P.numSyntheticRcvs*NumRndApe;  % use 1st Rcv structure of frame.
            Event(n).recon = 0;      % no reconstruction.
            Event(n).process = 0;    % no processing
            Event(n).seqControl = 2; %
...
save('MatFiles/Matrix1024-3MHz-WideBeam-RndApeHI.mat','-v7.3'); %filename = 'MatFiles/Matrix1024-8FlashDM.mat'; %VSX;
