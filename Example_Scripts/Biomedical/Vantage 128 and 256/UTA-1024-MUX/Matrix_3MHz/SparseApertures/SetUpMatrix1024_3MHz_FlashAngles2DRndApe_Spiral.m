%  This script does Flash multiAngles using Random Apertures and a spiral
%  pattern going from wider angles in the first transmits to a diverging
%  wave with no steering. The Random apertures use 256 elements of the
%  full apertures (1024) for each transmit. The receive is done with four 
%  subapertures (1-256, 257-512, 513-768, 769-1024). Therefore every
%  transmit needs to be repeated four times so that it can be received with
%  each subapertures.
%
%  A set of 100 Random Apertures taking into account the constrains of the
%  MUX switches have been created for this script which are loaded from a
%  .mat file "Saved100RndApod". However, the user is free to create new
%  sets of Random Apertures using the function "Rnd1024Aperture" for as
%  many apertures as needed.
%
%  For this script the number of angles used is defined by the variable
%  "na". The script calculates the steering of the "na" transmits from 18
%  degrees to 0 degrees (divering wave)angles as the planes revolve around
%  the z axis. In this case the spiral consists of 4*pi (two revolutions).
%  In this example 10 steps were chosen to go around the 4*pi. Broader
%  angles are chosen for the fist revolution and the angles quickly fall
%  to zero (no steering) in the second revolution.
%
...
%  slices (Region = 4) was estimated around 400 ms given the host computer
%  that was used at the time of the testing. Taking into accound
%  that the TTNA1 is 0.25 ms, which is used for the 10 angles and the 4
%  synthetic receives, the Tx/Rcv for 1 frame takes 10 ms. Then the TTNA2
%  (pause between frames) is set to 20 ms (this allow the time to do the
%  transfer to host). The total period for 1 acquisition is 30 ms.
%  Therefore the number of frames acquired x Period needs to be larger than
%  the time it takes to do 1 reconstruction (400 ms). In this particular
%  case for this scrip the number of angles should be larger than 14
...
%  the first reconstruction usually takes a significant amount of time due
%  to the initialization of the system.
...
% Media.MP(2,:) = [30,0,90,1.0];
% Media.MP(3,:) = [0,30,90,1.0];
% Media.MP(4,:) = [30,30,90,1.0];
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*na*na*P.numSyntheticRcvs;
...
rotang=linspace(0,4*pi,na); %

r=min(abs([Trans.ElementPosWL(1,1) Trans.ElementPosWL(1,2)])); % In WL

R=linspace(r,3*r/4,round(na/2));
R=[R(1:end-1) linspace(3*r/4,r/4,na-round(na/2)) 0]; % The zero at the end adds an extra value for the for AZ and EL to make a non steer diverging wave

AZ= atan(R.*cos(rotang)/zApex);
...
save('MatFiles/Matrix1024-3MHz-FlashAngles2DRndApe-Spiral.mat'); %filename = 'MatFiles/Matrix1024-8FullFlashDM.mat'; %VSX;
