%
% File name: SetUpMatrix1024_3MHz_FlashPerChWvfm - Example demonstrating
%   the use of per-element TW waveform definitions with the UTA 1024-Mux.
%
% Description:
%  The TW waveform is defined as an array of the 'parametric' waveform
%  type, one for each of the 1024 elements.  A Hamming window function is
%  appied to the relative pulse width in each waveform across each row of
%  the 32 X 32 transducer element array.  In all other aspects this
%  sequence is identical to SetUpMatrix1024_3MHz_Flash.
%
% Notice:
...
P.curved = 1; % set to 0 for flat or 1 for defocused at the reconstruction apex

P.numFrames = 2;
P.numTx = 1;
P.numSyntheticTx = 4; % divide 1024 elements into four 256 element subgroups
P.numSyntheticRcvs = 4; % ditto
...
TW.type = 'parametric';
TW.Parameters = repmat([Trans.frequency,1,2,1], 32, 1);  % one row of 32 elements
TW.Parameters(:, 2) = hamming(32); % apply window function
TW.Parameters = repmat(TW.Parameters, 32, 1); % apply to all 32 rows
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'peakCutOff', 2,...
                   'peakBLMax', 20,...
                   'aperture',1), 1,P.numTx*P.numSyntheticTx);

for m = 1:P.numTx
    for i=1:P.numSyntheticTx
        TX(i+(m-1)*P.numSyntheticTx).Apod = Papod(i, :);
        TX(i+(m-1)*P.numSyntheticTx).Delay = computeTXDelays(TX(i+(m-1)*P.numSyntheticTx));
        TX(i+(m-1)*P.numSyntheticTx).aperture = Paper(i);
%         TX(i+(m-1)*P.numSyntheticTx).TXPD = computeTXPD(TX(i+(m-1)*P.numSyntheticTx),PData(1));
    end
end

% uncomment these lines to enable an oddball waveform on one element, and
% test functionality with transmit enabled on only that element
% testEL = 537;
% TX(1).Apod = zeros(1, 1024);
% TX(2).Apod = zeros(1, 1024);
% TX(3).Apod = zeros(1, 1024);
% TX(4).Apod = zeros(1, 1024);
% TW.Parameters(testEL, :) = [Trans.frequency, 0.67, 12, 1];
% TX(3).Apod(testEL) = 1;
...
for j = 1:P.numFrames
    for i = 1:P.numSyntheticTx*P.numSyntheticRcvs*P.numTx
        ...
        Receive(i+(j-1)*P.numSyntheticTx*P.numSyntheticRcvs*P.numTx).Apod = Papod(Papodindex, :);
        Receive(i+(j-1)*P.numSyntheticTx*P.numSyntheticRcvs*P.numTx).aperture = Paper(Papodindex);
    end
end
...
save('MatFiles/Matrix1024-3MHz-Flash.mat'); %filename = 'MatFiles/Matrix1024-8FlashDM.mat'; %VSX;
