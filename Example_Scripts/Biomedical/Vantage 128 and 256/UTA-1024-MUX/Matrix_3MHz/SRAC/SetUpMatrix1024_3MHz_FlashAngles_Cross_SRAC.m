% SetUpMatrix1024_3MHz_FlashAngles_Cross_SRAC.m - Example of FlashAngles 
...
%  volume recontruction. The scanning pattern in this script follow a cross,
%  therefore 5 beams are transmited by tilting the plane in the XZ direction
%  and 5 beams in the YZ direction. The steering angles are determine in a
%  variable names TxAngles in degrees.
...
CompRndApod=CompRndApod(1:2*na,:,:);
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*na*2*P.numSyntheticRcvs;
...
AZ=[txAngles zeros(1,length(txAngles))];
EL=[zeros(1,length(txAngles)) txAngles ];
...
save('MatFiles/Matrix1024-3MHz-FlashAngles_Cross_SRAC.mat');
return
