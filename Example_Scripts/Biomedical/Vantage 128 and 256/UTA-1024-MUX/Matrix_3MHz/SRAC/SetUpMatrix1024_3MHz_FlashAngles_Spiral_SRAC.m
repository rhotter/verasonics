% SetUpMatrix1024_3MHz_FlashAngles_Spiral_SRAC.m - Example of FlashAngles 
% imaging for volume acquisitions using a matrix probe array (3 MHz) and
...
%
% Description:
%  This scripts transmits and receives 10 steer waves for compounding and
%  volume recontruction. The scanning patern in this script follow spiral
%  around the z axis. The spiral patter rotates twice around the z axis in
%  10 steps (the number of steps can be modified by the user). Therefore in 
%  this case, the steering is not done orthogonal to one plane (XZ or YZ)
%  but to both planes. The parameter "na" allows the user change the number
%  of angles for the script. 
...
%  then the script will transmit the same waves with different angles using 
%  the same Random aperture, however the recieve of the backscatter data will
%  be done with different random apertures. The random apertures used in this
%  script are complementary to each other (meaning that within a set of 
%  apertures, 1-4, no element is repeated). The script loads a file named 
%  "saved100CompRndApod" which has 100 sets with 4 complementary random 
%  apertures each. 
...
ReconRegion=1; %Set ReconRegion=5 to reconstruct only 3 orthogonal planes, or ReconRegion =1 to reconstruct the whole volume
...
CompRndApod=CompRndApod(1:na,:,:);


% create aperture index for each Papod
for i = 1:size(CompRndApod, 1)
    for j=1:P.numSyntheticRcvs
    Paper(i,j) = computeMuxAperture(squeeze(CompRndApod(i,j,:))', Trans);
    end
end
...
Resource.DisplayWindow(3).ReferencePt = [PData(1).Origin(1),-PData(1).Origin(2),XYDisp];%PData.Region(end).Shape.oPAIntersect];
...
for n=1:na
    ...
    TX(n).Apod = squeeze(CompRndApod(n,1,:))';
    TX(n).aperture = Paper(n,1);
...
for j = 1:P.numFrames
    ...
    for i = 1:na
        ...
        for w=1:P.numSyntheticRcvs
            ...
            Receive(w+kk+k).aperture = Paper(i,w);
            Receive(w+kk+k).Apod = squeeze(CompRndApod(i,w,:))';
        end
    end
end
...
for w=1:na
    ...
    for i = 1:P.numSyntheticRcvs
        ...
        ReconInfo(i+k).regionnum = ReconRegion; %1 for the whole volume, 4 for the slices
    end
...
save('MatFiles/Matrix1024-3MHz-FlashAngles_Spiral_SRAC.mat');
return
