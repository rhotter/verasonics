...
%   for its use
%
% File name: SetUpL11_5vAcquireRFSigSpctrmViewer.m - Example of RF data acquisition
%
% Description:
...
%   processing is used asynchronous with respect to acquisition. Acquired
%   RF data is displayed using the Verasonics Signal Spectrum Viewer 
%   (vsv.dsp.apps.SignalSpectrumViewer).
%
% Last update:
% 3/31/2021 - Created to show how the Signal Spectrum Viewer can be used to
%             show live RF data
%
...
% Initialize Viewer and Rcv Data Builder Classes

viewerModel = vsv.dsp.apps.models.SingleMapModel();
viewer = vsv.dsp.apps.SignalSpectrumViewer(viewerModel);
rcvBuilder = vsv.data.RcvDataBuilder();
...
save('MatFiles/L11-5vAcquireRFSigSpctrmViewer');


%% **** Callback routines used by External function definition (EF) ****

function myProcFunction(RData)
% Display RF Data using the Signal Spectrum Viewer app
%
    % Get needed components from base workspace if empty
    rcvBuilder = vsv.seq.getBaseComp('rcvBuilder');
    Receive    = vsv.seq.getBaseComp('Receive');
    Trans      = vsv.seq.getBaseComp('Trans');
    TransConnector = Trans.Connector;
    
    viewerModel = vsv.seq.getBaseComp('viewerModel');
    viewer      = vsv.seq.getBaseComp('viewer');
    signal      = vsv.seq.getBaseComp('signal');
    
    
    % Create and update the RF Data signal
    if isempty(signal)
        
        RFDataAdvcd = rcvBuilder.createRcvData({RData}, Receive, TransConnector);
        signal = RFDataAdvcd{1};
        
        viewerModel.Signal  = signal;
        % Show the RF Data in the Signal Spectrum Viewer
        viewer.showSignal(signal);
        
        % prevent figure to come to front all the time to prevent that the
        % focus is changing which would prevent other interactions
        viewer.SignalApp.DoBringFigToFront = false; 
        assignin('base', 'signal', signal );
        
    else
        
        viewerModel.RawData = RData;
        
        viewer.SignalApp.Controller.PlotController.updateSignalPlot();
        viewer.showSpectrum();
    end
