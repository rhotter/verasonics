...
%   for its use.
%
% File name: SetUpP4_2vFlashPolar.m - Example of phased array imaging with
%                                flash transmit scan
% Description:
...
%   the scanhead connector with elements 1-64 connected to inputs 33-96. We
...
%
%   PData is in polar coordinates.
%
% Last update:
% 9/6/2016 - first working version

clear all

P.startDepth = 0;
P.endDepth = 180;   % Acquisition depth in wavelengths
...
PData(1).Coord = 'polar';
PData(1).PDelta = [P.rayDelta/127,0.5,0]; % [dtheta,dr,dz]
PData(1).Origin = [0,0,-P.radius];
PData(1).Size(1) = ceil((P.endDepth - PData(1).Origin(3))/PData(1).PDelta(2)) + 10; % rows
PData(1).Size(2) = ceil(P.rayDelta/PData(1).PDelta(1) + 1); % cols
PData(1).Size(3) = 1;
...
Resource.DisplayWindow(1).Title = 'P4-2vFlashPolar';
