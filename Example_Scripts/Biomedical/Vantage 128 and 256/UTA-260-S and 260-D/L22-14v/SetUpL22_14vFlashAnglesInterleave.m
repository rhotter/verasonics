...
%
% Description:
%   Sequence programming file for L22-14v Linear array, using plane wave transmits
%   with multiple steering angles using 4X sampling. All 128 transmit and receive
%   channels are active for each acquisition. This script demonstrates the
%   "interleaved sampling" mechanism for RF data acquisition.  In this script, the
%   L22-14v is operated at a nominal center frequency for Recon processing of 20.83
%   MHz, with 4X sampling so the required RF data acquisition sample rate is 83.3
%   MHz. Since the HW system cannot acquire data at sample rates above 62.5 MHz, we
%   acquire two sets of receive data for each line, at an A/D sample rate of 41.7
%   MHz. These pairs of acquisition data are then interleaved in the reconstruction
%   processing function, to produce a single composite line sampled at 83.3 MHz as
%   expected by Recon.  The data acquired in the second acquistion of each pair must
%   have its data shifted by half a sample period, so it can be interleaved with the
%   first line.  This is accomplished by adding an offset to the transmit delays for
%   the first acquisition equal to that half-sample interval; this in effect shifts
%   receive sampling earlier by the desired one half sample period. Other than
%   the interleaved acquisition at 83.3 MHz, this script is identical to
%   L22-14vFlashAngles which uses acquisition at 62.5 MHz with no interleave.
%
% Last update:
% 2/1/2019 - updated for use with SW 4.0.1
...
Receive = repmat(struct('Apod', ones(1,Trans.numelements), ...
                        ...
                        'framenum', 1, ...
                        'acqNum', 1, ...
                        'sampleMode', 'NS200BWI', ... % Specify interleaved acquisition
