...
%   for its use
%
% File name: SetUpL22_14vXFlashDopplerLF.m - Example of plane wave Doppler imaging for Low Flow conditions
%                                           using longer ensembles and special wall filters.
% Description:
%   Sequence programming file for L22-14vX Linear array, using plane wave
%   transmits for 2D (B-mode) and Doppler imaging.
%   - for the 2D (B-mode) imaging, na steered angles are used.
%   - for the Doppler ensemble, ne pulses with long ensemble lengths are
...
%     13.9 MHz -3 dB bandwidth to support the 12 MHz bandwidth of the
%     L22-14vX (18 MHz center frequency, 67% bandwidth).
%
%
% Last update:
% 12/13/2017 - modified for SW 3.4.1
...
if (na > 1)
    dtheta2D = (36*pi/180)/(na-1);
    startAngle = -36*pi/180/2;
else
    dtheta2D = 0;
    startAngle=0;
end % set dtheta2D to range over +/- 18 degrees.
...
disp([mfilename,':EnsembleLength = ''',EnsembleLength,''''])
...
Resource.RcvBuffer(1).numFrames = 20;           % 25 frames allocated for RF acqusitions.
...
Resource.DisplayWindow(1).Title = 'L22-14vX FlashDopplerLF, 4X sampling at 62.5 MHz';
Resource.DisplayWindow(1).pdelta = 0.375;
...
TPC(1).name = '2D';
TPC(1).maxHighVoltage = 60;
TPC(2).name = 'Doppler';
TPC(2).maxHighVoltage = 45;
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:na  % acquisitions for 2D
        ...
        Receive(j+k).VDASvgaLowGain = 0;
    end
    for j = (na+1):(na+ne);
        % Doppler acquisition
        Receive(j+k).Apod = zeros(1,Trans.numelements);
...
% - 2D TGC
TGC(1).CntrlPts = [0,511,716,920,1023,1023,1023,1023]; %[400,500,625,700,750,800,850,950];
TGC(1).rangeMax = P(1).endDepth;
TGC(1).Waveform = computeTGCWaveform(TGC(1));
% - Doppler TGC
TGC(2).CntrlPts = [0,511,716,920,1023,1023,1023,1023]; %[500,725,775,800,800,800,800,800];
...
Process(2).Parameters = {'IntBufSrc',[2,1],...          % number of Interbuffer to process.
                         ...
                         'wallFilter',CDIFilterName,...  % 1 -> quadratic regression
...
save('MatFiles/L22-14vXFlashDopplerLF');
return
