...
%   for its use
%
% File name: SetUpL12_3vFlashAnglesHI.m - Example of 2-1 synthetic aperture
%                                       plane wave pulse inversion imaging
%                                       with steering angle transmits
% Description:
%   Sequence programming file for L12-3v Linear array, using 2-1 synthetic
%   aperture plane wave pulse inversion transmits with multiple steering
%   angles on 128 channel system. 128 transmit channels and 96 receive
%   channels are active and positioned as follows (each char represents 4
%   elements)for each of the 2 synthetic apertures.
%
%   Element Nos.                                1              1
%                               6       9       2              9
%               1               5       7       9              2
...
Resource.RcvBuffer(1).rowsPerFrame = 5120*2*na; % this size allows for 2 acqs with na angles at maximum range
...
Resource.DisplayWindow(1).Title = 'L12-3vFlashAnglesHI';
...
TW(1).Parameters = [4.5,.67,2,1];

TW(2).type = 'parametric';
TW(2).Parameters = [4.5,.67,2,-1];
...
for i = 1:Resource.RcvBuffer(1).numFrames  % 4*na acquisitions per frame
    k = 4*na*(i-1);
    m = k + 2*na;
...
save('MatFiles/L12-3vFlashAnglesHI');
return
