...
%   for its use
%
% File name: SetUpL12_3vFlash192TX.m - Example of 2-1 synthetic aperture plane
%                                 wave imaging with single transmit
% Description:
%   Sequence programming file for L12-3v Linear array flash imaging, using
%   2-1 synthetic receive acquisitions of 96 elements each (every other
%   element across the 192 element array) on a 128 channel system. For the
%   plane wave flash transmit, all 192 elements are driven in parallel in a
%   single transmit event using the Dynamic Mux programming feature. The
%   transmit and receive apertures are positioned as follows for each of
%   the 2 synthetic receive apertures.
%
%   Element Nos.                                1              1
%                               6       9       2              9
%               1               5       7       9              2
...
%
% Last update:
% May 2020 VTS-343 Trans.HVMux.SHIAperture replaces old "VDAS_Aperture"
% 03/19/2019 updated for use with 4.0 Dynamic Mux Programming feature
...
Resource.Parameters.numTransmit = 128;      % number of system channels.
...
% clear the predefined apertures if we don't want to use them
Trans.HVMux.ApertureES = [];
Trans.HVMux.SHIAperture = []; % VTS-343 May 2020
ApodTX = ones(1, 192); % all elements on in parallel for flash transmit
ApodR1 = zeros(1, 192);
ApodR1([1:2:95 98:2:192]) = 1; % every other element
ApodR2 = ApodTX - ApodR1; % all elements not in ApodR1.
aperTX = computeMuxAperture(ApodTX,Trans);
aperR1 = computeMuxAperture(ApodR1,Trans);
aperR2 = computeMuxAperture(ApodR2,Trans);
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*2; % this size allows for 2 acqs, maximum range
...
Resource.DisplayWindow(1).Title = 'L12-3vFlash192TX';
...
TGC.CntrlPts = [193,487,598,658,770,881,941,1023];
...
Receive = repmat(struct('Apod', zeros(1,192), ...
                        'aperture', [], ...
...
save('MatFiles/L12-3vFlash192TX');
return
