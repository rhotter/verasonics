...
%   for its use
%
% File name: SetUpL12_3v192RyLnsMultiFocal.m - Example of 192 ray line imaging
%                                        with multiple focal zones.
% Description:
%    Generate .mat file for L12-3v Linear array for Vantage system.
%    Transmit/Receive is performed with a 128 element mux aperture that is
%    moved across the 192 element aperture. The Tx aperture is moved with each
...
% Aperture for almost middle beam 128 (TX.aperture=33, numTx=24, numRay=128):
%   ------------------------------------------tttttttttttt------------------------------------------
%   ----------------rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr----------------
% Aperture for last wide beam (TX.aperture=65, numTx=12, numRay=128):
%   -------------------------------/----------------------------------------------------------tttttt
%   --------------------------------rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
%
...
Resource.DisplayWindow(1).Title = 'L12-3v_192RyLnsMultiFocal';
...
for j = 1:P.numRays  % specify P.numRays transmit events
    ...
    for n = 1:fz
        ...
        TX(n+k).Origin = TxOrgX(j);
        % Compute available transmit mux aperture
        [Dummy,ce] = min(abs(scaleToWvl*Trans.ElementPos(:,1)-TX(n+k).Origin(1))); % ce is closest element to center of aperture
        lft = round(ce - 64);
        if lft < 1, lft = 1; end
...
ReconInfo = repmat(struct('mode', 'replaceIntensity', ...  % default is to accumulate IQ data.
                   'txnum', 1, ...
                   'rcvnum', 1, ...
...
SeqControl(3).argument = 15000;  % 15000 usec = 15msec time between frames
...
save('MatFiles/L12-3v_192RyLnsMultiFocal');
