...
%   for its use.
%
% File name: SetUpP4_2vFlash_64.m - Example of phased array imaging with
%                                flash transmit scan
% Description:
...
%   the scanhead connector with element 0-63 connected to inputs 1-64.
%
% Last update:
% 08/28/2018 - Modified for 64 system.
...
Resource.DisplayWindow(1).Title = 'P4-2vFlashAngles_64';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    Receive((i-1)*na+1).callMediaFunc = 1;
    for j = 1:na
        Receive((i-1)*na+j).framenum = i;
        Receive((i-1)*na+j).acqNum = j;
    end
end
...
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:na
    Event(n).info = 'Acquire';
    Event(n).tx = j;
    Event(n).rcv = j+(i-1)*na;
    Event(n).recon = 0;
    Event(n).process = 0;
...
save('MatFiles/P4-2vFlashAngles_64');
return
