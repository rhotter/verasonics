...
%   for its use
%
% File name: SetUpC5_2vFlashDoppler_64.m -  Example of flash angles imaging with
%                                             3-1 synthetic aperture and single flash
%                                             transmit Doppler sensing.
%
% Description:
%   Sequence programming file for C5-2v Linear array, using 3-1 synthetic
%   aperture flash transmits and receive acquisitions for 2D on a Vantage
%   64 channel system. 64 transmit channels and 42 or 44 receive channels
...
%   - All 64 transmit and 64 receive channels are active for each 3-1
...
% set dtheta to range over +/- 15 degrees.
if (na > 1)
...
dopAngle = 0; %12 * pi/180;
dopPRF = 3.0e+03; % Doppler PRF in Hz.
pwrThres = 0.5;
dopR1 = 10;
dopR2 = P(2).endDepth;
dopWidth = 30*pi/180; % angular width of dopple sector
...
% RandR = P(1).endDepth*Media.MP(:,1)+radius;
...
Resource.DisplayWindow(1).Title = 'C5-2vFlashDoppler_64';
...
TX((3*na)+1).Apod(33:96) = 1;
TX((3*na)+1).aperture = computeMuxAperture(TX((3*na)+1).Apod, Trans);
...
maxAcqLngth2D = ceil(sqrt(((P(1).endDepth + radius)^2) + (radius^2) - (2*(P(1).endDepth+radius)*radius*cos(scanangle))));
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = (3*na+1):(3*na+ne)
        % Doppler acquisition
        Receive(j+k).Apod(33:96) = 1.0; % use all channels for Dop aperture
        Receive(j+k).aperture = computeMuxAperture(Receive(j+k).Apod, Trans);
...
Recon(1).RINums(1,1:3*na) = (1:3*na);  % 3*na ReconInfos needed for na angles and 3 apertures.
k = 3*na + 1;
...
save('MatFiles/C5-2vFlashDoppler_64');
return
