...
filename = 'Vermon1024_FlashSuper_RF';
%=== Settings ===%
% Bmode properties
...
switch RDMAconfig.rdmaRole
    case 'primary'
        ...
        Resource.RcvBuffer(1).rowsPerFrame = 1280*P.numTx;
...
for j = 1:P.numFrames
    Receive(P.numTx*(j-1)+1).callMediaFunc = 1;
    for i = 1:P.numTx
        if strcmp(RDMAconfig.rdmaRole,'secondary')
            Receive(P.numTx*(j-1)+i).Apod((RDMAconfig.nodeIndex-1)*256+[1:256])=1;
        else
            Receive(P.numTx*(j-1)+i).Apod(:)=1;
        end
        Receive(P.numTx*(j-1)+i).framenum = j;
        Receive(P.numTx*(j-1)+i).acqNum = i;
    end
end

%% Build Recon & ReconInfo Structures
if strcmp(RDMAconfig.rdmaRole,'primary')  %only do this for primary system
    ...
    ReconInfo = repmat(struct('mode', 'accumIQ', ...
        ...
        'Aperture', [[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]...
             256+[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]...
             512+[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]...
             768+[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]]), 1, P.numTx);
...
TTNA = 2;
SeqControl(TTNA).command = 'timeToNextAcq';  % time between TXs
SeqControl(TTNA).argument = 100;  % 250 usec
%
TTNAFRAME=3;
SeqControl(TTNAFRAME).command = 'timeToNextAcq';  % time between frames
SeqControl(TTNAFRAME).argument = 280e3;% 220 msec (in usec)
% Return to matlab
RETMAT = 4;
...
SeqControl(MSSYNC).argument = 10000;  % 10s timeout (in ms) 
%
SYNC = 6;
SeqControl(SYNC).command = 'sync'; % synchronize SW and HW sequencers
SeqControl(SYNC).argument = 20e3;     % 20 sec
%
TRIGO = 7;
