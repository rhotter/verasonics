...
%   for its use
%
% File name: SetUpVermon1024_RyLns_RF.m - Example of imaging with RayLines
%
% Description:
...
persist = 30;       % Set persistance level
senscutoff = 0.6;
...
for n = 1:P.numRays
    ...
    if strcmp(RDMAconfig.rdmaRole,'primary')  %only need to compute TXPD for systems that are doing recon
...
for j = 1:P.numFrames
    ...
    for i = 1:P.numRays
        if strcmp(RDMAconfig.rdmaRole,'secondary')
            Receive(P.numRays*(j-1)+i).Apod((RDMAconfig.nodeIndex-1)*256+[1:256])=1;
        else
            Receive(P.numRays*(j-1)+i).Apod(:)=1;
        end
...
if strcmp(RDMAconfig.rdmaRole,'primary')  %only do this for primary system
    ...
    ReconInfo = repmat(struct('mode', 'accumIQ', ...
        ...
        256+[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]...
        512+[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]...
        768+[128:-1:65, 1:1:64, 192:-1:129, 193:1:256]]), 1, P.numRays);
...
SeqControl(SYNC).argument = 10e6;     % 10s
