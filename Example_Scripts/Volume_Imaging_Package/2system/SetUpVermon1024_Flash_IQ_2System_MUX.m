...
%   for its use
%
% File name: SetUpVermon1024_Flash_IQ.m
%
% Description: The most basic Multi-System script. Transmits a single flash from all channels
%             in sync. and then performs a partial reconstruction on each node.  The partially
%             reconstructed IQ data is then transferred to the primary system for a final summation
%             and realtime display.
%
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%  Node 1 uses channels 1:256 & 512:767
%  Node 2 uses channels 257:512 & 768:1024
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% Last update:
...
filename = 'Vermon1024_Flash_IQ_2system_MUX';
...
Resource.RcvBuffer(1).rowsPerFrame = 1280*P.numSyntheticTx*P.numSyntheticRcvs;
...
TX = repmat(struct('waveform', 1, ...
    ...
    'TXPD', [], ...
    'peakCutOff', 2,...
    'peakBLMax', 20), 1, P.numSyntheticTx);

% Compute transmit delays
TX(1).Delay = computeTXDelays(TX(1));

for i=1:P.numSyntheticTx
    TX(i).Apod((i-1)*512 + [1:512]) = 1;
...
%% Receive
numReceivesPerFrame = P.numSyntheticTx*P.numSyntheticRcvs;
...
Receive = repmat(struct(...
    ...
    'acqNum', 1, ...
    'sampleMode', 'NS200BW', ...
    'callMediaFunc', 0), 1, P.numFrames*numReceivesPerFrame);

% Set event specific Receive attributes.

for k = 1:P.numFrames
    Receive((k-1)*numReceivesPerFrame + 1).callMediaFunc = 1;
    m = (k-1)*numReceivesPerFrame;
    for i = 1:numReceivesPerFrame
        Receive(m+i).acqNum = i;
...
SeqControl(TTNA).argument = 200; % 60 msec (~16.7fps)
% Time to next acquisition
TTNAFRAME = 3;
...
for j = 1:Resource.RcvBuffer(1).numFrames
    ...
    for k = 1:P.numSyntheticTx %Loop # of transmits
        for i = 1:P.numSyntheticRcvs %Loop # of apertures
            ...
            Event(n).rcv = (j-1)*P.numSyntheticRcvs*P.numSyntheticTx + ...
            ...
            if (k==1)&&(i==1)
                Event(n).seqControl = [TRIG, TTNA, MSSYNC];  %secondary systems just wait for primary system to give sync signal
            else
