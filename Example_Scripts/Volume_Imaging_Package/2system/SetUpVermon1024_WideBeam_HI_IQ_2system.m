...
%   for its use
%
%  File name:  SetUpVermon1024_WideBeam_HI_IQ_2system.m
%
...
filename = 'Vermon1024_WideBeam_HI_IQ_2system';
...
Trans.ElementPos = Trans.ElementPos(1:512,:);
Trans.ConnectorES=zeros(1,512)';
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*na;
...
for i = 1:na
    ...
    TX(i+na).waveform = 2;
    TX(i+na).Steer = [AZ(i) EL(i) 0];
    TX(i+na).Delay = computeTXDelays(TX(i));
end
...
Receive = repmat(struct(...
    ...
    'callMediaFunc', 0), 1, 2*na*P.numFrames);
...
for j = 1:P.numFrames
    ...
    for i = 1:na
        ...
        Receive(i+k).Apod(RDMAconfig.nodeIndex*256+[1:256]) = 1;

        Receive(i+k+na).acqNum = i;
        Receive(i+k+na).framenum = j;
        Receive(i+k+na).Apod(RDMAconfig.nodeIndex*256+[1:256]) = 1;
        Receive(i+k+na).mode = 1;
    end
end
