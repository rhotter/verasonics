...
%   for its use
%
%  File name:  SetUpVermon1024_WideBeam_HI_IQ_2system_MUX.m
%
%  Description: a volume imaging sequence that uses widebeams with pulse 
...
filename = 'Vermon1024_WideBeam_HI_IQ_2system_MUX';
...
% Bmode properties
XYdepth = 50; %depth of C plane imaging plane
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*na*P.numSyntheticTx*P.numSyntheticRcvs;
...
TX = repmat(struct('waveform', 1, ...
    ...
    'peakCutOff', 1,...
    'peakBLMax', 10), 1,2*na*P.numSyntheticTx);

for n = 1:na
    for i=1:P.numSyntheticTx
        
        k = (n-1)*P.numSyntheticTx*2;
        
        TX(k+i).Steer = [AZ(n) EL(n) 0];
        TX(k+i).Delay = computeTXDelays(TX(k+i));
        TX(k+i).aperture = i;
        TX(k+i+P.numSyntheticTx).waveform = 2;
        TX(k+i+P.numSyntheticTx).Steer = [AZ(n) EL(n) 0];
        TX(k+i+P.numSyntheticTx).Delay = computeTXDelays(TX(k+i+P.numSyntheticTx));
        TX(k+i+P.numSyntheticTx).aperture = i;
    end
end

%- Dummy Transmit -%
TX(2*na*P.numSyntheticTx+1)=TX(1); %doesn't really matter which one
TX(2*na*P.numSyntheticTx+1).Apod(:)=0;
...
Receive = repmat(struct(...
    ...
    'callMediaFunc', 0), 1, P.numFrames*numReceivesPerFrame*2);

%Set event specific Receive attributes.
for k = 1:P.numFrames
    for j = 1:na
        m = (k-1)*numReceivesPerFrame*2 + (j-1)*numReceivesPerAngle;  %*2 is for HI
        for i = 1:numReceivesPerAngle
            Receive(m+i).acqNum = (j-1)*numReceivesPerAngle + i;
            Receive(m+i).framenum = k;
            Receive(m+i).Apod(RDMAconfig.nodeIndex*256+[1:256]) = 1;
            ...
            Receive(m+i+numReceivesPerFrame).acqNum = Receive(m+i).acqNum;
            Receive(m+i+numReceivesPerFrame).framenum = Receive(m+i).framenum;
            Receive(m+i+numReceivesPerFrame).Apod = Papod(Papodindex, :);
            Receive(m+i+numReceivesPerFrame).aperture = Paper(Papodindex);
            Receive(m+i+numReceivesPerFrame).mode = 1;
        end
    end
end
...
SeqControl(TTNAFRAME).argument = 1000000;  % 200 msec
% Return to Matlab
RETMAT = 4;
    ...
    for m = 1:na
        for k = 1:P.numSyntheticTx %Loop # of transmits
            for j = 1:P.numSyntheticRcvs %Loop # of apertures
                Event(n).info = 'Aperture 1 Pos+ sync';
                Event(n).tx = (m-1)*P.numSyntheticTx + k;
                Event(n).rcv = (i-1)*numReceivesPerFrame*2 + ...
                               (m-1)*numReceivesPerAngle + ...
                               (k-1)*P.numSyntheticRcvs + ...
                               j;
                Event(n).recon = 0;
                Event(n).process = 0;
                %first event should be a syncup
                ...
                Event(n).info = 'Aperture 1 Neg';
                Event(n).tx = (m-1)*P.numSyntheticTx + k + (na*P.numSyntheticTx);
                Event(n).rcv = (i-1)*numReceivesPerFrame*2 + ...
                                     numReceivesPerFrame + ...
                               (m-1)*numReceivesPerAngle + ...
                               (k-1)*P.numSyntheticRcvs + ...
