...
%   for its use
%
% File name: SetUp8xL7_4Flash.m - Example of imaging with single plane wave
%                                 transmit from 8xL7-4 probes in a circle
% Description:
...
%
% Last update:
% 6/30/2019 working in simulation by BWC

clear all
P.startDepth = 0;   % Acquisition depth in wavelengths
P.endDepth = 512;   % This should preferrably be a multiple of 128 samples.
...
R = 75; % in mm
Trans.type = 1; %1 for 'curved'
Trans.radius=R/waveLength;
theta = (0:pi/4:2*pi-pi/4)+pi/8;
pitch_mm = 0.3;
pos_ele = (-63.5:63.5)*pitch_mm; % in mm
nElems=1024;  %adjust for debugging
Trans.id = 592; % need this still
Trans.connType = 0;
Resource.Parameters.connector = [1 2];
Trans.numelements=nElems;
for n = 1:length(theta)
    CT = cos(theta(n));
    ST = sin(theta(n));
    Trans.ElementPos([1:128]+(n-1)*128,:) = fliplr([...
        (  R*CT+pos_ele*ST')/waveLength;...    %X position
        zeros(1,128);...                       %Y position
        (-(R*ST-pos_ele*CT')+R)/waveLength;... %Z position (+Zoffset=R so that everything is in the recon field
        ones(1,128)*theta(n)-pi/4-pi/8;...    % angle element is facing
        zeros(1,128)...                        % angle
        ])';
end
Trans.ElementPos=Trans.ElementPos([1:nElems],:);
Trans.ConnectorES=[1:nElems]';


% Specify PData structure array.
PData(1).PDelta = [1, 0, 1];
...
PData(1).Size(1) = ceil(Trans.radius*2);
PData(1).Size(2) = ceil(Trans.radius*2);
PData(1).Size(3) = 1;      % single image page
PData(1).Origin = [-Trans.radius,0,0]; % x,y,z of upper lft crnr.
...
Media.MP(1,:) = [0,0,Trans.radius,1];
Media.numPoints = size(Media.MP,1);
% Specify Resources.
Resource.RcvBuffer(1).datatype = 'int16';
Resource.RcvBuffer(1).rowsPerFrame = 9216;   % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 100;       % 100 frames used for RF cineloop.
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = '8xL7-4Flash_sim';
