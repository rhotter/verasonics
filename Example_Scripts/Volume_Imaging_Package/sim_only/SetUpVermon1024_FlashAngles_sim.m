...
filename = 'MatFiles/Vermon1024_FlashAngles_sim.mat';

%=== Settings ===%
% Bmode properties
CompressionFactor = 20;
pgain = 8;          % Digital gain
persist = 10;       % Set persistance level
senscutoff = 0.6;

P.numTx = 3*3;   % no. of elements in TX aperture.(5x5)
...
for naX = 1:3
    for naY = 1:3
        steerAngles(i,:) = [stheta + (naX-1)*dtheta,
                            salpha + (naY-1)*dalpha]*pi/180;
            i = i+1;
    end
end
na = naX*naY;
for a = 1:na
    TX(a).Steer = steerAngles(a,:);
    TX(a).Delay = computeTXDelays(TX(a));
    % Compute transmit pixel data
%     disp(['computeTXPD(' num2str(a) ')'])
%     TX(a).TXPD = computeTXPD(TX(a),PData(1));
end
...
Receive = repmat(struct(...
    ...
    'acqNum', 1, ...
    'sampleMode', 'NS200BW', ...
    'callMediaFunc', 0), 1, P.numTx*P.numFrames);

%Set event specific Receive attributes.
for j = 1:P.numFrames
    Receive(i+(j-1)*P.numTx).callMediaFunc = 1;
    for i = 1:P.numTx
        Receive(i+(j-1)*P.numTx).acqNum = i;
        Receive(i+(j-1)*P.numTx).framenum = j;
    end
end
...
ReconInfo = repmat(struct('mode', 'accumIQ', ...  % default is to accumulate IQ data.
    ...
    'Post',[],...
    'scaleFactor', 1, ...
    'threadSync', 1), 1, P.numTx*3);  %need 3 x number of rays recon, 1 for each imaging plane
...
for j = 1:Resource.RcvBuffer(1).numFrames
    for k = 1:P.numTx
        ...
        Event(n).rcv = (j-1)*P.numTx+k;  % use 1st Rcv structure of frame.
...
% - Range Change
MinMaxVal = [50,85,80]; % default unit is wavelength
AxesUnit = 'mm';
