...
filename = 'MatFiles/Vermon1024_Flash_sim.mat';
...
P.numTx = 1;   % no. of elements in TX aperture.(5x5)
...
for j = 1:P.numFrames
    Receive(1+(j-1)).callMediaFunc = 1;
    Receive(1+(j-1)).acqNum = 1;
    Receive(1+(j-1)).framenum = j;
end
...
ReconInfo = repmat(struct('mode', 'accumIQ', ...  % default is to accumulate IQ data.
    ...
    'Post',[],...
    'scaleFactor', 1, ...
    'threadSync', 1), 1, 3);  %need 3 x number of rays recon, 1 for each imaging plane

ReconInfo(1).Pre = 'clearInterBuf';
for k = 1:3
    ReconInfo(k).txnum = 1;
    ReconInfo(k).rcvnum = 1;
    ReconInfo(k).regionnum = k;
end
...
for j = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).rcv = j;  % use 1st Rcv structure of frame.
...
% - Range Change
MinMaxVal = [25,100,P.endDepth]*(Resource.Parameters.speedOfSound/1000/Trans.frequency); % default unit is wavelength
AxesUnit = 'mm';
