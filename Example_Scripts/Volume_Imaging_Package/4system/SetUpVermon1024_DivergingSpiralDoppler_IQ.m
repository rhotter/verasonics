...
%   for its use
%
% File name: SetUpVermon1024_DivergingSpiralDoppler.m
...
filename = 'Vermon1024_DivergingSpiralDoppler_IQ';
...
nVol = 32;       % Set nVol = number of volume acquisitions in Doppler ensemble. 200x9
dopAngle = 2 * pi/180;
...
if strcmp(RDMAconfig.rdmaRole,'primary')
    %--- Interbuffer---%
    % Bmode needs 4
    Resource.InterBuffer(1).numFrames = 1;  % 1/4 buffer for the primary
    Resource.InterBuffer(2).numFrames = 1;  % 2/4 buffer for the primary
    Resource.InterBuffer(3).numFrames = 1;  % 3/4 buffer for the primary
    Resource.InterBuffer(4).numFrames = 1;  % 4/4 buffer for the primary
    % Doppler needs 4
    Resource.InterBuffer(5).numFrames = 1;  % 1/4 buffer for the primary
    Resource.InterBuffer(5).pagesPerFrame = nVol;
    Resource.InterBuffer(6).numFrames = 1;  % 2/4 buffer for the primary
    Resource.InterBuffer(6).pagesPerFrame = nVol;
    Resource.InterBuffer(7).numFrames = 1;  % 3/4 buffer for the primary
    Resource.InterBuffer(7).pagesPerFrame = nVol;
    Resource.InterBuffer(8).numFrames = 1;  % 4/4 buffer for the primary
    Resource.InterBuffer(8).pagesPerFrame = nVol;
    %---Destintation InterBuffer for Just Doppler Processing
    Resource.InterBuffer(9).numFrames = 1;  %
    Resource.InterBuffer(9).pagesPerFrame = nVol;
else
    Resource.InterBuffer(1).numFrames = 1;  % 1/4 buffer for the primary
    Resource.InterBuffer(2).numFrames = 1;          % 1/4 buffer for the primary
    Resource.InterBuffer(2).pagesPerFrame = nVol;     % 1/4 nVol pages per ensemble
...
Resource.Parameters.numTransmit=1024;
...
if strcmp(RDMAconfig.rdmaRole,'primary')
    ...
    Process(5).Parameters = {'IntBufSrc',[5,1,6,1,7,1,8,1],...
        'IntBufDest',[9,1]};
    %---------------------------------------------------------------------%
    Process(6).classname = 'Doppler'; % process structure for 1st Doppler ensemble
    Process(6).method = DopState;
    Process(6).Parameters = {'IntBufSrc', [9, 1],...      % buffer and frame num of interbuffer
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    if strcmp(RDMAconfig.rdmaRole,'secondary')    %--- secondary ---%
        ...
        RDMAcontrol(2).primaryDstBufNum = RDMAconfig.nodeIndex+1+4;
        ...
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = [nsc, nsc + 1]; %1 for bmode 1 for doppler
        ...
        %Generate 3x2 RDMA structures (3 secondaries) (2 Buffers) (x3 I,Q,N... in the end there should be 18)
        %buf1, primary Bmode
        %buf2, secondary1 Bmode
        %buf3, secondary2 Bmode
        %buf4, secondary3 Bmode
        %buf5, primary Doppler
        %buf6, secondary1 Doppler
        %buf7, secondary2 Doppler
        %buf8, secondary3 Doppler
        %buf9  combined doppler for doppler processing
        for a = 1:RDMAconfig.numSecondaries
            ...
            RDMAcontrol(a+RDMAconfig.numSecondaries).bufferType = 'inter';
            RDMAcontrol(a+RDMAconfig.numSecondaries).primaryDstBufNum = a+1+4;
            RDMAcontrol(a+RDMAconfig.numSecondaries).primaryDstFrameNum =1;
            RDMAcontrol(a+RDMAconfig.numSecondaries).secondarySrcBufNum = 2;
            RDMAcontrol(a+RDMAconfig.numSecondaries).secondarySrcFrameNum  = 1;
            RDMAcontrol(a+RDMAconfig.numSecondaries).secondaryIndex = a;
