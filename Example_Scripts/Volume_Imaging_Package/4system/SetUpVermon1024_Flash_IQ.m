...
if strcmp(RDMAconfig.rdmaRole,'primary')
    ...
    Resource.InterBuffer(5).numFrames = 1;  % buffer for sumIQnorm test
end
...
if strcmp(RDMAconfig.rdmaRole,'primary')
    ...
    Process(4).Parameters = {'IntBufSrc',[1,1,2,1,3,1,4,1],'ImgBufDest',[1,-1]};
...
for j = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = sprintf('Full Aperture + Sync frame %i',j);
    Event(n).tx = 1;
    Event(n).rcv = j;
    Event(n).recon = 0;
    Event(n).process = 0;
    ...
    if strcmp(RDMAconfig.rdmaRole,'secondary')    %--- secondary ---%
        ...
        % NOTE THAT NO RDMAWRITE EVENT IS NEEDED FOR MASTER SYSTEM BUT IT RECEIVES THE DATA HERE
