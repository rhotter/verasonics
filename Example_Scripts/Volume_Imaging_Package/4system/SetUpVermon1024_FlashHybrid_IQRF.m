...
%   for its use
%
% File name: SetUpVermon1024_FlashHybrid_IQRF.m
%
% Description: An example script that allows the user to preview with
%              partial beamforming and then press a GUI button to jump to a
%              part of the sequence where the RF is transferred to the
%              primary into a second RF Buffer for saving or anything else.
%
...
filename = 'Vermon1024_FlashHybrid_IQRF';

%=== Settings ===%
% Bmode properties
XYdepth = 70; %depth of C plane imaging plane
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*P.numTx;
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = P.numFrames;       % 100 frames used for RF cineloop.
%for RF transfer
Resource.RcvBuffer(2).datatype = 'int16';
Resource.RcvBuffer(2).rowsPerFrame = 4096*P.numTx;
Resource.RcvBuffer(2).colsPerFrame = 256;
Resource.RcvBuffer(2).numFrames = P.numFrames;       % 100 frames used for RF cineloop.

Resource.InterBuffer(1).numFrames = 1;  % buffer for primary IQ
%--- MultiSys ---%
if strcmp(RDMAconfig.rdmaRole,'primary')
    ...
    Resource.RcvBuffer(2).colsPerFrame = 1024;
end
...
Receive = repmat(struct(...
    ...
    'acqNum', 1, ...
    'sampleMode', 'NS200BW', ...
    'callMediaFunc', 0), 1, P.numFrames*2); %2 sets of receives

% Set event specific Receive attributes.

for j = 1:P.numFrames
    Receive(j).callMediaFunc = 1;
    Receive(j).Apod(RDMAconfig.nodeIndex*256+[1:256])=1;
    Receive(j).framenum = j;
    Receive(j).acqNum = 1;
    %The second send of Receives is for the RF transfer to bufnum 2
    Receive(j+P.numFrames).callMediaFunc = 1;
    Receive(j+P.numFrames).Apod(RDMAconfig.nodeIndex*256+[1:256])=1;
    Receive(j+P.numFrames).framenum = j;
    Receive(j+P.numFrames).acqNum = 1;
    Receive(j+P.numFrames).bufnum = 2;
...
Recon(1) = struct('senscutoff', senscutoff, ...
    ...
    'IntBufDest', [1,1],...
    'RINums', 1);
Recon(1).newFrameTimeOut = 10000; %10 seconds is generous
...
SeqControl(TTNAFRAME).argument = 60e3; % 60 msec (~16.7fps)
% Return to Matlab
RETMAT = 3;
...
SeqControl(SYNC).argument = 20e6;     % 20sec
% Trigger Out for debugging
TRIG = 7;
SeqControl(TRIG).command = 'triggerOut';
% TTNA for dummy TX
TTNADUMMY = 8;
...
% NOOP
JUMPNOOP = 9;
SeqControl(JUMPNOOP).command = 'noop';  % time between dummy TX and start of Frame
SeqControl(JUMPNOOP).argument = 200;
...
for j = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'Full aperature + Sync';
    Event(n).tx = 1;
    Event(n).rcv = j;
    Event(n).recon = 0;
    Event(n).process = 0;
    if strcmp(RDMAconfig.rdmaRole,'primary')
        Event(n).seqControl = [TRIG, TTNAFRAME, MSSYNC];  %use only the primary for timing of the frames
    else
        Event(n).seqControl = [TRIG, MSSYNC];  %secondary systems just wait for primary system to give sync signal
    end
    n = n+1;
    ...
    if strcmp(RDMAconfig.rdmaRole,'secondary')    %--- secondary ---%
        ...
        Event(n).info = sprintf('RDMA IQ Transfer - frame %i',j);
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 0;
        ...
        % NOTE THAT NO RDMAWRITE EVENT IS NEEDED FOR primary SYSTEM BUT IT RECEIVES THE DATA HERE
...
%THIS PART OF THE SEQUENCE THE USER JUMPS TO, IT ACQUIRES N FRAMES and then
%does an RF transer
if strcmp(RDMAconfig.rdmaRole,'primary')
    nrc = 4; %Next RDMA Control index
else
    nrc = 2; %Next RDMA Control index
end
jumpEvent = n;
% Acquire all frames defined in RcvBuffer
...
if strcmp(RDMAconfig.rdmaRole,'primary')
    Event(n).info = 'primary delay';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = 0;
    Event(n).seqControl = JUMPNOOP;
    n=n+1;
end

for j = 1:Resource.RcvBuffer(2).numFrames
    if strcmp(RDMAconfig.rdmaRole,'primary')
        Event(n).info = 'DUMMY TX';
        Event(n).tx = P.numTx + 1;
        Event(n).rcv = 0;
    ...
    Event(n).info = 'Full aperature + Sync';
    Event(n).tx = 1;
    Event(n).rcv = j + P.numFrames;
    Event(n).recon = 0;
    Event(n).process = 0;
    if strcmp(RDMAconfig.rdmaRole,'primary')
        Event(n).seqControl = [TTNAFRAME, MSSYNC];  %use only the primary for timing of the frames
    else
        Event(n).seqControl = [MSSYNC];  %secondary systems just wait for primary system to give sync signal
    end
    n = n+1;
    ...
    if strcmp(RDMAconfig.rdmaRole,'secondary') %--- secondary ---%
        ...
        RDMAcontrol(nrc).bufferType = 'receive';
        RDMAcontrol(nrc).primaryDstBufNum = 2;
        RDMAcontrol(nrc).primaryDstFrameNum = j;
        RDMAcontrol(nrc).primaryDstStartChNum = 256*(RDMAconfig.nodeIndex)+1;
        RDMAcontrol(nrc).secondarySrcBufNum = 2;
        RDMAcontrol(nrc).secondarySrcFrameNum  = j;
        RDMAcontrol(nrc).secondaryIndex  = RDMAconfig.nodeIndex;
        
        Event(n).info = sprintf('RDMA RF Transfer - frame %i',j);
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = nsc;
        SeqControl(nsc).command = 'rdmaWrite';
        SeqControl(nsc).argument = nrc; % link it to RDMA structure
        nsc = nsc + 1;
        nrc = nrc + 1;
        n = n+1;
        
        Event(n).info = 'RdmaSync';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 0;
        ...
        for a = 1:RDMAconfig.numSecondaries
            ...
            RDMAcontrol(nrc).primaryDstBufNum = 2;
            RDMAcontrol(nrc).primaryDstFrameNum = j;
            RDMAcontrol(nrc).primaryDstStartChNum = 256*a + 1;  %only use for receive
            RDMAcontrol(nrc).secondarySrcBufNum = 2;
            RDMAcontrol(nrc).secondarySrcFrameNum  = j;
            RDMAcontrol(nrc).secondaryIndex  = a;
            nrc = nrc+1;
        end
...
if strcmp(RDMAconfig.rdmaRole,'primary')
    Event(n).info = 'Save Data on Primary';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = 5;
...
%% CHECK %%
% go through all events and if the event contains an rdmaWrite seqControl,
% print out line
for a = 1:length(Event)
    if Event(a).seqControl~=0
        for b = 1:length(Event(a).seqControl)
            if strcmp(SeqControl(Event(a).seqControl(b)).command,'rdmaWrite')
                fprintf('Event(%g) --> ',a)
                fprintf('SeqControl(%g) --> ',Event(a).seqControl(b))
                fprintf('RDMAcontrol: %g\n', SeqControl(Event(a).seqControl(b)).argument);
            end
        end
    end
end
...
UI(2).Control =  {'UserB2','Style','VsPushButton','Label','RF Acq.'};
UI(2).Callback = text2cell('%RFAcq');
...
%EF#1
RFsave(rfData)
fname = ['RFData' datestr(now,30)];
tic
fprintf('saving RFData into file: %s\n',fname);
save(fname,'rfData','-v7.3');  %use the 7.3 flag for large files... use '-nocompression' flag for faster saving
toc
return
%EF#1
...
%RFAcq
jumpEvent=evalin('base','jumpEvent');
disp('Jump to RF acq')
Control(1).Command = 'set&Run';
Control(1).Parameters = {'Parameters',1,'startEvent',jumpEvent};
assignin('base','Control', Control);
%RFAcq
