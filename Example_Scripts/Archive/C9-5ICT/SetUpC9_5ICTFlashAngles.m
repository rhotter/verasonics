...
%   for its use
%
% File name: SetUpC9_5ICTFlashAngles.m - Example of curved array flash imaging
%                                      with steering angle transmits
% Description:
%   Sequence programming file for C9-5ICT curved array, using flash transmits
%   with multiple steering angles. All 128 transmit and receive channels
...
if (na > 1), dtheta = (24*pi/180)/(na-1); startAngle = -12*pi/180; else dtheta = 0; startAngle=0; end % set dtheta to range over +/- 12 degrees.
...
Trans = computeTrans(Trans);  % C9-5ICT transducer is 'known' transducer so we can use computeTrans.
...
Resource.DisplayWindow(1).Title = 'C9-5ICTFlashAngles';
...
save('MatFiles/C9-5ICTFlashAngles');
return
