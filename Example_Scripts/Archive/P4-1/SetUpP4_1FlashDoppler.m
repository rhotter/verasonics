...
%   for its use.
%
% File name: SetUpP4_1FlashDoppler.m - Example of phased array flash
%                                       Doppler imaging
% Description:
%   Sequence programming file for P4-1 phased array in virtual apex format,
%   using flash tranmists for 2D (B-mode) and doppler imaging. All 96 transmit
...
%   the scanhead connector with elements 0-31 connected to inputs 1-32, and
%   elements 32-63 connected to inputs 97-128.  We therefore need a
%   Trans.Connector array to specify the connector channels used, which will
%   be defined by the computeTrans function.
%
% Last update:
...
if (na > 1), dtheta2D = (60*pi/180)/(na-1); startAngle = -60*pi/180/2; else dtheta2D = 0; startAngle=0; end % set dtheta2D to range over +/- 30 degrees.
...
Resource.System.UTA = '260-M'; % this line was added
...
Trans.frequency = 1; % this line was added
...
aperture = Trans.numelements*Trans.spacing; % aperture based on 96 elements
P(1).theta = -pi/4;
...
Resource.DisplayWindow(1).Title = 'P4-1FlashDoppler';
...
TW(2).Parameters = [2.5,0.67,6,1];

% Specify TX structure array.
TX = repmat(struct('waveform', 1, ...
                   'Origin', [0.0,0.0,0.0], ...
                   'focus', 100 * P(1).radius, ...
                   'Steer', [0.0,0.0], ...
...
% - 2D TGC
TGC(1).CntrlPts = [100,601,701,710,821,932,992,1023];
...
Receive = repmat(struct('Apod',  apod, ... % this line was added
                        ...
                        'mode', 0, ...
                        'callMediaFunc', 0, ...
                        'aperture', 1),1,(na+ne)*Resource.RcvBuffer(1).numFrames);
...
save('MatFiles/P4-1FlashDoppler');
return
