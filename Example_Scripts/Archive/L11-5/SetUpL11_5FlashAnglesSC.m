...
%   for its use
%
% File name: SetUpL11_5FlashAnglesSC.m - Plane wave transmits with steering
%                                         angles and spatial compounding
% Description:
%   Sequence programming file for L11-5 Linear array, using plane wave
...
if (na > 1)
  dtheta = (40*pi/180)/(na-1); startAngle = -40*pi/180/2; % set dtheta to range over +/- 20 degrees.
else
...
Trans = computeTrans(Trans);  % L11-5 transducer is 'known' transducer so we can use computeTrans.
...
Resource.DisplayWindow(1).Title = 'L11-5FlashAnglesSC';
...
ReconInfo(j).normPower = 0.33;
ReconInfo(j+1).mode = 'replaceIQ';
ReconInfo(na).mode = 'accumIQ_multiplyIntensity'; % accum, detect and multiply-accum
ReconInfo(na).normPower = 0.33;
...
save('MatFiles/L11-5FlashAnglesSC');
return
