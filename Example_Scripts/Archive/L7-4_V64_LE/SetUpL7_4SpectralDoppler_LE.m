...
%   for its use
%
% File name: SetUpL7_4SpectralDoppler_LE.m - Spectral doppler with plane wave transmit.
%
% Description:
%   Sequence programming file for L7-4 Linear array, using plane wave
%   transmits for 2D and doppler imaging.
%   - for the 2D image, a single plane wave transmit is used.
%   - for the doppler ensemble, multiple pulses are transmitted at a
%     steering angle of dopAngle radians. The Doppler measurement region
%     is the region under the central 64 transducer elements.
%   - All 128 transmit and 64 receive channels are active for each synthetic
%     and doppler acquisitions.
...
%   - To simplify the architecture, Spectral Doppler PRF is limited to a
%     finite set of allowable values. Specified values for PRF will be
%     quantized to this set. These are displayed when this script is run.
%
% Last update:
% 03/27/2020 - modified for bug fix VTS-1609 @Author: Daniel Rohrbach
% 

clear all

% - 2D P(1) structure
P(1).startDepth = 6;          % Acquisition depth in wavelengths
...
sgp= feval(@spectralDoppler,'spectrogramParameters',framePeriod,dopSweepTimeIndex);
sweepTime = sgp.THist;
...
Resource.InterBuffer(2).numFrames = 1;          % two intermediate frame needed for color
...
Resource.DisplayWindow(1).Title = 'L7-4SpectralDoppler_LE';
...
Resource.DisplayWindow(1).Type = 'Matlab'; % Verasonics not supported
...
% We need 1 TX specifications for 2D and 1 TX specifications for Doppler (1 steering angles).
...
for i = 1:nfrms
    ...
    for j = 1:2:maxPRIs
        ...
        if fix((j+1)/4) == (j+1)/4 % if j = 3,7,11,...  every other 2D receive uses 2nd half of aperture
            Receive(k+j).Apod(1:64) = 0;
            Receive(k+j).Apod(65:128) = 1.0;
        end
...
SeqControl(5).argument = 4;  % jump to start of acquisition
nsc = 6; % nsc is count for next SeqControl object
...
Event(n).seqControl = nsc;
SeqControl(nsc).command = 'jump';
SeqControl(nsc).argument = nStartAcq;
...
UI(7).Statement = 'set(Resource.DisplayWindow(1).figureHandle,''WindowButtonDownFcn'',@wbdCallback);';
...
save('MatFiles/L7-4SpectralDoppler_LE');
return
...
Event(n)=struct('info','Jump back to first event',...
   'tx',0,'rcv',0,'recon',0,'process',0,'seqControl',nsc);
   SeqControl(nsc).command = 'jump';
   SeqControl(nsc).argument = nStartAcq;
...
sgp=feval(@spectralDoppler,'spectrogramParameters',TFrame,1);
...
if strcmp(get(hObject,'SelectionType'),'normal')
   if elypHndl == 0
      ...
      Control(2).Parameters = {'Parameters',1,'startEvent',3};
      evalin('base','Resource.Parameters.startEvent = 3;');
   else
