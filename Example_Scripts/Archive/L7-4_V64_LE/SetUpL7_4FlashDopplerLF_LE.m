...
%   for its use
%
% File name: SetUpL7_4FlashDopplerLF_LE.m - Plane wave doppler imaging with
%                                          "EnsembleLength" statement
% Description:
...
%     transmitted at a steering angle of dopAngle radians. Preset of ensemble
%     length is included via switch statement on "EnsembleLength".
...
if (na > 1), dtheta2D = (36*pi/180)/(na-1); startAngle = -36*pi/180/2; else dtheta2D = 0; startAngle=0; end % set dtheta2D to range over +/- 18 degrees.
...
switch EnsembleLength
    ...
    case 'WFlongest'
        ...
        dopPRF =5.0e+03; % Doppler PRF in Hz.
        pwrThres = 0.05;
    case 'WFlonger'
        %weak flow example
...
Resource.RcvBuffer(1).rowsPerFrame = 2048*(2*na + ne);
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numTransmit;
Resource.RcvBuffer(1).numFrames = 50;          % 50 frames allocated for RF acqusitions.
...
Resource.DisplayWindow(1).Title = 'L7-4FlashDopplerLF_LE';
...
% - 2D TGC
TGC.CntrlPts = [0,169,331,487,614,726,846,942];
...
Recon(1).RINums(1,1:2*na) = (1:2*na);  % na ReconInfos needed for na angles
k = 2*na + 1;
...
save('MatFiles/L7-4FlashDopplerLF_LE');
return
