...
%   for its use
%
% File name RunSetUpL7_4FlashHFR_reconAll_LE.m: "High Frame Rate" post-acquisition processing script (script 2 of 2)
%
% Description:
% To support higher acquisition frame rates with reduced DMA overhead, the first script (SetupL7_4FlashHFR_acquire) acquires
...
%
% To illustrate the HFR effect in simulation, the 'movePoints' function is invoked (in SetupL7_4FlashHFR_acquire)
...
simulateMode = 2; % use mode=2 for post processing data collected by 'SetupL7_4FlashHFR' and stored in 'RFdataHFR.mat'.
...
if simulateMode ==2,
    ...
    P.numFrames = 6;  % default no. of frames
end
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*P.numAcqs*2;   % this size allows for maximum range
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'sampleMode', 'NS200BW', ...
                        'mode', 0, ...
                        'callMediaFunc', 0),1,2*P.numFrames*P.numAcqs);

% - Set event specific Receive attributes.
for frame = 1:P.numFrames    % -- Acquisitions for 'super' frame.
    Receive(2*P.numAcqs*(frame-1) + 1).callMediaFunc = 1;   % move media points only every super frame
    for acqNum = 1:2:2*P.numAcqs
        % -- Acquisitions for 'super' frame using aperture 1.
        rcvnum = 2*P.numAcqs*(frame-1) + acqNum;
        Receive(rcvnum).Apod(1:64)  = 1;
        ...
        % -- Acquisitions for 'super' frame using aperture 2.
        Receive(rcvnum+1).Apod(65:128)  = 1;
        Receive(rcvnum+1).framenum = frame;       % each frame is transfered by one DMA ("transfer to host").
        Receive(rcvnum+1).acqNum   = acqNum+1;      % each acquisition is stored separately, indexed by framenum and acqNum
    end
end
...
for acqNum = 1:2:2*P.numAcqs
    Recon((acqNum+1)/2) = struct('senscutoff', 0.6, ...
                    'pdatanum', 1, ...
                    ...
                    'RINums', [acqNum acqNum+1]);          % one unique ReconInfo per Recon
                                                % NOTE: can never use the same ReconInfo in different Recons!

    ReconInfo(acqNum) = struct('mode', 'replaceIQ', ...
                    'txnum', 1, ...
                    'rcvnum', acqNum, ...       % the index of the receive acquisition
                    'regionnum', 1);
    ReconInfo(acqNum+1) = struct('mode', 'accumIQ_replaceIntensity', ...
                    'txnum', 1, ...
                    'rcvnum', acqNum+1, ...       % the index of the receive acquisition
                    'regionnum', 1);
end
...
for frame = 1:P.numFrames
    for acqNum = 1:2:2*P.numAcqs  % Acquire all acquisitions defined in one super frame
        Event(n).info = 'aperture 1st half';
        Event(n).tx = 1;
        Event(n).rcv = 2*P.numAcqs*(frame-1) + acqNum;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = [2,3];
        n = n+1;

        Event(n).info = 'aperture 2nd half';
        Event(n).tx = 1;
        Event(n).rcv = 2*P.numAcqs*(frame-1) + acqNum + 1;
        Event(n).recon = 0;
        Event(n).process = 0;
