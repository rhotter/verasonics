...
%   for its use
%
% File name: SetUpL7_4TripleBeam.m - Imaging with triple focused transmits
%
% Description:
%   Sequence programming file for L7-4 Linear array, using 3 focused beam
%   on transmit. For each acquisition, 96 channels are active for transmit
%   128 channels are active for receive. Processing is asynchronous with
%   respect to acquisition.
%
% Last update:
% 11/11/2015 - modified for SW 3.0
...
Resource.RcvBuffer(1).rowsPerFrame = 3*4096;   % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 50;       % 50 frames used for RF cineloop.
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = 'L7-4TripleBeam';
