...
%   for its use
%
% File name: SetUpL7_4FlashAccum.m - Example of plane wave imaging by accumulating
%                                      multiple RF acquisitions.
% Description:
%   Sequence programming file for L7-4 Linear array, using a plane wave
%   transmit and receive by accumulating multiple acquisitions. All
...
Resource.DisplayWindow(1).Title = 'L7-4FlashAccum';
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'sampleMode', 'NS200BW', ...
                        'mode', 0, ...
                        'callMediaFunc', 0),1,3*Resource.RcvBuffer(1).numFrames); % Create 3 Receive sturcture for synthetic aperture
% - Set event specific Receive attributes. First set of Receives uses mode 0.
for i = 1:Resource.RcvBuffer(1).numFrames
    % -- 1st acquisition for full frame.
    Receive(i).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
    Receive(i).framenum = i;
    Receive(i).acqNum = 1;
    Receive(i).callMediaFunc = 1;
end
% -- 2nd set of Receives uses mode 1.
k = Resource.RcvBuffer(1).numFrames;
for i = 1:Resource.RcvBuffer(1).numFrames
    % -- accumulated acquisition for full frame.
    Receive(i+k).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
    Receive(i+k).framenum = i;
    Receive(i+k).acqNum = 1;
    Receive(i+k).mode = 1;
end
% -- 3rd set of Receives uses mode 1.
k = 2*Resource.RcvBuffer(1).numFrames;
for i = 1:Resource.RcvBuffer(1).numFrames
    % -- accumulated acquisition for full frame.
    Receive(i+k).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
    Receive(i+k).framenum = i;
...
ReconInfo = struct('mode', 'replaceIntensity', ...
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 1);% initialise two ReconInfo for synthetic aperture
...
SeqControl(3).argument = 10000 - 200*P.numAccum;  % 10 msec
SeqControl(4).command = 'returnToMatlab';
SeqControl(5).command = 'loopCnt';
SeqControl(5).argument = (P.numAccum-1)/2;
nsc = 6; % nsc is count of SeqControl objects

n = 1; % n is count of Events
k = Resource.RcvBuffer(1).numFrames; % no. of Receives in an acquisition set
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'Accumulate 1st half of aperture into 1st bank.';
    Event(n).tx = 1;
    Event(n).rcv = 2*k+i;
    Event(n).recon = 0;
    Event(n).process = 0;
