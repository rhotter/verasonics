...
%   for its use
%
% File name: SetUpCL15_7WideBeamHI.m - Pulse inversion imaging with widebeam transmit
%
% Description:
%   Sequence programming file for CL15-7 Linear array, using widebeam pulse
...
Resource.RcvBuffer(1).rowsPerFrame = P.numRays*3072; % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 10;    % 10 frames stored in RcvBuffer.
Resource.InterBuffer(1).numFrames = 1;   % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = 'CL15-7WideBeamHI';
...
%     ReconInfo(P.numRays).mode = 'accumIQ_replaceIntensity'; % accum and detect
else
...
save('MatFiles/CL15-7WideBeamHI');
return
