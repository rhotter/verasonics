...
%   for its use
%
% File name: SetUpL12_5_50mmFlashDoppler.m - Example of 3-1 synthetic aperture plane
%                                           wave imaging with steering angle transmits
% Description:
%   Sequence programming file for L12-5 50mm Linear array, using 3-1
...
Trans = computeTrans(Trans);  % L12-5 50mm 50mm transducer is 'known' transducer so we can use computeTrans.
...
Resource.DisplayWindow(1).Title = 'L12-5 50mmFlashDoppler';
...
wl2sPer128 = 128/(2*2);  % wavelengths in a 128 sample block for 2 smpls per wave round trip.Receive = repmat(struct('Apod', zeros(1,128), ...
...
save('MatFiles/L12-5_50mmFlashDoppler');
return
