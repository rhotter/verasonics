% SetUpL11_5vFlash_MultiSysSync.m - Example of phase synchronizing two systems by sharing the 250 MHz clock and triggers
...
%
%   The original script was modified to use in a multi-system shared clock configuration, in which an HDMI cable
%   passes the 250 MHz clock from the "master" system Clock OUT to the synchronization box, which then redistributes
%   that clock and some other synchronization signals to all of the systems.  The SequenceControl command 'MultiSysSync'
%   places each event sequence in a wait for trigger state; when the master system arrives at the MultiSysSync command
%   it  produces a trigger output that is sent to the sync box, which buffers and redistributes it
%   to all systems (over the HDMI cables) as a trigger input. All systems, including the master, then continue executing
%   their individual seqeunces at the same time. Here, the assumption is that all systems will run this same script,
%   but only one script should set the variable 'master' to one, in line 37 below.
%   See "Application Note_Phase-Accurate Synchronization of Multiple Vantage Systems" for further documentation.
%   Note the sections of code below identified by the comment: "% === add for multi-system sync ==="
%
%
% Last update:
...
% 7/31/2015 - Add synchronization logic for cable-only sync. Tested with Software 2.10.4.
% 9/8/2015  - Tested with 2.11.6
%
% Notice:
...
filename = ('L11-5vFlash_MultiSysSync'); % assigning the matfile name to the variable 'filename' permits avoiding the VSX query

% === add for multi-system sync ===
master = 0; % =1 Master   =0 Slave  The Master script sets frame rate, and uses a TTNA command that is left out of the slave scripts.
...
nsc = 1;    % nsc-1 is count of SeqControl objects
...
SeqControl(nsc).argument = 20*1000;  % 20 msec for 50 fps
TTNA = nsc;     nsc = nsc+1;

SeqControl(nsc).command = 'returnToMatlab';
RTNMatlab = nsc;    nsc = nsc+1;

SeqControl(nsc).command = 'noop';
SeqControl(nsc).argument = 10*5;  % 1 microsec for 5 counts (200 ns per count)
NOOP = nsc;     nsc = nsc+1;
...
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = NOOP;
n=n+1;

SeqControl(JUMP).argument = n; % set jump address to back to top of loop
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    % === add for multi-system sync ===
    if master
        Event(n).seqControl = [TTNA, MSSYNC];
    else
...
VSX    % setting 'filename' variable permits running VSX without query
