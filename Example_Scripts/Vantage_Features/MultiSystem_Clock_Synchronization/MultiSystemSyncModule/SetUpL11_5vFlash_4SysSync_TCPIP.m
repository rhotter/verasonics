% SetUpL11_4vFlash_4SysDataTransfer_SyncModule_TCPIP.m -
...
%
% Testing: Tested with software release 3.2.2 on four Vantage systems.
%
% Last update:
...
slave1IP = '192.168.0.2';
slave2IP = '192.168.0.3';
slave3IP = '192.168.0.4';
portNum = 11335;

if master
    tcpipMtoS1 = tcpip(slave1IP,portNum,'NetworkRole','server','TimeOut',30,'InputBufferSize',2^30);
    tcpipMtoS2 = tcpip(slave2IP,portNum,'NetworkRole','server','TimeOut',30,'InputBufferSize',2^30);
    tcpipMtoS3 = tcpip(slave3IP,portNum,'NetworkRole','server','TimeOut',30,'InputBufferSize',2^30);
else
...
filename = ('L11-5vFlash_4SysDataTransfer_SyncModule'); % assigning the matfile name to the variable 'filename' permits avoiding the VSX query
...
if master
    UI(3).Control = VsButtonControl('LocationCode','UserC1',...
...
if master
    fopen([tcpipMtoS1,tcpipMtoS2,tcpipMtoS3]);
else
    fopen(tcpipStoM);
end
...
VSX % setting 'filename' variable permits running automatically unpon running the setup script
...
function TransferRFcallback()
    tcpipMtoS1 = evalin('base','tcpipMtoS1');
    fwrite(tcpipMtoS1,1); % pass one command
    fprintf(tcpipMtoS1,'TransferRF = 1;');

    tcpipMtoS2 = evalin('base','tcpipMtoS2');
    fwrite(tcpipMtoS2,1); % pass one command
    fprintf(tcpipMtoS2,'TransferRF = 1;');

    tcpipMtoS3 = evalin('base','tcpipMtoS3');
    fwrite(tcpipMtoS3,1); % pass one command
    fprintf(tcpipMtoS3,'TransferRF = 1;');
...
function FreezeFunctionMaster(varargin)
    ...
    while isequal(get(findobj('String','Freeze'),'Value'),1)
        pause(0.25),
        fprintf(tcpipMtoS1,'freeze');
        fprintf(tcpipMtoS2,'freeze');
        fprintf(tcpipMtoS3,'freeze');
    end
    fprintf(tcpipMtoS1,'unfreeze');
    fprintf(tcpipMtoS2,'unfreeze');
    fprintf(tcpipMtoS3,'unfreeze');
end

function FreezeFunctionSlave(varargin)
...
function saveSettings()
    ...
    fwrite(tcpipMtoS1,length(command)) % pass command length
    for i = 1:length(command)
        fprintf(tcpipMtoS1,command{i});
    end

    fwrite(tcpipMtoS2,length(command)) % pass command length
    for i = 1:length(command)
        fprintf(tcpipMtoS2,command{i});
    end

    fwrite(tcpipMtoS3,length(command)) % pass command length
    for i = 1:length(command)
        fprintf(tcpipMtoS3,command{i});
    end
...
function DataTransferMaster(RDataMaster)
    ...
    tic
    disp('Wait for data transfer from Slave 1....');
    SlaveBuffer1 = fread(tcpipMtoS1,segmentSize,'int16');
    disp('Wait for data transfer from Slave 2....');
    SlaveBuffer2 = fread(tcpipMtoS2,segmentSize,'int16');
    disp('Wait for data transfer from Slave 3....');
    SlaveBuffer3 = fread(tcpipMtoS3,segmentSize,'int16');
    disp(['Duration_Of_Transfer = ',num2str(toc,'%2.3f'),' s']);

    % combine it for image reconstruction
    tic
    disp('Wait for data combination....');
    RDataSlave1 = zeros(dataSize,'int16');
    RDataSlave1(1:RcvDepth,:) = reshape(SlaveBuffer1,RcvDepth,dataSize(2),dataSize(3));
    RDataSlave2 = zeros(dataSize,'int16');
    RDataSlave2(1:RcvDepth,:) = reshape(SlaveBuffer2,RcvDepth,dataSize(2),dataSize(3));
    RDataSlave3 = zeros(dataSize,'int16');
    RDataSlave3(1:RcvDepth,:) = reshape(SlaveBuffer3,RcvDepth,dataSize(2),dataSize(3));
    combinedRF = [RDataMaster RDataSlave1 RDataSlave2 RDataSlave3];
    ...
    fprintf(tcpipMtoS1,'finished'); % tell the slave that data combination is completed
    fprintf(tcpipMtoS2,'finished'); % tell the slave that data combination is completed
    fprintf(tcpipMtoS3,'finished'); % tell the slave that data combination is completed
end
...
function waitForSlave()
    ...
    if ~strcmp(fscanf(tcpipMtoS1,'%s'),'ready')
        error('TCPIP communication with Slave 1 is not ready!');
    end
    if ~strcmp(fscanf(tcpipMtoS2,'%s'),'ready')
        error('TCPIP communication with Slave 2 is not ready!');
    end
    if ~strcmp(fscanf(tcpipMtoS3,'%s'),'ready')
        error('TCPIP communication with Slave 3 is not ready!');
    end
end
