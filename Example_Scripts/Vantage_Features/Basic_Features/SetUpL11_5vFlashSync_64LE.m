...
%   for its use
%
% File name: SetUpL11_5vFlashSync_64LE.m - Plane wave imaging with Recon/processing
%                                     synchronised with acquisition
%
% Description:
%   Sequence programming file for L11-5v Linear array, using a plane wave
%   transmit with 2-1 synthetic aperture acquisition synchronised to software
%   reconstruction/processing sequences. All 128 transmit and 64 receive
%   channels are active for each synthetic aperture acquisition.
%
% Last update:
...
Resource.DisplayWindow(1).Title = 'L11-5vFlashSync_64LE';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'Acquisition first half of aperture.';
    Event(n).tx = 1;
    Event(n).rcv = 2*i-1;
    Event(n).recon = 0;
    Event(n).process = 0;
    ...
    Event(n).info = 'Acquisition second half of aperture.';
    Event(n).tx = 1;
    Event(n).rcv = 2*i;
    Event(n).recon = 0;
    Event(n).process = 0;
...
save('MatFiles/L11-5vFlashSync_64LE');
...
% filename = 'L11-5vFlashSync_64LE';  VSX;
