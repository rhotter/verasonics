...
%   for its use
%
% File name: SetUpL11_5vFlashAngles_SimulateOnlyWvfm.ms
% Description:
%   This is a duplicate of the L11_5vFlashAngles script, modified to
%   demonstrate use of TW.type 'sampled', a simulate-only waveform type
%   that cannot be used with a hardware system.  Search for the comment
%   string **SimulateOnlyWvfm** for comments explaining definition of the
%   TW 'sampled' waveform type and other changes in script neeeded to use
%   it.
...
% 4/21/2020 - created for 4.3.0 software release
...
% **SimulateOnlyWvfm** We have to tell the system this is a simulate-only
% script so it should not try to run with the hardware system.  If this is
% not done, hardware operation will be assumed by default and the
% simulate-only waveform will trigger an error, since hardware operation
% requires the "States" transmit waveform array which will not be present.
%
% Note that if you do not make this declaration in your script, and if
% verbose level is set to 2 or greater, VSX will automatically default to
% simulate-only operation with a status message to the user.  You can test
% this by commenting out the following line.
...
Resource.DisplayWindow(1).Title = 'L11-5vFlashAngles_SimulateOnlyWvfm';
...
% **SimulateOnlyWvfm**
% Define the transmit waveform as one cycle of a sine wave at
% Trans.frequency, using the TW 'sampled' waveform type.  Note 'parametric'
% waveform definition above has been commented out, but left in place for
% reference.
TW(1).type = 'sampled';
samplesPerCycle = round(250/Trans.frequency);
% The sampled type requires waveform samples at 250 MHz transmit clock
% rate.
T = 0:(2*pi/samplesPerCycle):(2*pi); % "Time" steps in radian units
TW(1).Waveform = sin(T); % this creates our sinusoidal waveform array with peak amplitude of 1.0
TW(1).numsamples = samplesPerCycle + 1; % add one since T includes zero sample at start of second cycle
...
UI(2).Control = VsSliderControl('LocationCode','UserA1',...
    ...
    'ValueFormat','%3.0f');
UI(2).Callback = @RangeChangeCallback;

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 5;

% Save all the structures to a .mat file.
save('MatFiles/L11-5vFlashAnglesSimOnly');
...
% filename = 'L11-5vFlashAnglesSimOnly';  VSX;
