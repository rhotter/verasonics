...
%   for its use
%
% File name: SetUpL11_4vFlashTImeTag.m - Example of using the Time Tag
% function
% Description:
...
SeqControl(2).argument = 1e5;  % 100 msec
...
UI(3).Control = VsSliderControl('LocationCode','UserB5',...
    'Label','Time Tag','SliderMinMaxVal',[0,2,TimeTagEna],...
    'SliderStep',[0.5, 0.5],'ValueFormat','%1.0f',...
    'Callback',@TimeTagCallback);

% - PRI control
UI(4).Control = VsSliderControl('LocationCode','UserB3',...
    'Label','PRI msec','SliderMinMaxVal',[10,500,100],...
    'SliderStep',[1/49, 1/49],'ValueFormat','%1.0f',...
    'Callback',@PRICallback);
...
save('MatFiles/L11-4vFlashTimeTag');
...
% filename = 'L11-4vFlashTimeTag';  VSX;
...
function PRICallback(~, ~, UIValue)
    PRI = 10*round(UIValue/10); % steps of 10 msec
    SeqControl = evalin('base', 'SeqControl');
    SeqControl(2).argument = 1000*PRI; % argument is in usec
