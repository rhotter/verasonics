...
PData(1).PDelta = [Trans.spacing/3, 0, 0.25];
...
% Define a storage class for each Buffer
    % Save Data Process
Process(3).classname  = 'External'; % define the storage class name
Process(3).method     = 'myImagePlot';    % to export files specify 'save'
Process(3).Parameters = { ...
    'srcbuffer', 'imageP', ...'receive',... % name of buffer to export.
    'srcbufnum',    1,    ... % buffer number of buffer to export
    'srcframenum', -1,    ... % process the last frame in a RcvBuffer
    'dstbuffer',   'none',... % destination buffer (optional)
};
...
UI(2).Control = VsButtonControl('LocationCode','UserB2',...
    'Label','Start','Callback',@StartCallback);

UI(3).Control = VsSliderControl('LocationCode','UserA1',...
...
EF(1).Function = vsv.seq.function.ExFunctionDef('myImagePlot', @myImagePlot);

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 5;

% Save all the structures to a .mat file.
save('MatFiles/L11-5vFlashAngles_Export_Image');
...
% filename = 'L11-5vFlashAngles_Export_Image';  VSX;


%% **** Callback routines used by External function definition (EF) ****

function myImagePlot(I)
    persistent fh
    if isempty(fh) || ~ishandle(fh)
        fh = figure;
    end
    imagesc(gca(fh), log(I));
    % imagesc(gca(fh), log(abs(I+Q).^2));
end
