...
%   for its use.
%
% File name: SetUp_ExtIO_PulseOnPosition.m - Example of extended IO for
% pulse on position operation using encoder input.
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el phased array, using
%   total focusing reconstruction. Acquisition for each frame occurs upon
%   reaching specified encoder tick increment.
%
% Last update:
% 10/28/2020 
...
Resource.ExtIO.createEncoder( 'NumEncoder', 3, 'Decoding', ["X4" "X4" "X4"]); %specifes 3 encoder inputs with X4 decoding
Resource.ExtIO.Encoder.PopSetting.EncNumber = 1; %specify ecodder to use for pulse on position
Resource.ExtIO.Encoder.PopSetting.IncrementTick = 100; %Pulse on position tick increment
Resource.ExtIO.Encoder.PopSetting.StartTick = 50; %Position for first acquisition
Resource.ExtIO.Encoder.PopSetting.NumSteps = 10 ;%Number of position steps to acquire at
...
Process(2).method     = 'vsv.extio.process.waitForPosition'; %Pauses software sequence until next tick increment reached
...
for i = 1:Resource.RcvBuffer(1).numFrames
        ...
        Event(n).info = 'Wait for position';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 2; %calling waitForPosition function
        Event(n).seqControl = 4; %Syncronising so that hwardware sequence waits for position increment
...
save('MatFiles/ExtIO_PulseOnPosition');
...
% filename = 'ExtIO_PulseOnPosition'; VSX;
