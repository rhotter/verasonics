...
%   for its use.
%
% File name: SetUp_ExtIO_AnalogueInput_Sampled.m - Example of extended IO for
% sampled reading of analog inputs.  
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el phased array, using
%   total focusing reconstruction. Scalar voltages for each analog input
%   are sampled at specfied rate and buffered internally on IO card.  The
%   acquisition is initiated by the Vantage hardware trigger at the first
%   trasmit event for accurate timing of Vantage and extended IO
%   acquisitions.  Available buffer is read and plotted once per frame
%   using external function. Once acquired, the sampled analog data can
%   be read into the matlab buffer using the function
%   vsv.extio.process.readAllInput();
%
% Last update:
% 10/27/2020 
...
Resource.ExtIO.createAnalogInput(3);
% using struct would work too
Resource.ExtIO.AnalogInput = vsv.extio.AnalogInputSetting(3);
Resource.ExtIO.AnalogInput.Mode = "single-ended"; %specifies connection type
Resource.ExtIO.AnalogInput.Range = 10; %specifes peak voltage range

Resource.ExtIO.AnalogInput.AcqType = "sampled";  %specifes sampled acquisition
Resource.ExtIO.AnalogInput.Samples = 1000; %Numer of samples to acquire
Resource.ExtIO.AnalogInput.SampleRate = 100; %Sampling rate (Hz)
Resource.ExtIO.AnalogInput.SampledAcqTrigger = "hardwareSingle"; %extended IO sampling initiated by hardware trigger

Resource.ExtIO.AnalogInput.BufferSetting.BufferLength = 1;%length of matlab buffer for analog reads
Resource.ExtIO.AnalogInput.BufferSetting.BufferType = "Circular"; %buffer type for encoder reads "Fixed" or "Circular"


% %Specify extended IO parameters
% Resource.ExtIOAnalogInputSetup = vsv.extio.AnalogInputSetting(3); %creates 3 extended IO analog inputs
% Resource.ExtIOAnalogInputSetup.Mode="single-ended"; %specifies connection type
% Resource.ExtIOAnalogInputSetup.Range=10; %specifes peak voltage range
% 
% Resource.ExtIOAnalogInputSetup.AcqType="sampled";  %specifes sampled acquisition
% Resource.ExtIOAnalogInputSetup.Samples=1000; %Numer of samples to acquire
% Resource.ExtIOAnalogInputSetup.SampleRate=100; %Sampling rate (Hz)
% Resource.ExtIOAnalogInputSetup.SampledAcqTrigger="hardwareSingle"; %extended IO sampling initiated by hardware trigger
% 
% Resource.ExtIOAnalogInputSetup.BufferSetting.BufferLength = 1;%length of matlab buffer for analog reads
% Resource.ExtIOAnalogInputSetup.BufferSetting.BufferType = "Circular"; %buffer type for encoder reads "Fixed" or "Circular"
...
Process(1).Parameters = {'imgbufnum',1,...   % number of buffer to process.
                         ...
                         'displayWindow',1};
%Encoder read process
Process(2).classname  = 'External'; %External process function
Process(2).method     = 'DisplayAI'; %Plot analog input buffer
...
SeqControl(5).command = 'triggerOut'; %Vantage output trigger
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:P.Elnum                 % Acquire frames
        ...
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = [2 5]; %Vantage output trigger on first transmit event (will trigger extended IO sampling of analog channe;s)
...
EF(1).Function = vsv.seq.function.ExFunctionDef('DisplayAI', @DisplayAI);

% Save all the structures to a .mat file.
save('MatFiles/ExtIO_AnalogInput_Sampled');
...
% filename = 'ExtIO_AnalogInput_Sampled'; VSX;
...
function DisplayAI() 
%External function to plot analog buffer for channel 1
    
    %Read buffer from extended IO device
    vsv.extio.process.readAllInput(); 
        
    % get the result buffer
    result  = vsv.extio.Util.getAnalogInputResult();
    % read data from channel 1
    aiData = result.Data(1,:);

    % get the axes handle
    axHandle = aiAxesHandle(aiData);
    
    % Plot the encoder data.
    plot(axHandle, aiData);
    drawnow( 'limitrate' );
    
end

function handle = aiAxesHandle(aiData)
% returns the analog axes handle

    persistent axesHandle
    
    % Create the figure if it does not exist.
    if isempty(axesHandle)||~ishandle(axesHandle)
        figure('name','Analogue channel 1 (V)','NumberTitle','off');
        axesHandle = axes('XLim',[0,length(aiData)],'YLim',[-10 10], ...
