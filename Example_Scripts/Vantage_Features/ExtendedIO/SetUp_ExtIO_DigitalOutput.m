...
%   for its use.
%
% File name: SetUp_ExtIO_DigitalOutput.m - Example of extended IO for
% digital output waveforms.  
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el phased array, using total
%   focusing reconstruction. Digital seqeunces output from two
...
data_tmp=zeros(1,100); data_tmp(1:50)=1;
data_tmp2=zeros(1,100); data_tmp2(1:10)=1;

digital_waveform=[data_tmp; data_tmp2];

Resource.ExtIO = vsv.extio.Setup();
Resource.ExtIO.createDigitalOutput( 2 ); %Creating two analogue output channels
Resource.ExtIO.DigitalOutput.SampleRate = 100; %Output samples read at 100Hz
Resource.ExtIO.DigitalOutput.Waveform = digital_waveform;
Resource.ExtIO.DigitalOutput.Repeat = "continuous"; %Output waveform will be repeated continuously
Resource.ExtIO.DigitalOutput.Trigger = "softwareEvent";
...
Process(2).method     = 'vsv.extio.process.digitalOutputSoftwareTrigger';
...
save('MatFiles/ExtIO_DigitalOutput');
...
%filename = 'ExtIO_DigitalOutput'; VSX;
