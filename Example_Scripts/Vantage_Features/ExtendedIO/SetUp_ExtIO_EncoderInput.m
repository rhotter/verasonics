...
%   for its use.
%
% File name: SetUp_ExtIO_EncoderInput.m - Example of extended IO for
% reading encoder inputs.  
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el phased array, using total
%   focusing reconstruction. Specifed encoder inputs are read once per
%   frame and stored in buffer. Current encoder data buffer plotted each
%   frame.
%
% Last update:
% 10/27/2020 
...
Resource.ExtIO.createEncoder( 'NumEncoder', 3, 'Decoding', ["X4" "X4" "X4"] );
Resource.ExtIO.Encoder.BufferSetting.BufferLength = 100;
Resource.ExtIO.Encoder.BufferSetting.BufferType   = "Circular";
...
%Encoder buffer display process
Process(3).classname  = 'External'; %External process function
Process(3).method     = 'DisplayPosition'; %External function to plot buffer encoder buffer
...
SeqControl(4).command = 'sync'; %sync sequence control to ensure IO reads matched to RF frames
...
for i = 1:Resource.RcvBuffer(1).numFrames
        ...
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 2; %Encoder read process
        Event(n).seqControl = 4; %Sync
    ...
    Event(n).info = 'display encoder data';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = 3; %plot encoder data external process
...
save('MatFiles/ExtIO_EncoderInput');
...
% filename = 'ExtIO_EncoderInput'; VSX;
...
%External function to plot encoder buffer for channel 1
    
    % get the result buffer
    buffer  = vsv.extio.Util.getEncoderResults();
    % get the encoder data at channel 1
    encData = buffer.Data(1,:);

    axHandle = encAxesHandle(encData);
    
    % returns an axes handle, creates a new figure if handle not exist
    % Plot the encoder data.
    plot(axHandle, encData);
    drawnow( 'limitrate' );

end

function handle = encAxesHandle( encData )
% returns the encoder axes handle

    persistent axesHandle

    % Create the figure if it does not exist.
    if isempty(axesHandle)|| ~ishandle(axesHandle)
        figure('name','Encoder count','NumberTitle','off');
        axesHandle = axes('XLim',[0,length(encData)],'YLim',[-1000 1000], ...
