...
%   for its use.
%
% File name: SetUp_ExtIO_AnalogueInput_OnDemand.m - Example of extended IO for
% on-demand reading of analog inputs.  
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el phased array, using total
%   focusing reconstruction. Scalar voltages for each analog input channel are read once per
%   frame and stored in buffer. The extended io analog buffer is plotted
%   and updated once per frame.
%
% Last update:
% 10/28/2020 
...
Resource.ExtIO.AnalogInput.AcqType = "OnDemand";  %specifes on demand sampling
Resource.ExtIO.AnalogInput.Range=10; %specifes peak voltage range

Resource.ExtIO.AnalogInput.BufferSetting.BufferLength = 100;%length of buffer for analog reads
...
Process(2).method     = 'vsv.extio.process.readAllInput'; %Read all available inputs when called and add to extended IO buffers
Process(2).Parameters = {'srcbuffer','none','srcbufnum','none'}; %no input or output buffers for extended IO functions

%Analogue input buffer display process
Process(3).classname  = 'External'; %External process function
Process(3).method     = 'DisplayPosition'; %External function to plot buffer analog buffer
Process(3).Parameters = {'srcbuffer','none','srcbufnum','none'}; %no input or output buffers for extended IO functions
...
for i = 1:Resource.RcvBuffer(1).numFrames
        ...
        Event(n).info = 'Read ExtIO inputs';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 2;
    ...
    Event(n).info = 'display extio data';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = 3;
...
EF(1).Function = vsv.seq.function.ExFunctionDef('DisplayPosition', @DisplayPosition);


% Save all the structures to a .mat file.
save('MatFiles/ExtIO_AnalogInput_OnDemand');
...
%filename = 'ExtIO_AnalogInput_OnDemand'; VSX;
...
function DisplayPosition() %External function to plot analog buffer for channel 1
