function RFout = externalFIR_CPU_M(RFin)
% RFout = externalFIR_CPU_M(RFin)
%
%   Simple 1-D FIR Filter on 2D data.
%   Test for comparison to Matlab GPU version and mex GPU version
%   
...
if isempty(xzp)
    xzp = [ single(RFin); zeros(Nfft-N,chs,'single')];
else
    xzp(1:N,:)=single(RFin);
end
...
%% -- Output data --%
RFout=int16(y(L/2+[1:N],:));  %Verasonics is looking for int16
