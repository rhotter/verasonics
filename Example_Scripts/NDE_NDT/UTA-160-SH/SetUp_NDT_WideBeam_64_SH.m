...
%   for its use.
%
% File name: SetUp_NDT_WideBeam_64_SH.m - Example of imaging using overlapping
% widebeams
%
% Description:
%   Sequence programming file for Imasonic 5MHz 64 el Phase array, using overlapping widebeams.
...
P.numRays = 24;     % no. of raylines to program
P.startDepth = 1;
...
P.aperture = P.Elnum*Trans.spacing; % aperture based on 64 elements
...
Resource.DisplayWindow(1).Title = ['WideBeam' num2str(P.numRays) 'Acqs'];
Resource.DisplayWindow(1).pdelta = PData(1).PDelta(1)/4;
...
ElOffset = floor((Trans.numelements - P.Elnum)/2); if ElOffset<0, error('reset m to a value smaller than Trans.numelements.'); end
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'focus', P.txFocus, ...
                   'Steer', [0.0,0.0], ...
                   'Apod', [zeros(1,ElOffset), ones(1,P.Elnum), zeros(1,Trans.numelements-P.Elnum-ElOffset)], ...  % set TX.Apod for 64 elements
                   'Delay', zeros(1,min([Trans.numelements, Resource.Parameters.numTransmit])), ...
                   'TXPD', [], ...
                   'peakCutOff', 1.5, ...
...
TGC.CntrlPts = [0,518,637,652,741,862,993,1023];
...
save('MatFiles/NDT_WideBeam_64_SH');
...
% filename = 'NDT_WideBeam_64_SH'; VSX;
