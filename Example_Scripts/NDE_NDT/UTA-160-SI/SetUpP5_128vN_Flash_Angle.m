...
%   for its use.
%
% File name: SetUpP5_128vN_FlashAngle.m - Example of imaging using tilted plane wave
%                                         compounding.
%
% Description:
%   Sequence programming file for P5-128vN array, using total focusing reconstruction
%   using plane wave compounding.  For each acquisition, all channels are active 
...
P.Elnum = 128;      % num of el in transmit aperture, must be no more than Trans.numelements
P.numAngles = 11;   % Number of angles

if P.numAngles > 1
    dtheta = (60*pi/180)/(P.numAngles-1); startAngle = -60*pi/180/2;  % set dtheta to range over +/- 30 degrees.
else
...
Resource.RcvBuffer(1).rowsPerFrame = 2560*P.numAngles;
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 50;     % 10 frames for RF data.
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = ['FlashAngles' num2str(P.numAngles) 'Acqs'];
...
% Specify P.numAngles TX structure arrays. Transmit on element n in the array for event n.
TX = repmat(struct('waveform', 1, ...
                   'Origin', zeros(1,3), ...
                   'focus', -P.radius, ...
                   'Steer', [0.0,0.0], ...
                   'Apod', [zeros(1,ElOffset), ones(1,P.Elnum), zeros(1,Trans.numelements-P.Elnum-ElOffset)], ...
                   'Delay', zeros(1,min([Trans.numelements, Resource.Parameters.numTransmit]))), 1, P.numAngles);

% - Set event specific TX attributes.
for n = 1:P.numAngles   % P.numAngles transmit events
...
% - We need P.numAngles Receive structures for each frame.
maxAcqLength = ceil(sqrt(P.aperture^2 + P.endDepth^2 - 2*P.aperture*P.endDepth*cos(P.theta-pi/2)) - P.startDepth);
wlsPer128 = 128/(2*4); % wavelengths in 128 samples for 2 samplesPerWave
...
%   P.numAngles ReconInfo structures, since we are using P.numAngles
...
if P.numAngles>1
    ...
    ReconInfo(P.numAngles).mode = 'accumIQ_replaceIntensity'; % accum & detect
else
...
SeqControl(4).argument = 33000;  % 33 ms
nsc = 5; % nsc is count of SeqControl objects

% Specify Event structure array.
n = 1;
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:P.numAngles                 % Acquire frames
        Event(n).info = 'Acquisition.';
        Event(n).tx = j;
        Event(n).rcv = P.numAngles*(i-1)+j;
        Event(n).recon = 0;
        Event(n).process = 0;
...
save('MatFiles/P5_128vN_Flash_Angle');
...
% filename = 'P5_128vN_Flash_Angle'; VSX;
