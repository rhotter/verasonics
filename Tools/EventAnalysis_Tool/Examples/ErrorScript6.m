% ErrorScript6 has a bug about the uniqueness of the transferToHost in the event sequence.
% EventAnalysisTool shows an error message if a transferToHost SeqControl
% is used in multiple events.
...
SeqControl(10).command = 'transferToHost';  % DMA for Bmode
SeqControl(11).command = 'transferToHost';  % DMA for Doppler
nsc = 12;  % next SeqControl number
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n-1).seqControl = [2,3,10];   % replace last 2D acquisition Event's seqControl
    ...
    Event(n-1).seqControl = [5,6,11]; % replace last Doppler acquisition Event's seqControl
