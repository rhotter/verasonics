% ErrorScript3 has a bug about Recon/ReconInfo
% A image frame consists of 7 acqs (7 angles), so the framenum shown in
% the ReconInfo.rcvnum should be consistent.
% EventAnalysisTool shows a warning message about the sonsistency of
% frameNum or BufferNum in Recon/ReconInfo
% Incorrect Recon/ReconInfo setting may crash Matlab!!
...
Resource.DisplayWindow(1).Title = 'Error3';
...
Recon = struct('senscutoff', 0.6, ...
               'pdatanum', 1, ...
               'rcvBufFrame',1, ...       % changed -1 to 1
               'IntBufDest', [1,1], ...
               'ImgBufDest', [1, 1], ...  % changed [1, -1] to [1, 1]
               'RINums',(1:na*Resource.RcvBuffer(1).numFrames)');

% Define ReconInfo structures.
ReconInfo = repmat(struct('mode', 'replaceIQ', ...  % default is to accumulate IQ data.           % changed 'mode' 4 to 'mode' 3
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 1), 1, na*Resource.RcvBuffer(1).numFrames);

for j = 1:na*Resource.RcvBuffer(1).numFrames
    ReconInfo(j).txnum = mod(j-1, na) + 1;
...
n = 1;
for i = 1:na*Resource.RcvBuffer(1).numFrames
    Event(n).info = 'Full aperture';
    Event(n).tx = mod(i-1, na) + 1;
    Event(n).rcv = i;
    Event(n).recon = 0;
    Event(n).process = 0;
